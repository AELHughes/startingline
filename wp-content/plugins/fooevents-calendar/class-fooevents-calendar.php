	...
	public function fetch_events( $events, $display_type, $include_desc = true ) {
		...
		foreach ( $events as $event ) {
			...
			if ( '' !== $event_timezone ) {
                ...
                // Add City (saved by your portal as _foop_city) and Category
                $city = get_post_meta( $event->ID, '_foop_city', true );
                if ( $city === '' ) {
                    // optional fallback: first product tag as city
                    $tags = wp_get_post_terms( $event->ID, 'product_tag', array( 'fields' => 'names' ) );
                    if ( ! empty( $tags ) ) { $city = $tags[0]; }
                }
                
                $cats      = wp_get_post_terms( $event->ID, 'product_cat', array( 'fields' => 'names' ) );
                $category  = ! empty( $cats ) ? implode( ', ', $cats ) : '';

			if ( 'bookings' !== $event_type ) {

                $city_meta = get_post_meta( $event->ID, '_foop_city', true ); // we saved city here earlier
                $city      = $city_meta ?: '';                                  // fall back to empty
                
                $cat = '';
                $terms = get_the_terms( $event->ID, 'product_cat' );
                if ( $terms && ! is_wp_error( $terms ) ) {
                    // pick the first/top level term for display
                    $cat = $terms[0]->name;
