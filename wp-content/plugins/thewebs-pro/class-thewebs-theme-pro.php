...
final class Thewebs_Theme_Pro {
	...
	/**
	 * Main Thewebs_Theme_Pro Instance.
	 *
	 * Insures that only one instance of Thewebs_Theme_Pro exists in memory at any one
	 ...
	 *
	 * @return Thewebs_Theme_Pro The one true Thewebs_Theme_Pro
	 */
	...
	public function __clone() {
		...
		_doing_it_wrong( __FUNCTION__, esc_html__( 'Cheatin&#8217; huh?', 'thewebs-pro' ), '1.0' );
	}
	...
	private static function is_instantiated() {
		...
		if ( ! empty( self::$instance ) && ( self::$instance instanceof Thewebs_Theme_Pro ) ) {
			return true;
		}
	...
	private static function setup_instance( $file = '' ) {
		self::$instance       = new Thewebs_Theme_Pro();
	...
	public function include_files() {
		...
		$enabled = json_decode( get_option( 'thewebs_pro_theme_config' ), true );
	...
	public function basic_css_menu_support() {
		wp_register_style( 'thewebs-pro-admin', false );
		wp_enqueue_style( 'thewebs-pro-admin' );
		$css = '#menu-appearance .wp-submenu a[href^="themes.php?page=thewebs-"]:before, #menu-appearance .wp-submenu a[href^="edit.php?post_type=thewebs_element"]:before, #menu-appearance .wp-submenu a[href^="edit.php?post_type=kt_font"]:before {content: "\21B3";margin-right: 0.5em;opacity: 0.5;}';
		wp_add_inline_style( 'thewebs-pro-admin', $css );
	}
	/**
	 * On Load
	 */
	public function include_frontend() {
		add_shortcode( 'thewebs_breadcrumbs', array( $this, 'output_thewebs_breadcrumbs' ) );
	}
	/**
	 * On Load
	 */
	public function output_thewebs_breadcrumbs( $atts ) {
		$args = shortcode_atts(
			array(
				'show_title' => true,
			),
		...
		$output = '';
		if ( function_exists( 'Thewebs\thewebs' ) ) {
			ob_start();
				Thewebs\thewebs()->print_breadcrumb( $args );
			$output = ob_get_clean();
		}
		return $output;
	}
	...
	public function admin_notice_need_thewebs_theme() {
		if ( get_transient( 'thewebs_theme_pro_free_theme_notice' ) || ! current_user_can( 'manage_options' ) ) {
			return;
		}
	...
	public function register_api_endpoints() {
		$controller = new Thewebs_Pro\Post_Select_Controller();
 ...
 */
function thewebs_theme_pro() {
	return Thewebs_Theme_Pro::instance();
}
