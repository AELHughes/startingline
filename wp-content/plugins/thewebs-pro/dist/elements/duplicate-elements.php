<?php
/**
 * Class Thewebs_Pro\Duplicate_Elements
 *
 * @package Thewebs Pro
 */
...
class Duplicate_Elements {
	...
	public function __construct( $slug = '' ) {
		if ( ! empty( $slug ) ) {
			$this->slug = $slug;
			add_action( "admin_action_thewebs_duplicate_{$slug}", array( $this, 'duplicate_action' ) );
	...
	public function dupe_link( $actions, $post ) {
		...
		if ( ! current_user_can( apply_filters( 'thewebs_duplicate_post_capability', 'manage_options' ) ) ) {
			return $actions;
		}
		...
		$actions['duplicate'] = '<a href="' . wp_nonce_url( admin_url( 'edit.php?post_type=' . $this->slug . '&action=thewebs_duplicate_' . $this->slug . '&amp;post=' . $post->ID ), 'thewebs_duplicate_post_' . $post->ID ) . '" aria-label="' . esc_attr__( 'Make a duplicate from this.', 'thewebs-pro' )
			. '" rel="permalink">' . esc_html__( 'Duplicate', 'thewebs-pro' ) . '</a>';
	...
	public function duplicate_action() {
		if ( empty( $_REQUEST['post'] ) ) {
			wp_die( esc_html__( 'No post to duplicate has been supplied!', 'thewebs-pro' ) );
		}

		$post_id = isset( $_REQUEST['post'] ) ? absint( $_REQUEST['post'] ) : '';
		check_admin_referer( 'thewebs_duplicate_post_' . $post_id );

		$clone_post = get_post( $post_id );

		if ( false === $clone_post ) {
			/* translators: %s: product id */
			wp_die( sprintf( esc_html__( 'Post creation failed, could not find original post: %s', 'thewebs-pro' ), esc_html( $post_id ) ) );
		}
	...
	public function duplicate( $post ) {
		...
		$title              = sprintf( esc_html__( '%s (Copy)', 'thewebs-pro' ), $post->post_title );
		...
		if ( $new_post_id !== 0 && ! is_wp_error( $new_post_id ) ) {
			...
			do_action( 'thewebs_elements_duplicate_post', $new_post_id, $post );
		}
	...
	public function duplicate_meta_info( $new_id, $post ) {
		 ...
		 * @return array
		 */
		$meta_blacklist = apply_filters( 'thewebs_elements_duplicate_post_exclude_list', $meta_blacklist );
		 ...
		 * @return array
		 */
		$meta_keys = apply_filters( 'thewebs_elements_duplicate_meta_keys', $meta_keys );
