...
(function() {
	...
	window.thewebsProSticky = {
		...
		stickHeadItem: function( element ) {
			var offsetTop = parseInt( window.thewebsProSticky.getOffset( document.getElementById( 'wrapper' ) ).top ),
				activeOffsetTop = parseInt( window.thewebsProSticky.getOffset( element ).top );
			...
			window.addEventListener( 'thewebs-update-sticky', updateSticky, false );
			updateSticky();
		},
		/**
		 * Find the sticky items.
		...
		stickTransHeadItem: function( element ) {
			var offsetTop = window.thewebsProSticky.getOffset( document.body ).top,
				activeOffsetTop = window.thewebsProSticky.getOffset( element ).top;
			var updateSticky = function( e ) {
				var parent = element.parentNode;
				parent.style.height = element.offsetHeight + 'px';
				if ( Math.max(0, window.scrollY ) === Math.floor( activeOffsetTop - offsetTop ) ) {
		...
		stickScrollItem: function( element ) {
			var updateSticky = function( e ) {
				var offsetTop = window.thewebsProSticky.getOffset( document.getElementById( 'wrapper' ) ).top,
		...
		initStickyHeadItems: function() {
			var stickyHeadItems = document.querySelectorAll( '.thewebs-pro-fixed-above' );
			// No point if no drawers.
			if ( ! stickyHeadItems.length ) {
				return;
			}
			for ( let i = 0; i < stickyHeadItems.length; i++ ) {
				window.thewebsProSticky.stickHeadItem( stickyHeadItems[ i ] );
			}
		},
		/**
		 * Find the sticky items.
		...
		initStickyHeadTransItems: function() {
			var stickyHeadTransItems = document.querySelectorAll( '.thewebs-pro-fixed-above-trans' );
			// No point if no drawers.
			if ( ! stickyHeadTransItems.length ) {
				return;
			}
			for ( let i = 0; i < stickyHeadTransItems.length; i++ ) {
				window.thewebsProSticky.stickTransHeadItem( stickyHeadTransItems[ i ] );
			}
		},
		/**
		 * Find the sticky items.
		...
		initStickyScrollItems: function() {
			var stickyScrollItems = document.querySelectorAll( '.thewebs-pro-fixed-on-scroll' );
			// No point if no drawers.
			if ( ! stickyScrollItems.length ) {
				return;
			}
			for ( let i = 0; i < stickyScrollItems.length; i++ ) {
				window.thewebsProSticky.stickScrollItem( stickyScrollItems[ i ] );
			}
		},
		/**
		 * Find the sticky items.
		...
		initStickyScrollFooterItems: function() {
			var stickyScrollItemsFooter = document.querySelectorAll( '.thewebs-pro-fixed-on-scroll-footer' );
			// No point if no drawers.
			if ( ! stickyScrollItemsFooter.length ) {
				return;
			}
			for ( let i = 0; i < stickyScrollItemsFooter.length; i++ ) {
				window.thewebsProSticky.stickScrollFooterItem( stickyScrollItemsFooter[ i ] );
			}
		},
		/**
		 * Find the sticky items.
		...
		initStickyScrollFooterItemsSpace: function() {
			var stickyScrollItemsFooter = document.querySelectorAll( '.thewebs-pro-fixed-on-scroll-footer-space' );
			// No point if no drawers.
			if ( ! stickyScrollItemsFooter.length ) {
				return;
			}
			for ( let i = 0; i < stickyScrollItemsFooter.length; i++ ) {
				window.thewebsProSticky.stickScrollFooterItemSpace( stickyScrollItemsFooter[ i ] );
			}
		},
		/**
		 * Find the sticky items.
		...
		initStickyFootItems: function() {
			var stickyFootItems = document.querySelectorAll( '.thewebs-pro-fixed-below' );
			// No point if no sticky
			if ( ! stickyFootItems.length ) {
				return;
			}
			for ( let i = 0; i < stickyFootItems.length; i++ ) {
				window.thewebsProSticky.stickFootItemBelow( stickyFootItems[ i ] );
			}
		},
		/**
		 * Find the sticky items.
		...
		initStickyFootOnItems: function() {
			var stickyFootOnItems = document.querySelectorAll( '.thewebs-pro-fixed-bottom' );
			// No point if no sticky
			if ( ! stickyFootOnItems.length ) {
				return;
			}
			for ( let i = 0; i < stickyFootOnItems.length; i++ ) {
				window.thewebsProSticky.stickFootItem( stickyFootOnItems[ i ] );
			}
		},
		// Initiate sticky when the DOM loads.
		init: function() {
			window.thewebsProSticky.initStickyHeadItems();
			window.thewebsProSticky.initStickyHeadTransItems();
			window.thewebsProSticky.initStickyScrollItems();
			window.thewebsProSticky.initStickyScrollFooterItems();
			window.thewebsProSticky.initStickyFootItems();
			window.thewebsProSticky.initStickyFootOnItems();
			window.thewebsProSticky.initStickyScrollFooterItemsSpace();
		}
	}
	if ( 'loading' === document.readyState ) {
		// The DOM has not yet been loaded.
		document.addEventListener( 'DOMContentLoaded', window.thewebsProSticky.init );
	} else {
		// The DOM has already been loaded.
		window.thewebsProSticky.init();
	}
})();