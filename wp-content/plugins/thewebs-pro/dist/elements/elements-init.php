<?php
/**
 * Class Thewebs_Pro\Elements_Controller
 *
 * @package Thewebs Pro
 */
...
class Elements_Controller {
	...
	public function __construct() {
		...
		add_action( 'thewebs_single_product_ajax_added_to_cart', array( $this, 'init_mini_cart_hooks' ) );
		add_filter( 'thewebs_post_layout', array( $this, 'element_single_layout' ), 99 );
		add_action( 'init', array( $this, 'element_gutenberg_template' ) );
		add_action( 'init', array( $this, 'register_ace_script_block' ) );
		add_action( 'thewebs_theme_admin_menu', array( $this, 'create_admin_page' ) );
		...
		add_action( 'wp_ajax_thewebs_elements_change_status', array( $this, 'ajax_change_status' ) );
		if ( class_exists( 'Thewebs_Pro\Duplicate_Elements' ) ) {
	...
	public function action_enqueue_admin_scripts() {
		...
		if ( 'edit-' . self::SLUG === $current_page->id ) {
			...
			wp_enqueue_style( 'thewebs-elements-admin', KTP_URL . 'dist/elements/thewebs-pro-element-post-admin.css', false, KTP_VERSION );
			wp_enqueue_script( 'thewebs_elements-admin', KTP_URL . 'dist/elements/thewebs-pro-element-post-admin.min.js', array( 'jquery' ), KTP_VERSION, true );
			wp_localize_script(
				'thewebs_elements-admin',
				'thewebs_elements_params',
				array(
					'ajax_url'   => admin_url( 'admin-ajax.php' ),
					'ajax_nonce' => wp_create_nonce( 'thewebs_elements-ajax-verification' ),
					'draft' => esc_attr__( 'Draft', 'thewebs-pro' ),
					'publish' => esc_attr__( 'Published', 'thewebs-pro' ),
				)
			);
		}
	}
	...
	public function ajax_change_status() {
		check_ajax_referer( 'thewebs_elements-ajax-verification', 'security' );

		if ( ! isset ( $_POST['post_id'] ) || ! isset( $_POST['post_status'] ) ) {
			wp_send_json_error( __( 'Error: No post information was retrieved.', 'thewebs-pro' ) );
		}
	...
	public function admin_filter_results( $query ) {
		...
		if ( ! ( isset( $query->query['post_type'] ) && 'thewebs_element' === $query->query['post_type'] ) ) {
			return $query;
		}
		$screen = get_current_screen();
		if ( ! empty( $screen ) && $screen->id == 'edit-thewebs_element' ) {
	...
	public function admin_print_tabs( $views ) {
		...
		?>
		<div id="thewebs-element-tabs-wrapper" class="nav-tab-wrapper">
			<a class="nav-tab<?php echo esc_attr( $active_class ); ?>" href="<?php echo esc_url( $baseurl ); ?>">
				<?php echo esc_html__( 'All Element Items', 'thewebs-pro' ); ?>
			</a>
			<?php
			$types = array(
				'default' => array( 
					'label' => __( 'Sections', 'thewebs-pro' ),
				),
				'fixed' => array( 
					'label' => __( 'Fixed Items', 'thewebs-pro' ),
				),
				'template' => array( 
					'label' => __( 'Templates', 'thewebs-pro' ),
				),
				'script' => array( 
					'label' => __( 'HTML code', 'thewebs-pro' ),
				),
			);
	...
	public function init_classic_metabox() {
		$path = KTP_URL . 'build/';
		wp_enqueue_style( 'thewebs-element-meta', KTP_URL . 'dist/build/meta-controls.css', array( 'wp-components' ), KTP_VERSION );
		wp_register_script(
			'thewebs-element-classic-meta',
	...
	public function add_metabox() {
		add_meta_box(
			...
			__( 'Element Settings', 'thewebs-pro' ),
	...
	public function render_classic_metabox( $post ) {
		wp_localize_script(
			...
			array(
				...
				//'postTypes' => thewebs_blocks_pro_get_post_types(),
				'taxonomies'    => $this->get_taxonomies(),
				'templateHooks' => $this->get_template_hook_options(),
			)
		);
		$path = KTP_URL . 'build/';
		wp_enqueue_style( 'thewebs-element-meta', KTP_URL . 'dist/build/meta-controls.css', false, KTP_VERSION );
		wp_enqueue_script( 'thewebs-element-classic-meta' );
		// Add nonce for security and authentication.
		wp_nonce_field( 'thewebs_elements_classic_meta_nonce_action', 'thewebs_elements_classic_meta_nonce' );
		$output = '<div class="thewebs_classic_meta_boxes">';
		$output .= '<div class="thewebs_elements_classic_meta" data-post-id="' . esc_attr( $post->ID ) . '" style="padding: 10px 0 0;">';
		$output .= '</div>';
		$output .= '</div>';
	...
	public function enqueue_scripts() {
		wp_register_style( 'thewebs-pro-sticky', KTP_URL . 'dist/elements/thewebs-pro-sticky.css', array(), KTP_VERSION );
		wp_register_script( 'thewebs-pro-sticky', KTP_URL . 'dist/elements/thewebs-pro-sticky.min.js', array(), KTP_VERSION, true );
	}
	/**
	 * Make sure elements are not in yoast sitemap.
	 *
	...
	public function create_admin_page() {
		$page = add_theme_page(
			null,
			esc_html__( 'Elements', 'thewebs-pro' ),
			'edit_pages',
	...
	public static function use_ace_block() {
		if ( is_null( self::$aceblock ) ) {
			self::$aceblock = boolval( apply_filters( 'thewebs-pro-use-ace-block', true ) );
		}
	...
	public function register_ace_script_block() {
		...
		register_block_type(
			'thewebs-pro/ace-html',
			array(
				'editor_script' => 'thewebs-pro-script',
			)
		);
	}	
	/**
	...
	public function filter_rest_request_for_kses( $response, $handler, $request ) {
		...
		if ( isset( $request['id'] ) && ! empty( $request['id'] ) && isset( $request['content'] ) && ! empty( $request['content'] ) && strpos( $request['content'], '<!-- wp:thewebs-pro/ace-html -->') === 0 ) { 
		}
		...
		add_filter(
			...
			static function ( $post_content ) use ( &$post_value ) {
				...
				$post_value = str_replace( '<!-- wp:thewebs-pro/ace-html -->', '', $post_value );
				$post_value = str_replace( '<!-- /wp:thewebs-pro/ace-html -->', '', $post_value );
				//error_log( $post_value );
				$post_content = '<!-- wp:thewebs-pro/ace-html -->[element_code]<!-- /wp:thewebs-pro/ace-html -->';
				return $post_content;
			},
			0
		);
	...
	public function shortcode_output( $atts ) {
		...
		if ( ! apply_filters( 'thewebs_element_display', $this->check_element_conditionals( $element, $meta ), $element, $meta ) ) {
			return;
		}
	...
	public function init_mini_cart_hooks() {
		...
		foreach ( $posts as $post ) {
			...
			if ( isset( $meta['hook'] ) && ! empty( $meta['hook'] ) && ( 'woocommerce_before_mini_cart_contents' === $meta['hook'] || 'woocommerce_mini_cart_contents' === $meta['hook'] || 'woocommerce_widget_shopping_cart_before_buttons' === $meta['hook'] || 'woocommerce_widget_shopping_cart_after_buttons' === $meta['hook'] ) ) {
				if ( apply_filters( 'thewebs_element_display', $this->check_element_conditionals( $post, $meta ), $post, $meta ) ) {
					add_action(
						esc_attr( $meta['hook'] ),
	...
	public function init_frontend_hooks() {
		...
		foreach ( $posts as $post ) {
			$meta = $this->get_post_meta_array( $post );
			if ( apply_filters( 'thewebs_element_display', $this->check_element_conditionals( $post, $meta ), $post, $meta ) ) {
				if ( isset( $meta['hook'] ) && ! empty( $meta['hook'] ) && 'fixed' !== substr( $meta['hook'], 0, 5 ) && 'custom' !== $meta['hook'] && 'replace_header' !== $meta['hook'] && 'replace_404' !== $meta['hook'] && 'replace_footer' !== $meta['hook'] && 'thewebs_before_wrapper' !== $meta['hook'] && 'replace_login_modal' !== $meta['hook'] && 'replace_hero_header' !== $meta['hook'] && 'replace_single_content' !== $meta['hook'] && 'replace_archive_content' !== $meta['hook'] && 'replace_loop_content' !== $meta['hook'] && 'replace_meta' !== $meta['hook'] && 'woocommerce_before_single_product_image' !== $meta['hook'] && 'woocommerce_after_single_product_image' !== $meta['hook'] && 'thewebs_inside_the_content_before_h1' !== $meta['hook'] && 'thewebs_inside_the_content_after_h1' !== $meta['hook'] && 'thewebs_inside_the_content_after_p1' !== $meta['hook'] && 'thewebs_inside_the_content_after_p2' !== $meta['hook'] && 'thewebs_inside_the_content_after_p3' !== $meta['hook'] && 'thewebs_inside_the_content_after_p4' !== $meta['hook'] && 'thewebs_replace_sidebar' !== $meta['hook'] ) {
					add_action(
						esc_attr( $meta['hook'] ),
						function() use( $post, $meta ) {
							$this->output_element( $post, $meta );
					...
					$this->enqueue_element_styles( $post, $meta );
				} else if ( isset( $meta['hook'] ) && 'thewebs_before_wrapper' === $meta['hook'] ) {
					add_action(
						'thewebs_before_wrapper',
						function() use( $post, $meta ) {
							echo '<!-- [special-element-' . esc_attr( $post->ID ) . '] -->';
							echo '<div class="thewebs-before-wrapper-item">';
							$this->output_element( $post, $meta );
							echo '</div>';
							echo '<!-- [/special-element-' . esc_attr( $post->ID ) . '] -->';
						},
					...
					remove_action( 'thewebs_hero_header', 'Thewebs\hero_title' );
					add_action(
						'thewebs_hero_header',
						function() use( $post, $meta ) {
							$this->output_element( $post, $meta );
					...
					$this->enqueue_element_styles( $post, $meta );
				} else if ( isset( $meta['hook'] ) && 'thewebs_replace_sidebar' === $meta['hook'] ) {
					add_filter(
						'thewebs_dynamic_sidebar_content',
						function() use( $post, $meta ) {
							$this->output_element( $post, $meta );
						},
						absint( $meta['priority'] )
					...
					if ( is_404() ) {
						remove_action( 'thewebs_404_content', 'Thewebs\get_404_content' );
						add_filter(
							'thewebs_post_layout',
							function( $layout ) {
					...
					remove_action( 'thewebs_single_content', 'Thewebs\single_content' );
					add_action(
						'thewebs_single_content',
						function() use( $post, $meta ) {
							$this->output_element( $post, $meta );
						},
						absint( $meta['priority'] )
					...
					remove_action( 'thewebs_loop_entry', 'Thewebs\loop_entry' );
					add_action(
						'thewebs_loop_entry',
						function() use ( $post, $meta ) {
					...
					add_action(
						...
						remove_action( 'thewebs_account_login_form', 'Thewebs_Pro\account_login_form' );
						add_action(
							'thewebs_account_login_form',
							function() use( $post, $meta ) {
								$this->output_element( $post, $meta );
							},
							absint( $meta['priority'] )
						...
						$this->enqueue_element_styles( $post, $meta );
				} elseif ( isset( $meta['hook'] ) && strlen( $meta['hook'] ) > 26 && 'thewebs_inside_the_content' === substr( $meta['hook'], 0, 26 ) ) {
					switch ( $meta['hook'] ) {
						case 'thewebs_inside_the_content_before_h1':
							add_filter(
								'the_content',
						...
						break;
						case 'thewebs_inside_the_content_after_h1':
							add_filter(
								'the_content',
								function( $content ) use( $post, $meta ) {
									if ( ! self::apply_in_content_filter( $post ) ) {
										ob_start();
											...
											echo '<div class="thewebs-pro-in-content-element">';
												$this->output_element( $post, $meta );
											echo '</div>';
											echo '<!-- [/in-content-element-' . esc_attr( $post->ID ) . '] -->';
											$insertion = ob_get_clean();
						...
						break;
						case 'thewebs_inside_the_content_after_p1':
							add_filter(
								'the_content',
								function( $content ) use( $post, $meta ) {
									if ( ! self::apply_in_content_filter( $post ) ) {
						...
						break;
						case 'thewebs_inside_the_content_after_p2':
							add_filter(
								'the_content',
								function( $content ) use( $post, $meta ) {
									if ( ! self::apply_in_content_filter( $post ) ) {
						...
						break;
						case 'thewebs_inside_the_content_after_p3':
							add_filter(
								'the_content',
								function( $content ) use( $post, $meta ) {
									if ( ! self::apply_in_content_filter( $post ) ) {
						...
						break;
						case 'thewebs_inside_the_content_after_p4':
							add_filter(
								'the_content',
								function( $content ) use( $post, $meta ) {
									if ( ! self::apply_in_content_filter( $post ) ) {
					...
					switch ( $meta['hook'] ) {
						case 'fixed_above_trans_header':
							add_action(
								'thewebs_before_wrapper',
								function() use( $post, $meta ) {
									wp_enqueue_style( 'thewebs-pro-sticky' );
									wp_enqueue_script( 'thewebs-pro-sticky' );
									echo '<!-- [fixed-element-' . esc_attr( $post->ID ) . '] -->';
									echo '<div class="thewebs-pro-fixed-wrap">';
										echo '<div class="thewebs-pro-fixed-item thewebs-pro-fixed-header-item thewebs-pro-fixed-above-trans">';
										$this->output_element( $post, $meta );
										echo '</div>';
									echo '</div>';
									echo '<!-- [/fixed-element-' . esc_attr( $post->ID ) . '] -->';
						...
						case 'fixed_above_header':
							add_action(
								'thewebs_before_header',
								function() use( $post, $meta ) {
									wp_enqueue_style( 'thewebs-pro-sticky' );
									wp_enqueue_script( 'thewebs-pro-sticky' );
									echo '<!-- [fixed-element-' . esc_attr( $post->ID ) . '] -->';
									echo '<div class="thewebs-pro-fixed-wrap">';
										echo '<div class="thewebs-pro-fixed-item thewebs-pro-fixed-header-item thewebs-pro-fixed-above">';
										$this->output_element( $post, $meta );
										echo '</div>';
									echo '</div>';
									echo '<!-- [/fixed-element-' . esc_attr( $post->ID ) . '] -->';
						...
						case 'fixed_on_header':
							add_action(
								...
								function() use( $post, $meta ) {
									...
									$added_classes = '';
									if ( '' != $fixed_width ) {
										$added_classes = 'thewebs-fixed-width-' . $fixed_width . ' thewebs-fixed-xposition-' . $position;
									}
									$offset = ( isset( $meta['scroll'] ) && '' !== $meta['scroll'] ? absint( $meta['scroll'] ) : 300 );
									echo '<!-- [fixed-element-' . esc_attr( $post->ID ) . '] -->';
									echo '<div class="thewebs-pro-fixed-item thewebs-pro-fixed-header-item thewebs-pro-fixed-on-scroll item-at-start' . esc_attr( $added_classes ? ' ' . $added_classes : '' ) . '" style="' . ( '' != $fixed_width ? $position . ':' . $xposition . 'px;' : '' ) . ( '' != $fixed_width ? 'transform:translateY(' . $yposition . 'px);' : '' ) . ( 'fixed' === $fixed_width ? 'max-width:' . $width . 'px;' : '' ) . '" data-scroll-offset="'. esc_attr( $offset ) . '">';
									$this->output_element( $post, $meta );
									echo '</div>';
									echo '<!-- [/fixed-element-' . esc_attr( $post->ID ) . '] -->';
								},
						...
						case 'fixed_below_footer':
							add_action(
								'thewebs_after_footer',
								function() use( $post, $meta ) {
									wp_enqueue_style( 'thewebs-pro-sticky' );
									wp_enqueue_script( 'thewebs-pro-sticky' );
									echo '<!-- [fixed-element-' . esc_attr( $post->ID ) . '] -->';
									echo '<div class="thewebs-pro-fixed-wrap">';
										echo '<div class="thewebs-pro-fixed-item thewebs-pro-fixed-footer-item thewebs-pro-fixed-below">';
										$this->output_element( $post, $meta );
										echo '</div>';
									echo '</div>';
									echo '<!-- [/fixed-element-' . esc_attr( $post->ID ) . '] -->';
						...
						case 'fixed_on_footer':
							add_action(
								...
								function() use( $post, $meta ) {
									...
									echo '<!-- [fixed-element-' . esc_attr( $post->ID ) . '] -->';
									echo '<div class="thewebs-pro-fixed-item thewebs-pro-fixed-footer-item thewebs-pro-fixed-bottom' . esc_attr( $added_classes ? ' ' . $added_classes : '' ) . '" style="' . ( '' != $fixed_width ? $position . ':' . $xposition . 'px;' : '' ) . ( '' != $fixed_width ? 'bottom:' . $yposition . 'px;' : '' ) . ( 'fixed' === $fixed_width ? 'max-width:' . $width . 'px;' : '' ) . '">';
									$this->output_element( $post, $meta );
									echo '</div>';
									echo '<!-- [/fixed-element-' . esc_attr( $post->ID ) . '] -->';
								},
						...
						case 'fixed_on_footer_scroll':
							add_action(
								...
								function() use( $post, $meta ) {
									...
									$offset = ( isset( $meta['scroll'] ) && '' !== $meta['scroll'] ? absint( $meta['scroll'] ) : 300 );
									echo '<div class="thewebs-pro-fixed-item thewebs-pro-fixed-footer-item thewebs-pro-fixed-on-scroll-footer thewebs-pro-fixed-bottom-scroll item-at-start' . esc_attr( $added_classes ? ' ' . $added_classes : '' ) . '" style="' . ( '' != $fixed_width ? $position . ':' . $xposition . 'px;' : '' ) . ( '' != $fixed_width ? 'bottom:' . $yposition . 'px;' : '' ) . ( 'fixed' === $fixed_width ? 'max-width:' . $width . 'px;' : '' ) . '" data-scroll-offset="'. esc_attr( $offset ) . '">';
									$this->output_element( $post, $meta );
									echo '</div>';
									echo '<!-- [/fixed-element-' . esc_attr( $post->ID ) . '] -->';
								},
						...
						case 'fixed_on_footer_scroll_space':
							add_action(
								...
								function() use( $post, $meta ) {
									...
									echo '<div class="thewebs-pro-fixed-wrap">';
									$offset = ( isset( $meta['scroll'] ) && '' !== $meta['scroll'] ? absint( $meta['scroll'] ) : 300 );
									echo '<div class="thewebs-pro-fixed-item thewebs-pro-fixed-footer-item thewebs-pro-fixed-on-scroll-footer-space thewebs-pro-fixed-bottom-scroll item-at-start" data-scroll-offset="'. esc_attr( $offset ) . '">';
									$this->output_element( $post, $meta );
									echo '</div>';
									echo '</div>';
									echo '<!-- [/fixed-element-' . esc_attr( $post->ID ) . '] -->';
	...
	public function insert_inside_content( $content, $insertion = null, $element = '</h2>', $placement_id = 1, $after_element = true, $min_elements = 1 ) {
		...
		if ( ! extension_loaded( 'dom' ) || apply_filters( 'thewebs_pro_inject_content_simple_method', false ) ) {
			...
			switch ( $tag_option ) {
				...
				case 'h5':
				case 'h6':
					$headlines = apply_filters( 'thewebs-headlines-for-element-in-content', array( 'h1', 'h2', 'h3', 'h4' ) );
	...
	public static function apply_in_content_filter( $post ) {
		...
		return apply_filters( 'thewebs_pro_run_in_the_content_filter', $run );
	}
	/**
	 * Outputs the custom before wrap element.
	 */
	public function product_image_before_wrap() {
		echo '<div class="thewebs-product-image-wrap images">';
	}
	/**
	 * Outputs the custom before wrap element.
	 */
	...
	public function output_element_template( $post, $meta ) {
		...
		// 	return $context;
		// };
		//add_filter( 'thewebs_blocks_render_head_css', $filter_block_context, 10, 3 );
	...
	public function output_element( $post, $meta, $shortcode = false ) {
		...
		if ( isset( $meta['device'] ) && ! empty( $meta['device'] ) && is_array( $meta['device'] ) ) {
			$element_device_classes = array( 'thewebs-element-wrap' );
			$devices = array();
	...
	public function enqueue_element_styles( $post, $meta, $shortcode = false ) {
		...
		if ( ! empty( $css_output ) || ! empty( $js_output ) ) {
			add_action(
				...
				function() use( $post, $css_output, $js_output ) {
					if ( ! empty( $css_output ) ) {
						echo '<style id="thewebs-blocks-post-custom-css-' . $post->ID . '">';
						echo $css_output;
						echo '</style>';
					}
					if ( ! empty( $js_output ) ) {
		...
		if ( has_blocks( $content ) ) {
			if ( class_exists( 'Thewebs_Blocks_Frontend' ) ) {
				$thewebs_blocks = \Thewebs_Blocks_Frontend::get_instance();
				if ( method_exists( $thewebs_blocks, 'frontend_build_css' ) ) {
					$thewebs_blocks->frontend_build_css( $post );
				}
				if ( class_exists( 'Thewebs_Blocks_Pro_Frontend' ) ) {
					$thewebs_blocks_pro = \Thewebs_Blocks_Pro_Frontend::get_instance();
					if ( method_exists( $thewebs_blocks_pro, 'frontend_build_css' ) ) {
						$thewebs_blocks_pro->frontend_build_css( $post );
					}
				}
			}
			return;
	...
	public function script_enqueue() {
		...
		$path = KTP_URL . 'build/';
		wp_enqueue_style( 'thewebs-element-meta', KTP_URL . 'dist/build/meta-controls.css', false, KTP_VERSION );
		wp_enqueue_script( 'thewebs-element-meta' );
		if ( get_post_meta( get_the_ID(), '_kad_element_preview_post', true ) ) {
			...
			$the_post_id = array(
				...
				'name'=> __( 'Latest Post', 'thewebs-pro' ),
			);
			$the_post_id = wp_json_encode( $the_post_id );
			$the_post_type  = 'post';
		}
		wp_localize_script(
			'thewebs-element-meta',
			'thewebsElementParams',
			array(
				'post_type'  => $post_type,
				'hooks'      => $this->get_hook_options(),
				...
				'previewPostID'      => apply_filters( 'thewebs_elements_dynamic_content_preview_post', $the_post_id ),
				'previewPostType'    => apply_filters( 'thewebs_elements_dynamic_content_preview_post_type', $the_post_type ),
			)
		);
		if ( function_exists( 'wp_set_script_translations' ) ) {
			wp_set_script_translations( 'thewebs-element-meta', 'thewebs-pro' );
		}
	}
	/**
	...
	public function get_post_types() {
		...
		return apply_filters( 'thewebs_pro_post_types', $output );
	}
	/**
	...
	public function get_language_options() {
		...
		return apply_filters( 'thewebs_pro_element_display_languages', $languages_options );
	}
	/**
	...
	public function get_user_options() {
		$user_basic = array(
			array(
				'label' => esc_attr__( 'Basic', 'thewebs-pro' ),
				'options' => array(
					array(
						'value' => 'public',
						'label' => esc_attr__( 'All Users', 'thewebs-pro' ),
					),
					array(
						'value' => 'logged_out',
						'label' => esc_attr__( 'Logged out Users', 'thewebs-pro' ),
					),
					array(
						'value' => 'logged_in',
						'label' => esc_attr__( 'Logged in Users', 'thewebs-pro' ),
					),
				),
			),
		);
		...
		$user_roles[] = array(
			'label' => esc_attr__( 'Specific Role', 'thewebs-pro' ),
			'options' => $specific_roles,
		);
		$roles = array_merge( $user_basic, $user_roles );
		return apply_filters( 'thewebs_pro_element_user_options', $roles );
	}
	...
	public function get_display_options() {
		$display_general = array(
			array(
				'label' => esc_attr__( 'General', 'thewebs-pro' ),
				'options' => array(
					array(
						'value' => 'general|site',
						'label' => esc_attr__( 'Entire Site', 'thewebs-pro' ),
					),
					array(
						'value' => 'general|front_page',
						'label' => esc_attr__( 'Front Page', 'thewebs-pro' ),
					),
					array(
						'value' => 'general|home',
						'label' => esc_attr__( 'Blog Page', 'thewebs-pro' ),
					),
					array(
						'value' => 'general|search',
						'label' => esc_attr__( 'Search Results', 'thewebs-pro' ),
					),
					array(
						'value' => 'general|404',
						'label' => esc_attr__( 'Not Found (404)', 'thewebs-pro' ),
					),
					array(
						'value' => 'general|singular',
						'label' => esc_attr__( 'All Singular', 'thewebs-pro' ),
					),
					array(
						'value' => 'general|archive',
						'label' => esc_attr__( 'All Archives', 'thewebs-pro' ),
					),
					array(
						'value' => 'general|author',
						'label' => esc_attr__( 'Author Archives', 'thewebs-pro' ),
					),
					array(
						'value' => 'general|date',
						'label' => esc_attr__( 'Date Archives', 'thewebs-pro' ),
					),
					array(
						'value' => 'general|paged',
						'label' => esc_attr__( 'Paged', 'thewebs-pro' ),
					),
				),
			),
		);
		$thewebs_public_post_types = thewebs()->get_post_types();
		$ignore_types              = thewebs()->get_public_post_types_to_ignore();
		$display_singular = array();
		foreach ( $thewebs_public_post_types as $post_type ) {
			...
			if ( ! in_array( $post_type_name, $ignore_types, true ) ) {
				$post_type_options = array(
					array(
						...
						'label' => esc_attr__( 'Single', 'thewebs-pro' ) . ' ' . $post_type_label_plural,
					),
				);
				...
				foreach ( $post_type_tax_objects as $taxonomy_slug => $taxonomy ) {
					if ( $taxonomy->public && $taxonomy->show_ui && 'post_format' !== $taxonomy_slug ) {
						$post_type_options[] = array(
							...
							'label' => sprintf( esc_attr__( '%1$s Archives', 'thewebs-pro' ), $taxonomy->labels->singular_name ),
						);
					}
				}
				if ( ! empty( $post_type_item->has_archive ) ) {
					$post_type_options[] = array(
						...
						'label' => sprintf( esc_attr__( '%1$s Archive', 'thewebs-pro' ), $post_type_label_plural ),
					);
				}
				if ( class_exists( 'woocommerce' ) && 'product' === $post_type_name ) {
					$post_type_options[] = array(
						...
						/* translators: %1$s: post type plural label  */
						'label' => sprintf( esc_attr__( '%1$s Search', 'thewebs-pro' ), $post_type_label_plural ),
					);
				}
				$display_singular[] = array(
		...
		return apply_filters( 'thewebs_pro_element_display_options', $display );
	}
	/**
	...
	public function get_fixed_hook_options() {
		$hooks        = array(
			array(
				'label' => esc_attr__( 'Fixed', 'thewebs-pro' ),
				'options' => array(
					array(
						'value' => 'fixed_above_header',
						'label' => __( 'Fixed On Top', 'thewebs-pro' ),
					),
					array(
						'value' => 'fixed_above_trans_header',
						'label' => __( 'Fixed Above Transparent Header', 'thewebs-pro' ),
					),
					array(
						'value' => 'fixed_on_header',
						'label' => __( 'Fixed Top After Scroll', 'thewebs-pro' ),
					),
					array(
						'value' => 'fixed_on_footer_scroll',
						'label' => __( 'Fixed Bottom After Scroll (no space below footer)', 'thewebs-pro' ),
					),
					array(
						'value' => 'fixed_on_footer_scroll_space',
						'label' => __( 'Fixed Bottom After Scroll', 'thewebs-pro' ),
					),
					array(
						'value' => 'fixed_below_footer',
						'label' => __( 'Fixed On Bottom', 'thewebs-pro' ),
					),
					array(
						'value' => 'fixed_on_footer',
						'label' => __( 'Fixed Bottom (no space below footer)', 'thewebs-pro' ),
					),
				),
			),
		);
	...
	public function get_author_options() {
		...
		return apply_filters( 'thewebs_pro_element_display_authors', $output );
	}
	/**
	...
	public function get_code_hook_options() {
		$hooks        = array(
			array(
				'label' => esc_attr__( 'Scripts', 'thewebs-pro' ),
				'options' => array(
					array(
						'value' => 'wp_head',
						'label' => __( 'Header - Inside <head> tag', 'thewebs-pro' ),
					),
					array(
						'value' => 'wp_body_open',
						'label' => __( 'After <body> tag open', 'thewebs-pro' ),
					),
					array(
						'value' => 'wp_footer',
						'label' => __( 'Footer - Before </body> tag close', 'thewebs-pro' ),
					),
				),
			),
		);
	...
	public function get_template_hook_options() {
		$hooks        = array(
			array(
				'label' => esc_attr__( 'Templates', 'thewebs-pro' ),
				'options' => array(
					array(
						'value' => 'replace_header',
						'label' => esc_attr__( 'Replace Header', 'thewebs-pro' ),
					),
					array(
						'value' => 'replace_hero_header',
						'label' => esc_attr__( 'Replace Above Content Hero', 'thewebs-pro' ),
					),
					array(
						'value' => 'replace_single_content',
						'label' => __( 'Replace Single Post Content', 'thewebs-pro' ),
					),
					// array(
					// 	'value' => 'replace_archive_content',
					// 	'label' => __( 'Replace Archive Content', 'thewebs-pro' ),
					// ),
					array(
						'value' => 'replace_loop_content',
						'label' => __( 'Replace Archive Loop Item Content', 'thewebs-pro' ),
					),
					// array(
					// 	'value' => 'replace_meta',
					// 	'label' => __( 'Replace Post Meta', 'thewebs-pro' ),
					// ),
					array(
						'value' => 'thewebs_replace_sidebar',
						'label' => esc_attr__( 'Replace Sidebar', 'thewebs-pro' ),
					),
					array(
						'value' => 'replace_footer',
						'label' => esc_attr__( 'Replace Footer', 'thewebs-pro' ),
					),
					array(
						'value' => 'replace_404',
						'label' => esc_attr__( 'Replace 404 Page Content', 'thewebs-pro' ),
					),
				),
			),
		);

		return apply_filters( 'thewebs_pro_element_template_hooks_options', $hooks );
	}
	/**
	 * Get all taxonomies
	 */
	public function get_taxonomies() {
			...
			if ( ! in_array( $post_type_name, $ignore_types, true ) ) {
				...
				if ( 'sfwd-lessons' === $post_type ) {
					$taxs['assigned_course'] = array(
						...
						'label' => __( 'Assigned Course', 'thewebs-pro' ),
					);
					$args = array(
		...
		return apply_filters( 'thewebs_pro_element_display_taxonomies', $output );
	}
	/**
	...
	public function get_hook_options() {
		$hooks = array(
			array(
				'label' => esc_attr__( 'Body', 'thewebs-pro' ),
				'options' => array(
					array(
						'value' => 'thewebs_before_wrapper',
						'label' => esc_attr__( 'Before Site Wrapper', 'thewebs-pro' ),
					),
					array(
						'value' => 'thewebs_after_wrapper',
						'label' => esc_attr__( 'After Site Wrapper', 'thewebs-pro' ),
					),
				),
			),
			array(
				'label'   => esc_attr__( 'Header', 'thewebs-pro' ),
				'options' => array(
					array(
						'value' => 'thewebs_before_header',
						'label' => esc_attr__( 'Before Header', 'thewebs-pro' ),
					),
					array(
						'value' => 'thewebs_after_header',
						'label' => esc_attr__( 'After Header', 'thewebs-pro' ),
					),
					array(
						'value' => 'replace_header',
						'label' => esc_attr__( 'Replace Header', 'thewebs-pro' ),
			...
			array(
				'label'   => esc_attr__( 'Content Wrap', 'thewebs-pro' ),
				'options' => array(
					array(
						'value' => 'thewebs_before_content',
						'label' => esc_attr__( 'Before All Content', 'thewebs-pro' ),
					),
					array(
						'value' => 'thewebs_after_content',
						'label' => esc_attr__( 'After All Content', 'thewebs-pro' ),
					),
				),
			),
			array(
				'label'   => esc_attr__( 'Above Content Hero Title', 'thewebs-pro' ),
				'options' => array(
					array(
						'value' => 'thewebs_entry_hero',
						'label' => esc_attr__( 'SINGLE: Above Title Content (Use Priority for Before/After Placement)', 'thewebs-pro' ),
					),
					array(
						'value' => 'thewebs_entry_archive_hero',
						'label' => esc_attr__( 'ARCHIVE: Above Title Content (Use Priority for Before/After Placement)', 'thewebs-pro' ),
					),
					array(
						'value' => 'replace_hero_header',
						'label' => esc_attr__( 'Replace Above Content Hero', 'thewebs-pro' ),
			...
			array(
				'label'   => esc_attr__( 'Content and Sidebar', 'thewebs-pro' ),
				'options' => array(
					array(
						'value' => 'thewebs_before_main_content',
						'label' => esc_attr__( 'Before Content', 'thewebs-pro' ),
					),
					array(
						'value' => 'thewebs_after_main_content',
						'label' => esc_attr__( 'After Content', 'thewebs-pro' ),
					),
					array(
						'value' => 'thewebs_before_sidebar',
						'label' => esc_attr__( 'Before Sidebar', 'thewebs-pro' ),
					),
					array(
						'value' => 'thewebs_replace_sidebar',
						'label' => esc_attr__( 'Replace Sidebar', 'thewebs-pro' ),
					...
					array(
						'value' => 'thewebs_after_sidebar',
						'label' => esc_attr__( 'After Sidebar', 'thewebs-pro' ),
					),
				),
			),
			array(
				'label'   => esc_attr__( 'Single Inner Content', 'thewebs-pro' ),
				'options' => array(
					array(
						'value' => 'thewebs_single_before_inner_content',
						'label' => esc_attr__( 'Before Inner Content', 'thewebs-pro' ),
					),
					array(
						'value' => 'thewebs_single_before_entry_header',
						'label' => esc_attr__( 'Before Inner Title Area', 'thewebs-pro' ),
					),
					array(
						'value' => 'thewebs_single_before_entry_title',
						'label' => esc_attr__( 'Before Inner Title', 'thewebs-pro' ),
					),
					array(
						'value' => 'thewebs_single_after_entry_title',
						'label' => esc_attr__( 'After Inner Title', 'thewebs-pro' ),
					),
					array(
						'value' => 'thewebs_single_after_entry_header',
						'label' => esc_attr__( 'After Inner Title Area', 'thewebs-pro' ),
					),
					array(
						'value' => 'thewebs_single_before_entry_content',
						'label' => esc_attr__( 'Before Entry Content', 'thewebs-pro' ),
					),
					array(
						'value' => 'thewebs_single_after_entry_content',
						'label' => esc_attr__( 'After Entry Content', 'thewebs-pro' ),
					),
					array(
						'value' => 'thewebs_single_after_inner_content',
						'label' => esc_attr__( 'After Inner Content', 'thewebs-pro' ),
					),
				),
			),
			array(
				'label'   => esc_attr__( 'Inside the Content', 'thewebs-pro' ),
				'options' => array(
					array(
						'value' => 'thewebs_inside_the_content_before_h1',
						'label' => esc_attr__( 'Before First Heading Tag', 'thewebs-pro' ),
					),
					array(
						'value' => 'thewebs_inside_the_content_after_h1',
						'label' => esc_attr__( 'After First Heading Tag', 'thewebs-pro' ),
					),
					array(
						'value' => 'thewebs_inside_the_content_after_p1',
						'label' => esc_attr__( 'After First Paragraph', 'thewebs-pro' ),
					),
					array(
						'value' => 'thewebs_inside_the_content_after_p2',
						'label' => esc_attr__( 'After Second Paragraph', 'thewebs-pro' ),
					),
					array(
						'value' => 'thewebs_inside_the_content_after_p3',
						'label' => esc_attr__( 'After Third Paragraph', 'thewebs-pro' ),
					),
					array(
						'value' => 'thewebs_inside_the_content_after_p4',
						'label' => esc_attr__( 'After Fourth Paragraph', 'thewebs-pro' ),
					),
				),
			),
			array(
				'label'   => esc_attr__( 'Comments', 'thewebs-pro' ),
				'options' => array(
					array(
						'value' => 'thewebs_before_comments',
						'label' => esc_attr__( 'Before Comments', 'thewebs-pro' ),
					),
					array(
						'value' => 'thewebs_before_comments_list',
						'label' => esc_attr__( 'Before Comments List', 'thewebs-pro' ),
					),
					array(
						'value' => 'thewebs_after_comments_list',
						'label' => esc_attr__( 'After Comments List', 'thewebs-pro' ),
					),
					array(
						'value' => 'thewebs_after_comments',
						'label' => esc_attr__( 'After Comments', 'thewebs-pro' ),
					),
				),
			),
			array(
				'label'   => esc_attr__( 'Archive Inner Content', 'thewebs-pro' ),
				'options' => array(
					array(
						'value' => 'thewebs_archive_before_entry_header',
						'label' => esc_attr__( 'Before Archive Inner Title', 'thewebs-pro' ),
					),
					array(
						'value' => 'thewebs_archive_after_entry_header',
						'label' => esc_attr__( 'After Archive Inner Title', 'thewebs-pro' ),
					),
				),
			),
			array(
				'label' => esc_attr__( 'Footer', 'thewebs-pro' ),
				'options' => array(
					array(
						'value' => 'thewebs_before_footer',
						'label' => esc_attr__( 'Before Footer', 'thewebs-pro' ),
					),
					array(
						'value' => 'thewebs_after_footer',
						'label' => esc_attr__( 'After Footer', 'thewebs-pro' ),
					),
					array(
						'value' => 'replace_footer',
						'label' => esc_attr__( 'Replace Footer', 'thewebs-pro' ),
			...
			array(
				'label' => esc_attr__( '404 Page', 'thewebs-pro' ),
				'options' => array(
					array(
						'value' => 'thewebs_404_before_inner_content',
						'label' => esc_attr__( 'Before 404 Page Content', 'thewebs-pro' ),
					),
					array(
						'value' => 'thewebs_404_after_inner_content',
						'label' => esc_attr__( 'After 404 Page Content', 'thewebs-pro' ),
					),
					array(
						'value' => 'replace_404',
						'label' => esc_attr__( 'Replace 404 Page Content', 'thewebs-pro' ),
			...
			array(
				'label' => esc_attr__( 'Mobile Menu Off Canvas Area', 'thewebs-pro' ),
				'options' => array(
					array(
						'value' => 'thewebs_before_mobile_navigation_popup',
						'label' => esc_attr__( 'Before Mobile Off Canvas Content', 'thewebs-pro' ),
					),
					array(
						'value' => 'thewebs_after_mobile_navigation_popup',
						'label' => esc_attr__( 'After Mobile Off Canvas Content', 'thewebs-pro' ),
					),
				),
			),
			array(
				'label' => esc_attr__( 'Header Account Login Modal', 'thewebs-pro' ),
				'options' => array(
					array(
						'value' => 'thewebs_before_account_login_popup',
						'label' => esc_attr__( 'Left of Login Form', 'thewebs-pro' ),
					),
					array(
						'value' => 'thewebs_before_account_login_inner_popup',
						'label' => esc_attr__( 'Before Login Form', 'thewebs-pro' ),
					),
					array(
						'value' => 'thewebs_after_account_login_inner_popup',
						'label' => esc_attr__( 'After Login Form', 'thewebs-pro' ),
					),
					array(
						'value' => 'thewebs_after_account_login_popup',
						'label' => esc_attr__( 'Right of Login Form', 'thewebs-pro' ),
					),
					array(
						'value' => 'replace_login_modal',
						'label' => esc_attr__( 'Replace Login Modal', 'thewebs-pro' ),
					),
				),
			),
		);
		if ( class_exists( 'woocommerce' ) ) {
			$woo_add = array(
				array(
					'label' => esc_attr__( 'Woocommerce Content', 'thewebs-pro' ),
					'options' => array(
						array(
							'value' => 'woocommerce_before_main_content',
							'label' => esc_attr__( 'Before Woocommerce Content', 'thewebs-pro' ),
						),
						array(
							'value' => 'woocommerce_after_main_content',
							'label' => esc_attr__( 'After Woocommerce Content', 'thewebs-pro' ),
						),
					),
				),
				array(
					'label' => esc_attr__( 'Woocommerce Product', 'thewebs-pro' ),
					'options' => array(
						array(
							'value' => 'woocommerce_before_single_product',
							'label' => esc_attr__( 'Before Single Product', 'thewebs-pro' ),
						),
						array(
							'value' => 'woocommerce_before_single_product_summary',
							'label' => esc_attr__( 'Single Product Image', 'thewebs-pro' ),
						),
						array(
							'value' => 'woocommerce_before_single_product_image',
							'label' => esc_attr__( 'Before Single Product Image', 'thewebs-pro' ),
						),
						array(
							'value' => 'woocommerce_after_single_product_image',
							'label' => esc_attr__( 'After Single Product Image', 'thewebs-pro' ),
						),
						array(
							'value' => 'woocommerce_single_product_summary',
							'label' => esc_attr__( 'Single Product Summary', 'thewebs-pro' ),
						),
						array(
							'value' => 'woocommerce_before_add_to_cart_form',
							'label' => esc_attr__( 'Before Add to Cart Form', 'thewebs-pro' ),
						),
						array(
							'value' => 'woocommerce_after_add_to_cart_form',
							'label' => esc_attr__( 'After Add to Cart Form', 'thewebs-pro' ),
						),
						array(
							'value' => 'woocommerce_share',
							'label' => esc_attr__( 'Single Product Share', 'thewebs-pro' ),
						),
						array(
							'value' => 'woocommerce_after_single_product_summary',
							'label' => esc_attr__( 'Single Product Tabs', 'thewebs-pro' ),
						),
						array(
							'value' => 'woocommerce_after_single_product',
							'label' => esc_attr__( 'After Single Product', 'thewebs-pro' ),
						),
					),
				),
				array(
					'label' => esc_attr__( 'Woocommerce Archive', 'thewebs-pro' ),
					'options' => array(
						array(
							'value' => 'woocommerce_before_shop_loop',
							'label' => esc_attr__( 'Before Shop Loop', 'thewebs-pro' ),
						),
						array(
							'value' => 'woocommerce_before_shop_loop_item',
							'label' => esc_attr__( 'Before Loop Item', 'thewebs-pro' ),
						),
						array(
							'value' => 'woocommerce_before_shop_loop_item_title',
							'label' => esc_attr__( 'Loop Item Image', 'thewebs-pro' ),
						),
						array(
							'value' => 'woocommerce_shop_loop_item_title',
							'label' => esc_attr__( 'Loop Item Title', 'thewebs-pro' ),
						),
						array(
							'value' => 'woocommerce_after_shop_loop_item_title',
							'label' => esc_attr__( 'Loop Item Price', 'thewebs-pro' ),
						),
						array(
							'value' => 'woocommerce_after_shop_loop_item',
							'label' => esc_attr__( 'After Shop Loop Item', 'thewebs-pro' ),
						),
						array(
							'value' => 'woocommerce_after_shop_loop',
							'label' => esc_attr__( 'After Shop Loop', 'thewebs-pro' ),
						),
					),
				),
				array(
					'label' => esc_attr__( 'Woocommerce Account', 'thewebs-pro' ),
					'options' => array(
						array(
							'value' => 'woocommerce_before_customer_login_form',
							'label' => esc_attr__( 'Before Login Form (Logged Out View)', 'thewebs-pro' ),
						),
						array(
							'value' => 'woocommerce_after_customer_login_form',
							'label' => esc_attr__( 'After Login Form (Logged Out View)', 'thewebs-pro' ),
						),
						array(
							'value' => 'woocommerce_before_account_navigation',
							'label' => esc_attr__( 'Before Account Navigation', 'thewebs-pro' ),
						),
						array(
							'value' => 'woocommerce_after_account_navigation',
							'label' => esc_attr__( 'After Account Navigation', 'thewebs-pro' ),
						),
						array(
							'value' => 'woocommerce_account_content',
							'label' => esc_attr__( 'Account Content (Logged In View)', 'thewebs-pro' ),
						),
						array(
							'value' => 'woocommerce_account_dashboard',
							'label' => esc_attr__( 'Account Dashboard', 'thewebs-pro' ),
						),
					),
				),
				array(
					'label' => esc_attr__( 'Woocommerce Cart', 'thewebs-pro' ),
					'options' => array(
						array(
							'value' => 'woocommerce_before_cart',
							'label' => esc_attr__( 'Before Cart Content', 'thewebs-pro' ),
						),
						array(
							'value' => 'woocommerce_before_cart_table',
							'label' => esc_attr__( 'Before Cart Table', 'thewebs-pro' ),
						),
						array(
							'value' => 'woocommerce_after_cart_table',
							'label' => esc_attr__( 'After Cart Table', 'thewebs-pro' ),
						),
						array(
							'value' => 'woocommerce_cart_collaterals',
							'label' => esc_attr__( 'Cart Totals', 'thewebs-pro' ),
						),
						array(
							'value' => 'woocommerce_after_cart',
							'label' => esc_attr__( 'After Cart Content', 'thewebs-pro' ),
						),
					),
				),
				array(
					'label' => esc_attr__( 'Woocommerce Side Cart', 'thewebs-pro' ),
					'options' => array(
						array(
							'value' => 'thewebs-before-side-cart',
							'label' => esc_attr__( 'Before Side Cart Content', 'thewebs-pro' ),
						),
						array(
							'value' => 'woocommerce_before_mini_cart_contents',
							'label' => esc_attr__( 'Before Side Cart List Items', 'thewebs-pro' ),
						),
						array(
							'value' => 'woocommerce_mini_cart_contents',
							'label' => esc_attr__( 'After Side Cart List Items', 'thewebs-pro' ),
						),
						array(
							'value' => 'woocommerce_widget_shopping_cart_before_buttons',
							'label' => esc_attr__( 'Before Side Cart Buttons', 'thewebs-pro' ),
						),
						array(
							'value' => 'woocommerce_widget_shopping_cart_after_buttons',
							'label' => esc_attr__( 'After Side Cart Buttons', 'thewebs-pro' ),
						),
						array(
							'value' => 'thewebs-after-side-cart',
							'label' => esc_attr__( 'After Side Cart Content', 'thewebs-pro' ),
						),
					),
				),
				array(
					'label' => esc_attr__( 'Woocommerce Checkout', 'thewebs-pro' ),
					'options' => array(
						array(
							'value' => 'woocommerce_before_checkout_form',
							'label' => esc_attr__( 'Before Checkout Form', 'thewebs-pro' ),
						),
						array(
							'value' => 'woocommerce_checkout_before_customer_details',
							'label' => esc_attr__( 'Before Customer Details', 'thewebs-pro' ),
						),
						array(
							'value' => 'woocommerce_checkout_after_customer_details',
							'label' => esc_attr__( 'After Customer Details', 'thewebs-pro' ),
						),
						array(
							'value' => 'woocommerce_checkout_order_review',
							'label' => esc_attr__( 'Order Review', 'thewebs-pro' ),
						),
						array(
							'value' => 'woocommerce_review_order_after_cart_contents',
							'label' => esc_attr__( 'After Order Review Contents', 'thewebs-pro' ),
						),
						array(
							'value' => 'woocommerce_review_order_before_order_total',
							'label' => esc_attr__( 'Before Order Review Total', 'thewebs-pro' ),
						),
						array(
							'value' => 'woocommerce_review_order_after_order_total',
							'label' => esc_attr__( 'After Order Review Total', 'thewebs-pro' ),
						),
						array(
				...
				array(
					'label' => esc_attr__( 'Woocommerce Order Received', 'thewebs-pro' ),
					'options' => array(
						array(
							'value' => 'woocommerce_before_thankyou',
							'label' => esc_attr__( 'Before Order Received Content', 'thewebs-pro' ),
						),
						array(
							'value' => 'woocommerce_thankyou',
							'label' => esc_attr__( 'After Order Received Content', 'thewebs-pro' ),
						),
					),
				),
				array(
					'label' => esc_attr__( 'Shop Widget Toggle Off Canvas Area', 'thewebs-pro' ),
					'options' => array(
						array(
							'value' => 'thewebs_before_product_off_canvas_filter',
							'label' => esc_attr__( 'Before Widget Toggle Off Canvas Content', 'thewebs-pro' ),
						),
						array(
							'value' => 'thewebs_after_product_off_canvas_filter',
							'label' => esc_attr__( 'After Widget Toggle Off Canvas Content', 'thewebs-pro' ),
						),
					),
				),
			);
		...
		if ( class_exists( 'SFWD_LMS' ) ) {
			$learn_add = array(
				array(
					'label' => esc_attr__( 'LearnDash Focus Mode', 'thewebs-pro' ),
					'options' => array(
						array(
							'value' => 'learndash-focus-sidebar-nav-before',
							'label' => esc_attr__( 'Focus Mode: Before Sidebar Nav', 'thewebs-pro' ),
						),
						array(
							'value' => 'learndash-focus-sidebar-nav-after',
							'label' => esc_attr__( 'Focus Mode: After Sidebar Nav', 'thewebs-pro' ),
						),
						array(
							'value' => 'learndash-focus-content-title-before',
							'label' => esc_attr__( 'Focus Mode: Before Title', 'thewebs-pro' ),
						),
						array(
							'value' => 'learndash-focus-content-content-before',
							'label' => esc_attr__( 'Focus Mode: Before Content', 'thewebs-pro' ),
						),
						array(
							'value' => 'learndash-focus-content-content-after',
							'label' => esc_attr__( 'Focus Mode: After Content', 'thewebs-pro' ),
						),
					),
				),
				array(
					'label' => esc_attr__( 'LearnDash Course Page', 'thewebs-pro' ),
					'options' => array(
						array(
							'value' => 'learndash-course-before',
							'label' => esc_attr__( 'Before Course Content', 'thewebs-pro' ),
						),
						array(
							'value' => 'learndash-course-after',
							'label' => esc_attr__( 'After Course Content', 'thewebs-pro' ),
						),
					),
				),
				array(
					'label' => esc_attr__( 'LearnDash Group Page', 'thewebs-pro' ),
					'options' => array(
						array(
							'value' => 'learndash_group_before',
							'label' => esc_attr__( 'Before Group Content', 'thewebs-pro' ),
						),
						array(
							'value' => 'learndash_group_after',
							'label' => esc_attr__( 'After Group Content', 'thewebs-pro' ),
						),
					),
				),
			);
		...
		if ( defined( 'TRIBE_EVENTS_FILE' ) ) {
			$events_add = array(
				array(
					'label' => esc_attr__( 'The Events Calendar', 'thewebs-pro' ),
					'options' => array(
						array(
							'value' => 'thewebs_tribe_events_before_main_tag',
							'label' => esc_attr__( 'Events: Before Content', 'thewebs-pro' ),
						),
						array(
							'value' => 'thewebs_tribe_events_after_main_tag',
							'label' => esc_attr__( 'Events: After Content', 'thewebs-pro' ),
						),
						array(
							'value' => 'tribe_events_single_event_before_the_content',
							'label' => esc_attr__( 'Single Event: Before Inner Content (Classic Mode Only)', 'thewebs-pro' ),
						),
						array(
							'value' => 'tribe_events_single_event_after_the_content',
							'label' => esc_attr__( 'Single Event: After Inner Content (Classic Mode Only)', 'thewebs-pro' ),
						),
						array(
							'value' => 'tribe_events_single_event_before_the_meta',
							'label' => esc_attr__( 'Single Event: Before Meta (Classic Mode Only)', 'thewebs-pro' ),
						),
						array(
							'value' => 'tribe_events_single_event_after_the_meta',
							'label' => esc_attr__( 'Single Event: After Meta (Classic Mode Only)', 'thewebs-pro' ),
						),
					),
				),
			);
		...
		$custom_add = array(
			array(
				'label' => esc_attr__( 'Custom', 'thewebs-pro' ),
				'options' => array(
					array(
						'value' => 'custom',
						'label' => esc_attr__( 'Custom Hook', 'thewebs-pro' ),
					),
				),
			),
		);
		$hooks = array_merge( $hooks, $custom_add );
		return apply_filters( 'thewebs_pro_element_hooks_options', $hooks );
	}
	/**
	...
	public function register_post_type() {
		$labels = array(
			'name'                  => __( 'Elements', 'thewebs_pro' ),
			'singular_name'         => __( 'Element', 'thewebs_pro' ),
			'menu_name'             => _x( 'Elements', 'Admin Menu text', 'thewebs_pro' ),
			'add_new'               => _x( 'Add New', 'Element', 'thewebs_pro' ),
			'add_new_item'          => __( 'Add New Element', 'thewebs_pro' ),
			'new_item'              => __( 'New Element', 'thewebs_pro' ),
			'edit_item'             => __( 'Edit Element', 'thewebs_pro' ),
			'view_item'             => __( 'View Element', 'thewebs_pro' ),
			'all_items'             => __( 'All Elements', 'thewebs_pro' ),
			'search_items'          => __( 'Search Elements', 'thewebs_pro' ),
			'parent_item_colon'     => __( 'Parent Element:', 'thewebs_pro' ),
			'not_found'             => __( 'No Elements found.', 'thewebs_pro' ),
			'not_found_in_trash'    => __( 'No Elements found in Trash.', 'thewebs_pro' ),
			'archives'              => __( 'Element archives', 'thewebs_pro' ),
			'insert_into_item'      => __( 'Insert into Element', 'thewebs_pro' ),
			'uploaded_to_this_item' => __( 'Uploaded to this Element', 'thewebs_pro' ),
			'filter_items_list'     => __( 'Filter Elements list', 'thewebs_pro' ),
			'items_list_navigation' => __( 'Elements list navigation', 'thewebs_pro' ),
			'items_list'            => __( 'Elements list', 'thewebs_pro' ),
		);

		$args = array(
			'labels'             => $labels,
			'description'        => __( 'Element areas to include in your site.', 'thewebs_pro' ),
			'public'             => apply_filters( 'thewebs_element_public_cpt', true ),
			'publicly_queryable' => apply_filters( 'thewebs_element_public_cpt', true ),
			'has_archive'        => false,
			...
			'show_in_rest'       => true,
			'rewrite'            => false,
			'rest_base'          => 'thewebs_element',
			'capability_type'    => array( 'thewebs_element', 'thewebs_elements' ),
			'map_meta_cap'       => true,
			'supports'           => array(
				'title',
				'editor',
	...
	public function filter_post_type_user_caps( $allcaps ) {
		if ( isset( $allcaps['edit_theme_options'] ) ) {
			$allcaps['edit_thewebs_elements']             = $allcaps['edit_theme_options'];
			$allcaps['edit_others_thewebs_elements']      = $allcaps['edit_theme_options'];
			$allcaps['edit_published_thewebs_elements']   = $allcaps['edit_theme_options'];
			$allcaps['edit_private_thewebs_elements']     = $allcaps['edit_theme_options'];
			$allcaps['delete_thewebs_elements']           = $allcaps['edit_theme_options'];
			$allcaps['delete_others_thewebs_elements']    = $allcaps['edit_theme_options'];
			$allcaps['delete_published_thewebs_elements'] = $allcaps['edit_theme_options'];
			$allcaps['delete_private_thewebs_elements']   = $allcaps['edit_theme_options'];
			$allcaps['publish_thewebs_elements']          = $allcaps['edit_theme_options'];
			$allcaps['read_private_thewebs_elements']     = $allcaps['edit_theme_options'];
		}
	...
	private function filter_post_type_columns( array $columns ) : array {
		...
		$add = array(
			'type'            => esc_html__( 'Type', 'thewebs-pro' ),
			'hook'            => esc_html__( 'Placement', 'thewebs-pro' ),
			'display'         => esc_html__( 'Display On', 'thewebs-pro' ),
			'user_visibility' => esc_html__( 'Visible To', 'thewebs-pro' ),
			'shortcode'       => esc_html__( 'Shortcode', 'thewebs-pro' ),
			'status'          => esc_html__( 'Status', 'thewebs-pro' ),
		);
	...
	private function render_post_type_column( string $column_name, int $post_id ) {
		...
		if ( 'status' === $column_name ) {
			if ( 'publish' === $post->post_status || 'draft' === $post->post_status ) {
				$title = ( 'publish' === $post->post_status ? __( 'Published', 'thewebs-pro' ) : __( 'Draft', 'thewebs-pro' ) );
				echo '<button class="thewebs-status-toggle thewebs-element-status thewebs-status-' . esc_attr( $post->post_status ) . '" data-post-status="' . esc_attr( $post->post_status ) . '" data-post-id="' . esc_attr( $post_id ) . '"><span class="thewebs-toggle"></span><span class="thewebs-status-label">' . $title . '</span><span class="spinner"></span></button>';
			} else {
				echo '<div class="thewebs-static-status-toggle">' . esc_html( $post->post_status ) . '</div>';
			}
		}
		if ( 'hook' === $column_name ) {
		...
		if ( 'type' === $column_name ) {
			if ( isset( $meta['type'] ) && ! empty( $meta['type'] ) ) {
				...
				echo esc_html__( 'Default', 'thewebs-pro' );
			}
		}
		if ( 'display' === $column_name ) {
			if ( isset( $meta ) && isset( $meta['show'] ) && is_array( $meta['show'] ) && ! empty( $meta['show'] ) ) {
				...
				echo esc_html__( '[UNSET]', 'thewebs-pro' );
			}
		}
		if ( 'user_visibility' === $column_name ) {
			if ( isset( $meta ) && isset( $meta['user'] ) && is_array( $meta['user'] ) && ! empty( $meta['user'] ) ) {
				...
				if ( count( $show_roles ) !== 0 ) {
					echo esc_html__( 'Visible to:', 'thewebs-pro' );
					echo '<br>';
		...
		if ( 'shortcode' === $column_name ) {
			echo '<code>[thewebs_element id="' . esc_attr( $post_id ) . '"]</code>';
		}
	}
