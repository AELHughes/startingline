...
class Conditional_Headers {
	...
	public function __construct() {
		...
		add_filter( 'thewebs_settings_extra_source', array( $this, 'add_current_header_source' ) );
		...
		// Ajax Calls.
		add_action( 'wp_ajax_thewebs_pro_header_import', array( $this, 'import_data' ) );
		add_action( 'wp_ajax_thewebs_pro_header_remove', array( $this, 'remove_header' ) );
		add_action( 'wp_ajax_thewebs_pro_header_default', array( $this, 'import_default_data' ) );
		add_action( 'wp_ajax_thewebs_pro_header_export', array( $this, 'export_data' ) );
	}
	/**
	 * Imports uploaded thewebs conditional header settings
	 */
	public function remove_header() {
		// Make sure we have a valid nonce.
		if ( ! wp_verify_nonce( $_POST['security'], 'thewebs-conditional-header-importing' ) ) {
			return;
		}
		$option_key = sanitize_text_field( $_POST['header'] );
		if ( empty( $option_key ) ) {
			wp_send_json_error( __( 'Error removing settings, Missing option information.', 'thewebs-pro' ) );
		}
	...
	public function import_default_data() {
		// Make sure we have a valid nonce.
		if ( ! wp_verify_nonce( $_POST['security'], 'thewebs-conditional-header-importing' ) ) {
			wp_send_json_error( __( 'Security Error, Reload Page', 'thewebs-pro' ) );
		}
		$option_key = sanitize_text_field( $_POST['header'] );
		if ( empty( $option_key ) ) {
			wp_send_json_error( __( 'Error importing settings, Missing option information.', 'thewebs-pro' ) );
		}
		$data    = array();
	...
	public function import_data() {
		...
		if ( ! file_exists( $file['file'] ) ) {
			wp_send_json_error( __( 'Error importing settings file! Please try again.', 'thewebs-pro' ) );
			return;
		}
		...
		if ( 'array' != gettype( $data ) ) {
			wp_send_json_error( __( 'Error importing settings! Please check that you uploaded a customizer export file.', 'thewebs-pro' ) );
			return;
		}
		...
		if ( $data['header'] != $template ) {
			wp_send_json_error( __( 'Error importing settings! The settings you uploaded are not for the Thewebs Theme Conditional Header.', 'thewebs-pro' ) );
			return;
		}
	...
	public function get_filename() {
		return 'thewebs-header-data-export.dat';
	}
	/**
	...
	public function export_data() {
		if ( ! wp_verify_nonce( $_POST['security'], 'thewebs-conditional-header-exporting' ) ) {
			return;
		}
		$option_key = sanitize_text_field( $_POST['header'] );
		if ( empty( $option_key ) ) {
		...
		// // Set the download headers.
		// header( 'Content-disposition: attachment; filename=thewebs-header-data-export.dat' );
		// header( 'Content-Type: application/octet-stream; charset=' . $charset );
	...
	public function add_current_header_source( $source ) {
		$preview_header = thewebs()->option( 'current_header_preview' );
	...
	public function enqueue_customizer_scripts() {
		...
		$script     = $wp_scripts->query( 'thewebs-customizer-controls', 'registered' );
		...
		$path = KTP_URL . 'build/';
		wp_enqueue_script( 'thewebs-pro-customizer-controls', $path . 'customizer.js', $editor_dependencies, KTP_VERSION, true );
		wp_enqueue_style( 'thewebs-conditional-controls', KTP_URL . '/dist/build/customizer-controls.css', false, KTP_VERSION );
		wp_localize_script(
			'thewebs-pro-customizer-controls',
			'thewebsCustomizerConditionalData',
			array(
				...
				'resetConfirm'   => __( "Attention! This will remove all customizations to this header!\n\nThis action is irreversible!", 'thewebs-pro' ),
				'emptyImport'	 => __( 'Please choose a file to import.', 'thewebs-pro' ),
				'conditional_url'	 => admin_url( 'customize.php' ) . '?autofocus%5Bsection%5D=thewebs_customizer_conditional_header',
				'header_url'	 => admin_url( 'customize.php' ) . '?autofocus%5Bpanel%5D=thewebs_customizer_header',
				'ajax_url'       => admin_url( 'admin-ajax.php' ),
				'nonce'          => array(
					'export' => wp_create_nonce( 'thewebs-conditional-header-exporting' ),
					'import' => wp_create_nonce( 'thewebs-conditional-header-importing' ),
				),
			)
		);
		if ( function_exists( 'wp_set_script_translations' ) ) {
			wp_set_script_translations( 'thewebs-pro-customizer-controls', 'thewebs-pro' );
		}
	}
	/**
	...
	public function get_language_options() {
		...
		return apply_filters( 'thewebs_pro_conditional_header_display_languages', $languages_options );
	}
	/**
	 * Get all taxonomies
	 */
	public function get_taxonomies() {
		...
		return apply_filters( 'thewebs_pro_conditional_header_display_taxonomies', $output );
	}
	/**
	...
	public function get_author_options() {
		...
		return apply_filters( 'thewebs_pro_conditional_header_display_authors', $output );
	}
	/**
	...
	public function get_display_options() {
		...
		if ( defined( 'TRIBE_EVENTS_FILE' ) ) {
			$thewebs_public_post_types = array_merge( $thewebs_public_post_types, array( 'tribe_events' ) );
		}
		$ignore_types              = thewebs()->get_public_post_types_to_ignore();
		...
		return apply_filters( 'thewebs_pro_conditional_header_display_options', $display );
	}
	/**
	 * Get all Display Options
	 */
	public function get_user_options() {
		...
		return apply_filters( 'thewebs_pro_conditional_header_user_options', $roles );
	}
	/**
	...
	public function init_hook_in_current_header() {
		...
		if ( is_customize_preview() ) {
			...
			$conditional_headers = thewebs()->option( 'conditional_headers' );
			...
			if ( ! empty( $the_contitional_headers ) ) {
				foreach ( $the_contitional_headers as $key => $header ) {
					if ( apply_filters( 'thewebs_conditional_header_display', $this->check_header_conditionals( $header ), $header ) ) {
		...
		if ( ! empty( $current_header ) ) {
			add_filter(
				'thewebs_settings_key_custom_mapping',
	...
	public function add_option_defaults( $defaults ) {
		$conditional_headers = array(
			...
			'conditional_headers'  => array(
				'items' => array(
					array(
						'id'             => 'thewebs_conditional_header_01',
						'label'          => 'Extra Header',
						'enabled'        => false,
						'show'           => '',
						'all_show'       => false,
	...
	public function add_customizer_sections( $sections ) {
		$sections['conditional_header']        = array(
			'title'    => __( 'Conditional Headers', 'thewebs-pro' ),
			'panel'    => 'header',
			'priority' => 20,
		);
		return $sections;
	...
	public function register_controls( $wp_customize ) {
		require_once KTP_PATH . 'dist/conditional-headers/class-thewebs-control-conditional.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once KTP_PATH . 'dist/conditional-headers/class-thewebs-control-conditional-select.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once KTP_PATH . 'dist/conditional-headers/class-thewebs-control-conditional-heading.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
	}
}
