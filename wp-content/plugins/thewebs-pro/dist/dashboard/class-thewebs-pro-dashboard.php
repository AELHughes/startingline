 ...
 * @category class
 */
class Thewebs_Pro_API_Manager {
	/**
	 * This is where we make api calls.
	 *
	 * @var api url
	 */
	public $upgrade_url = 'https://www.thewebswp.com/';
	 ...
	 * @var url
	 */
	private $fallback_api_url = 'https://api.thewebswp.com/';
	 ...
	 * @var url
	 */
	private $renewal_url = 'https://www.thewebswp.com/my-account/';
	...
	/**
	 * This is the theme name for the thewebs theme.
	 *
	 * @var theme name
	 */
	...
	public function __construct( $kt_product_id_key, $kt_instance_key, $kt_activated_key, $kt_product_id, $kt_product_name ) {
		...
		if ( is_admin() ) {
			...
			add_action( 'thewebs_theme_dash_side_panel', array( $this, 'render_license_form' ) );
			add_action( 'wp_ajax_thewebs_add_elementor', array( $this, 'add_elementor_ajax_callback' ) );
			 ...
			 */
			$this->kt_product_name                = apply_filters( 'thewebs_whitelabel_theme_name', $kt_product_name );
			$this->kt_license_link                = apply_filters( 'thewebs_whitelabel_license_link', $this->renewal_url );
			$this->kt_product_id_key              = $kt_product_id_key;
			$this->kt_instance_key                = $kt_instance_key;
			$this->kt_activated_key               = $kt_activated_key;
			$this->kt_settings_menu_title         = __( 'Getting Started', 'thewebs-pro' );
			$this->kt_menu_tab_activation_title   = __( 'API License Activation', 'thewebs-pro' );
			$this->kt_menu_tab_deactivation_title = __( 'Deactivation', 'thewebs-pro' );
	...
	public function options_update() {
		if ( isset( $_POST['thewebs_theme_license_form_submit'] ) ) {
	...
	public function add_elementor_ajax_callback() {
		if ( ! check_ajax_referer( 'thewebs-ajax-verification', 'security', false ) ) {
			wp_send_json_error( __( 'Security Error, please reload the page.', 'thewebs-pro' ) );
		}
		if ( class_exists( '\Elementor\Plugin' ) ) {
			$cpt_support = get_option( 'elementor_cpt_support' );
			if ( ! $cpt_support ) {
				$cpt_support = array( 'page', 'post', 'thewebs_element' );
				update_option( 'elementor_cpt_support', $cpt_support );
			} else if ( ! in_array( 'thewebs_element', $cpt_support ) ) {
				$cpt_support[] = 'thewebs_element';
				update_option( 'elementor_cpt_support', $cpt_support );
			}
		}
	...
	public function license_form_submit() {
		...
		if ( isset( $_POST['thewebs_theme_license_form_submit'] ) && isset( $_POST['thewebs_theme_license_deactivate'] ) && wp_verify_nonce( $_POST['thewebs_theme_license_nonce'], 'thewebs_theme_license') ) {
			$this->validate_deactivation_options();
		}

		if ( isset( $_POST['thewebs_theme_license_form_submit'] ) && isset( $_POST['thewebs_theme_license_activate'] ) && wp_verify_nonce( $_POST['thewebs_theme_license_nonce'], 'thewebs_theme_license' ) ) {
	...
	public function load_api_settings() {
		register_setting(
			'thewebs_pro_theme_config',
			'thewebs_pro_theme_config',
			array(
				'type'              => 'string',
				'description'       => __( 'Config Thewebs Pro Modules', 'thewebs-blocks' ),
	...
	public function hide_inactive_notice() {
		if ( isset( $_GET['thewebs-wp-activation-notice'] ) && isset( $_GET['_notice_nonce'] ) ) {
			if ( ! wp_verify_nonce( wp_unslash( sanitize_key( $_GET['_notice_nonce'] ) ), 'thewebs_wp_hide_activation_notice' ) ) {
				wp_die( esc_html__( 'Authorization failed. Please refresh the page and try again.', 'thewebs-pro' ) );
			}
	...
	public function inactive_notice() {
		...
		if ( isset( $_GET['page'] ) && 'thewebs' === $_GET['page'] ) {
			return;
		}
		...
		<div id="message" class="error" style="position: relative;">
			<p><?php /* translators: %1$s and %2$s refer to an internal link markup */ printf( __( 'The Pro Thewebs Theme has not been activated! %1$sClick here%2$s to activate the license key.', 'thewebs-pro' ), '<a href="' . esc_url( admin_url( 'themes.php?page=thewebs' ) ) . '">', '</a>' ); ?></p>
				<a href="<?php echo esc_url( wp_nonce_url( add_query_arg( 'thewebs-wp-activation-notice', 'install' ), 'thewebs_wp_hide_activation_notice', '_notice_nonce' ) ); ?>" style="text-decoration:none" class="notice-dismiss kt-close-theme-notice"><span class="screen-reader-text"><?php esc_html_e( 'hide', 'thewebs-pro' ); ?></span></a>
		</div>
		<?php
	}
	...
	public function check_external_blocking() {
		...
		if ( defined( 'WP_HTTP_BLOCK_EXTERNAL' ) && WP_HTTP_BLOCK_EXTERNAL === true ) {
			...
			if ( ! defined( 'WP_ACCESSIBLE_HOSTS' ) || stristr( WP_ACCESSIBLE_HOSTS, $host ) === false ) {
				?>
				<div class="error">
				<p><?php /* translators: %1$s  refers to product name. %2$s refer to host information and %3$s an internal link markup */ printf( esc_html__( 'Warning! You\'re blocking external requests which means you won\'t be able to get %1$s updates. Please add %2$s to %3$s.', 'thewebs-pro' ), $this->kt_product_id, '<strong>' . $host . '</strong>', '<code>WP_ACCESSIBLE_HOSTS</code>' ); ?></p>
				</div>
				<?php
			}
	...
	public function admin_interface_notices() {
		global $thewebs_theme_license_interface_messages;

		if ( isset( $_GET['page'] ) && 'thewebs' !== $_GET['page'] ) {
			return;
		}

		if ( ! is_array( $thewebs_theme_license_interface_messages ) ) {
			return;
		}

		if ( count( $thewebs_theme_license_interface_messages ) > 0 ) {
			foreach ( $thewebs_theme_license_interface_messages as $message ) {
	...
	public function license_form() {
		...
		?>
		<form name="form" method="post" id="thewebs-pro-license-activate">
			<input type="hidden" name="thewebs_theme_license_form_submit" value="true" />
			<input type="hidden" name="thewebs_theme_license_activate" value="true" />
			<?php wp_nonce_field( 'thewebs_theme_license', 'thewebs_theme_license_nonce' ); ?>
			<h2>Thewebs Pro<span class="kt-license-status k-inactive">Inactive</span></h2>
			<div class="toggle-ithemes" style="font-size: 11px;margin-bottom: 10px;">
				<input onChange="document.getElementById('thewebs-pro-license-activate').submit()" <?php echo checked( $is_ithemes, true ); ?> type="checkbox" id="thewebs_toggle_plateform" name="<?php echo esc_attr( $this->kt_data_key . '[ithemes]' ); ?>" value="on">
				<label for="thewebs_toggle_plateform"><?php echo __( 'Use iThemes Toolkit/Agency License', 'thewebs-pro' ); ?></label>
			</div>
			Please activate your license to get updates. If you need your api key you will find it by <a href="https://www.thewebswp.com/my-account/" target="_blank">logging into your account</a>.
			<?php
			if ( $is_ithemes ) {
				?>
			...
			<?php } ?>
			<p class="submit">
				<input type="submit" name="bnt-submit" id="btn-submit" class="button button-primary" value="<?php echo esc_attr__( 'Activate', 'thewebs-pro' ); ?>">
			</p>
		</form>
		<?php
	}
	...
	public function license_deactivate_form() {
		...
		<form id="form_data" name="form" method="post">
			<input type="hidden" name="thewebs_theme_license_form_submit" value="true" />
			<input type="hidden" name="thewebs_theme_license_deactivate" value="true" />
			<?php wp_nonce_field( 'thewebs_theme_license', 'thewebs_theme_license_nonce' ); ?>
			<h2>Thewebs Pro<span class="kt-license-status k-active">Active</span></h2>
			<?php
			if ( $is_ithemes ) {
				?>
				<table class="form-table" role="presentation">
			...
			<?php } ?>
			<p class="submit">
				<input type="submit" name="submit" id="submit" class="button button-primary" value="<?php echo esc_attr__( 'Deactivate', 'thewebs-pro' ); ?>">
			</p>
		</form>
		<?php
	}
	...
	public function validate_activation_options() {
		...
		if ( $activation_status == 'Deactivated' || $activation_status == '' || $api_key == '' || $api_email == '' || $checkbox_status == 'on' || $current_api_key != $api_key ) {
			...
			if ( isset( $activate_results['activated'] ) && $activate_results['activated'] === true ) {
				$thewebs_theme_license_interface_messages[] = array(
					'type' => 'updated',
					'text' => __( 'Pro activated.', 'thewebs-pro' ),
				);
			...
			if ( $activate_results == false ) {
				$thewebs_theme_license_interface_messages[] = array(
					'type' => 'error',
					'text' => __( 'Connection failed to the License Key API server. Make sure your host servers php version has the curl module installed and enabled.', 'thewebs-pro' ),
				);
	...
	public function replace_license_key( $current_api_key ) {
		...
		$thewebs_theme_license_interface_messages[] = array(
			'type' => 'error',
			'text' => __( 'The license could not be deactivated. Please try again.', 'thewebs-pro' ),
		);
		...
		if ( $activation_status === 'Activated' && $this->kt_options[ $this->kt_api_key ] != '' && $this->kt_options[ $this->kt_activation_email ] != '' ) {
			...
			if ( $activate_results['deactivated'] == true ) {
				if ( ! empty( $this->kt_activated_key ) ) {
					...
					$thewebs_theme_license_interface_messages[] = array(
						'type' => 'updated',
						'text' => __( 'Pro license deactivated. ', 'thewebs-pro' ),
					);
				}
	...
	public function scripts() {
		...
		wp_enqueue_script( 'thewebs-pro-dashboard', KTP_URL . 'build/dashboard.js', array( 'wp-i18n', 'wp-element', 'wp-plugins', 'wp-components', 'wp-api', 'wp-hooks', 'wp-edit-post', 'lodash', 'wp-block-library', 'wp-block-editor', 'wp-editor', 'jquery' ), KTP_VERSION, true );
		wp_localize_script(
			'thewebs-pro-dashboard',
			'thewebsProDashboardParams',
			array(
				'adminURL'   => admin_url(),
				'settings'   => get_option( 'thewebs_pro_theme_config' ),
		...
		if ( function_exists( 'wp_set_script_translations' ) ) {
			wp_set_script_translations( 'thewebs-pro-dashboard', 'thewebs-pro' );
		}
	}
	/**
...
Thewebs_Pro_API_Manager::instance( 'thewebs_pro_activation', 'thewebs_pro_api_manager_instance', 'thewebs_pro_api_manager_activated', 'thewebs_pro', 'Thewebs Pro' );
