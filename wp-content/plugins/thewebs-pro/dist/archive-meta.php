...
class Archive_Meta {
	...
	public function generate_archive_css() {
		...
		if ( is_archive() && thewebs()->show_hero_title() ) {
			$term_id   = get_queried_object_id();
			if ( $term_id ) {
				$post_type = get_post_type();
				...
				if ( $image_id ) {
					...
					if ( $image ) {
						...
						$bg_settings = thewebs()->sub_option( $post_type . '_archive_title_background', 'desktop' );
	...
	public function init() {
		$this->_taxonomies = apply_filters( 'thewebs-pro-archive-image-taxonomies', get_taxonomies() );
		foreach ( $this->_taxonomies as $tax_name ) {
			add_action( $tax_name . '_edit_form_fields', array( $this, 'show_thewebs_fields' ), 11, 2 );
		}
	}
	/**
	 * Add media script.
	...
	public function show_image_field_pre( $taxonomy ) {
		...
		<div class="form-field term-group">
			<label for="kwp-tax-image-id"><?php esc_html_e( 'Above Header Background Image', 'thewebs-pro' ); ?></label>
			...
			<p>
				<input type="button" class="button button-secondary kwp-tax-media-button" id="kwp-tax-media-button" name="kwp-tax-media-button" value="<?php esc_html_e( 'Add Image', 'thewebs-pro' ); ?>" />
				<input type="button" class="button button-secondary kwp-tax-media-remove" id="kwp-tax-media-remove" name="kwp-tax-media-remove" value="<?php esc_html_e( 'Remove Image', 'thewebs-pro' ); ?>" />
			</p>
		</div>
		<?php
	}
	 ...
	 * Edit the form field
	 */
	public function show_thewebs_fields( $term, $taxonomy ) {
		?>
		...
		if ( $term->taxonomy !== 'product_cat' && $term->taxonomy !== 'product_tag' && $term->taxonomy !== 'product_brands' ) {
			...
			<tr class="form-field term-group-wrap">
				<th scope="row">
					<label for="kwp-tax-columns"><?php esc_html_e( 'Archive Columns', 'thewebs-pro' ); ?></label>
				</th>
				<td>
	...
	public function add_customizer_sections( $sections ) {
		$sections['scripts'] = array(
			'title'    => __( 'Custom Scripts', 'thewebs-pro' ),
			'priority' => 889,
		);
		return $sections;
	}
