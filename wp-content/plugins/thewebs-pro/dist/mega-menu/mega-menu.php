...
class Mega_Menu {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_thewebs_get_menu_item_data', array( $this, 'get_item_data_ajax_callback' ) );
		add_action( 'wp_ajax_thewebs_save_menu_item_data', array( $this, 'save_item_data_ajax_callback' ) );
	...
	public function frontend_settings( $nav_menu_args, $nav_menu, $args, $instance ) {
		if ( apply_filters( 'thewebs_ultimate_menu_addon_widget_nav', true ) ) {
	...
	public function apply_menu_item_replace_changes( $item_output, $item, $depth, $args ) {
		...
		if ( ! empty( $data['menu_item_custom'] ) && $data['menu_item_custom'] && ! empty( $data['menu_item_custom_element'] ) && is_array( $data['menu_item_custom_element'] ) && isset( $data['menu_item_custom_element']['value'] ) ) {
			ob_start();
			echo do_shortcode( '[thewebs_element id="'. $data['menu_item_custom_element']['value'] . '"]' );
			...
			if ( isset( $data['menu_item_custom_divider'] ) && is_array( $data['menu_item_custom_divider'] ) && isset( $data['menu_item_custom_divider']['width'] ) ) {
				...
				$css->set_selector( '.header-navigation .header-menu-container .thewebs-menu-mega-enabled > .sub-menu > li#menu-item-' . $item->ID . ' > a' );
	...
	public function output_css() {
		if ( ! empty( self::$css ) ) {
			wp_register_style( 'thewebs_mega_menu_inline', false );
			wp_enqueue_style( 'thewebs_mega_menu_inline' );
			wp_add_inline_style( 'thewebs_mega_menu_inline', self::$css );
			wp_print_styles( 'thewebs_mega_menu_inline' );
		}
	}
	/**
	...
	public function apply_menu_class_changes( $classes, $item, $args, $depth ) {
		...
		if ( $data['mega_menu'] ) {
			wp_enqueue_script( 'thewebs-mega-menu' );
			...
			$classes[] = 'thewebs-menu-mega-enabled';
			$classes[] = ( $data['mega_menu_width'] ? 'thewebs-menu-mega-width-' . $data['mega_menu_width'] : 'thewebs-menu-mega-width-content' );
			$classes[] = ( $data['mega_menu_columns'] ? 'thewebs-menu-mega-columns-' . $data['mega_menu_columns'] : 'thewebs-menu-mega-columns-3' );
			$classes[] = ( $data['mega_menu_layout'] ? 'thewebs-menu-mega-layout-' . $data['mega_menu_layout'] : 'thewebs-menu-mega-layout-equal' );
			...
			if ( isset( $data['mega_menu_width'] ) && 'custom' === $data['mega_menu_width'] ) {
				...
				$css->set_selector( '#menu-item-' . $item->ID . '.thewebs-menu-mega-enabled > .sub-menu' );
				$css->add_property( 'width', ( $data['mega_menu_custom_width'] ? $data['mega_menu_custom_width'] : '400' ) . 'px' );
				$css->set_selector( '.header-navigation[class*="header-navigation-dropdown-animation-fade"] #menu-item-' . $item->ID . '.thewebs-menu-mega-enabled > .sub-menu' );
			...
			if ( is_array( $data['menu_dropdown_item_color'] ) ) {
				if ( isset( $data['menu_dropdown_item_color']['color'] ) && ! empty( $data['menu_dropdown_item_color']['color'] ) ) {
					$css->set_selector( '.header-navigation .header-menu-container #menu-item-' . $item->ID . '.thewebs-menu-mega-enabled > .sub-menu li.menu-item > a' );
				...
				if ( isset( $data['menu_dropdown_item_color']['hover'] ) && ! empty( $data['menu_dropdown_item_color']['hover'] ) ) {
					$css->set_selector( '.header-navigation .header-menu-container #menu-item-' . $item->ID . '.thewebs-menu-mega-enabled > .sub-menu li.menu-item > a:hover' );
				...
				if ( isset( $data['menu_dropdown_item_color']['active'] ) && ! empty( $data['menu_dropdown_item_color']['active'] ) ) {
					$css->set_selector( '.header-navigation .header-menu-container #menu-item-' . $item->ID . '.thewebs-menu-mega-enabled > .sub-menu li.menu-item.current-menu-item > a' );
			...
			if ( isset( $data['menu_dropdown_item_divider'] ) && is_array( $data['menu_dropdown_item_divider'] ) && isset( $data['menu_dropdown_item_divider']['width'] ) ) {
				...
				$css->set_selector( '.header-navigation .header-menu-container #menu-item-' . $item->ID . '.thewebs-menu-mega-enabled > .sub-menu > li.menu-item > a, .header-navigation .header-menu-container #menu-item-' . $item->ID . '.thewebs-menu-mega-enabled li.menu-item' );
	...
	public function enqueue_scripts() {
		...
		wp_register_script( 'thewebs-mega-menu', KTP_URL . 'dist/mega-menu/thewebs-mega-menu.min.js', array(), KTP_VERSION, true );
	}
	/**
	...
	public function scripts( $hook ) {
		...
		wp_enqueue_style( 'thewebs-pro-mega',  KTP_URL . 'dist/build/mega-menu-controls.min.css', array( 'wp-components' ), KTP_VERSION );
		wp_enqueue_script( 'thewebs-pro-mega', KTP_URL . 'build/mega-menu.js', array( 'wp-i18n', 'wp-element', 'wp-plugins', 'wp-components', 'wp-api', 'wp-hooks', 'wp-edit-post', 'lodash', 'wp-block-library', 'wp-block-editor', 'wp-editor', 'jquery' ), KTP_VERSION, true );
		$settings = array(
	...
	public function render_field_control_containers( $item_id, $item, $depth, $args, $id ) {
		?>
		<p class="field-thewebs-mega description-wide thewebs-menu-options" data-item-id="<?php echo esc_attr( $item_id ); ?>" data-nav-id="<?php echo esc_attr( $id ); ?>">
			<!-- <button class="button button-secondary thewebs-mega-options-button" data-item-id="<?php echo esc_attr( $item_id ); ?>">
				<?php esc_html_e( 'Mega Menu - Container Settings', 'thewebs-pro' ); ?>
			</button> -->
		</p>
		<?php
	}
	...
	public function render_modal_container() {
		...
		?>
		<div id="thewebs-mega-options-panel">
		</div>
		<?php
	}
}
