...
class Woocommerce_Addons {
	...
	public function __construct() {
		...
		add_filter( 'thewebs_product_archive_show_top_row', array( $this, 'check_to_show_product_top_row' ) );
		...
		add_action( 'wp_ajax_nopriv_thewebs_pro_add_to_cart', array( $this, 'add_to_cart' ) );
		add_action( 'wp_ajax_thewebs_pro_add_to_cart', array( $this, 'add_to_cart' ) );
		...
		add_action( 'thewebs_single_product_ajax_added_to_cart', array( $this, 'thewebs_check_for_fragment_support' ) );
	}
	/**
	 * Add mini cart notice.
	 */
	public function thewebs_check_for_fragment_support() {
		$thewebs_theme_class = Theme::instance();
		if ( isset( $thewebs_theme_class->components['woocommerce'] ) && method_exists( $thewebs_theme_class->components['woocommerce'], 'check_for_fragment_support' ) ) {
			$thewebs_theme_class->components['woocommerce']->check_for_fragment_support();
		}
	}
	/**
	 * Add mini cart notice.
	...
	public function add_mini_cart_notice() {
		if ( thewebs()->option( 'cart_pop_show_free_shipping' ) ) {
			$min_amount = thewebs()->option( 'cart_pop_free_shipping_price' );
			$current = WC()->cart->subtotal;
			$output = '<div class="thewebs-mini-cart-shipping">';
			if ( $current && $current < $min_amount ) {
				$message = thewebs()->option( 'cart_pop_free_shipping_message' );
				$message = str_replace( '{cart_difference}', wc_price( $min_amount - $current ), $message );
				$output .= '<span class="thewebs-mini-cart-shipping-message">';
				$output .= $message;
				$output .= '</span>';
				$output .= '<span class="thewebs-mini-cart-shipping-progress-wrap"><span class="thewebs-mini-cart-shipping-progress" style="width:' . esc_attr( ( $current / $min_amount ) * 100 ) . '%">';
				$output .= '</span></span>';
			}
			$output .= '</div>';
			echo $output;
	...
	public static function add_to_cart() {
		...
		if ( $passed_validation && false !== WC()->cart->add_to_cart( $product_id, $quantity, $variation_id, $variation ) && 'publish' === $product_status ) {
			...
			do_action( 'thewebs_single_product_ajax_added_to_cart', $product_id );
	...
	public function generate_pro_header_css() {
		...
		$css->add_property( 'max-width', $css->render_size( thewebs()->option( 'product_filter_widget_pop_width' ) ) );
		...
		$css->add_property( 'background', $css->render_color( thewebs()->sub_option( 'product_archive_shop_filter_background', 'color' ) ) );
		$css->add_property( 'color', $css->render_color( thewebs()->sub_option( 'product_archive_shop_filter_color', 'color' ) ) );
		$css->add_property( 'border-color', $css->render_color( thewebs()->sub_option( 'product_archive_shop_filter_border_color', 'color' ) ) );
		$css->add_property( 'padding', $css->render_measure( thewebs()->option( 'product_archive_shop_filter_padding' ) ) );
		$css->render_font( thewebs()->option( 'product_archive_shop_filter_typography' ), $css );
		$css->start_media_query( $media_query['tablet'] );
		$css->set_selector( '.filter-toggle-open-container .filter-toggle-open' );
		$css->add_property( 'font-size', $css->render_font_size( thewebs()->option( 'product_archive_shop_filter_typography' ), 'tablet' ) );
		$css->add_property( 'line-height', $css->render_font_height( thewebs()->option( 'product_archive_shop_filter_typography' ), 'tablet' ) );
		$css->stop_media_query();
		$css->start_media_query( $media_query['mobile'] );
		$css->set_selector( '.filter-toggle-open-container .filter-toggle-open' );
		$css->add_property( 'font-size', $css->render_font_size( thewebs()->option( 'product_archive_shop_filter_typography' ), 'mobile' ) );
		$css->add_property( 'line-height', $css->render_font_height( thewebs()->option( 'product_archive_shop_filter_typography' ), 'mobile' ) );
		$css->stop_media_query();
		$css->set_selector( '.filter-toggle-open-container .filter-toggle-open.filter-toggle-style-bordered' );
		$css->add_property( 'border', $css->render_border( thewebs()->option( 'product_archive_shop_filter_border' ) ) );
		$css->set_selector( '.filter-toggle-open-container .filter-toggle-open .filter-toggle-icon' );
		$css->add_property( 'font-size', thewebs()->sub_option( 'product_archive_shop_filter_icon_size', 'size' ) . thewebs()->sub_option( 'product_archive_shop_filter_icon_size', 'unit' ) );
		...
		$css->add_property( 'color', $css->render_color( thewebs()->sub_option( 'product_archive_shop_filter_color', 'hover' ) ) );
		$css->add_property( 'background', $css->render_color( thewebs()->sub_option( 'product_archive_shop_filter_background', 'hover' ) ) );
		$css->add_property( 'border-color', $css->render_color( thewebs()->sub_option( 'product_archive_shop_filter_border_color', 'hover' ) ) );

		$css->set_selector( '#filter-drawer .drawer-inner' );
		$css->render_background( thewebs()->sub_option( 'product_filter_widget_pop_background', 'desktop' ), $css );
		$css->start_media_query( $media_query['tablet'] );
		$css->set_selector( '#filter-drawer .drawer-inner' );
		$css->render_background( thewebs()->sub_option( 'product_filter_widget_pop_background', 'tablet' ), $css );
		$css->stop_media_query();
		$css->start_media_query( $media_query['mobile'] );
		$css->set_selector( '#filter-drawer .drawer-inner' );
		$css->render_background( thewebs()->sub_option( 'product_filter_widget_pop_background', 'mobile' ), $css );
		$css->stop_media_query();
		$css->set_selector( '#filter-drawer .drawer-header .drawer-toggle, #filter-drawer .drawer-header .drawer-toggle:focus' );
		$css->add_property( 'color', $css->render_color( thewebs()->sub_option( 'product_filter_widget_close_color', 'color' ) ) );
		$css->set_selector( '#filter-drawer .drawer-header .drawer-toggle:hover, #filter-drawer .drawer-header .drawer-toggle:focus:hover' );
		$css->add_property( 'color', $css->render_color( thewebs()->sub_option( 'product_filter_widget_close_color', 'hover' ) ) );
		// Toggle Widget area.
		...
		$css->render_font( thewebs()->option( 'product_filter_widget_title' ), $css );
		$css->set_selector( '#filter-drawer .drawer-inner .product-filter-widgets' );
		$css->render_font( thewebs()->option( 'product_filter_widget_content' ), $css );
		...
		$css->add_property( 'color', $css->render_color( thewebs()->sub_option( 'product_filter_widget_link_colors', 'color' ) ) );
		...
		$css->add_property( 'color', $css->render_color( thewebs()->sub_option( 'product_filter_widget_link_colors', 'hover' ) ) );
		$css->set_selector( '#filter-drawer .drawer-inner .product-filter-widgets' );
		$css->add_property( 'padding', $css->render_measure( thewebs()->option( 'product_filter_widget_padding' ) ) );

		$css->set_selector( '.thewebs-shop-active-filters' );
		$css->add_property( 'display', 'flex' );
		$css->add_property( 'flex-wrap', 'wrap' );
		$css->set_selector( '.thewebs-clear-filters-container a' );
		$css->add_property( 'text-decoration', 'none' );
		...
		$css->set_selector( '.thewebs-clear-filters-container ul' );
		...
		$css->set_selector( '.thewebs-clear-filters-container ul li' );
		$css->add_property( 'float', 'left' );
		$css->add_property( 'padding', '0 0 1px 1px' );
		$css->add_property( 'list-style', 'none' );
		$css->set_selector( '.thewebs-clear-filters-container a:hover' );
	...
	public function action_register_sidebars() {
		$widgets = array(
			'product-filter' => __( 'Catalog Off Canvas Sidebar', 'thewebs-pro' ),
		);
	...
	public function check_to_show_product_top_row( $show ) {
		if ( thewebs()->option( 'product_archive_shop_filter_popout' ) ) {
			return true;
		}
		return $show;
	}
	...
	public function trigger_add_to_cart_on_load() {
		...
		wp_enqueue_script( 'thewebs-pro-woocommerce' );
		wp_localize_script(
			'thewebs-pro-woocommerce',
			'thewebsProWooConfig',
			array(
				'openCart' => $open_cart,
			)
	...
	public function action_enqueue_scripts() {
		...
		if ( ! thewebs()->option( 'cart_pop_show_on_add' ) ) {
			return;
		}
		...
		wp_script_add_data( 'thewebs-pro-woocommerce', 'async', true );
		wp_script_add_data( 'thewebs-pro-woocommerce', 'precache', true );
	...
	public function action_single_product_sticky_enqueue_scripts() {
		...
		// If not enabled then bail.
		if ( ! thewebs()->option( 'product_sticky_add_to_cart' ) ) {
			return;
		}
		if ( function_exists( 'is_product' ) && is_product() ) {
			// Enqueue the sticky script.
			wp_enqueue_style( 'thewebs-sticky-add-to-cart', KTP_URL . 'dist/woocommerce-addons/thewebs-sticky-add-to-cart.css', array(), KTP_VERSION );
			wp_enqueue_script(
				'thewebs-sticky-add-to-cart',
				KTP_URL . 'dist/woocommerce-addons/thewebs-sticky-add-to-cart.min.js',
				array(),
				KTP_VERSION,
				true
			);
	...
	public function action_enqueue_mini_cart_scripts() {
		...
		wp_enqueue_style( 'thewebs-min-cart-shipping-notice', KTP_URL . 'dist/woocommerce-addons/mini-cart-notice.css', array(), KTP_VERSION );
	}
	/**
	...
	public function action_single_product_enqueue_scripts() {
		...
		// If not enabled then bail.
		if ( ! thewebs()->option( 'ajax_add_single_products' ) ) {
			return;
		}
		if ( function_exists( 'is_product' ) && is_product() ) {
			global $post;
			$product = wc_get_product( $post->ID );
			if ( ( $product->is_type( 'simple' ) || $product->is_type( 'external' ) || $product->is_type( 'subscription' ) || $product->is_type( 'variable' ) || $product->is_type( 'variable-subscription' ) ) && apply_filters( 'thewebs_enable_single_ajax_add_to_cart', true, $product ) ) {
				// Enqueue the ajax-add script.
				wp_enqueue_script(
					'thewebs-single-ajax-add',
	...
	public function add_customizer_sections( $sections ) {
		$sections['cart_behavior'] = array(
			'title'    => __( 'Add to Cart Behavior', 'thewebs-pro' ),
			'panel'    => 'woocommerce',
			'priority' => 24,
		);
		return $sections;
	...
	public function archive_template_loader( $template ) {
		...
		if ( thewebs()->option( 'product_archive_shop_custom' ) ) {
