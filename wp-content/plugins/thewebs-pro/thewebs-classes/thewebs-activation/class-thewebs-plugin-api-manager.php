...
if ( ! class_exists( 'Thewebs_Plugin_API_Manager' ) ) {
	/**
	 * Class to check for license
	 *
	 * @category Class
	 */
	class Thewebs_Plugin_API_Manager {
		private $api_url             = 'https://www.thewebswp.com/';
		 ...
		 * @var url
		 */
		private $fallback_api_url    = 'https://www.thewebsthemes.com/';
		private $renewal_url         = 'https://www.thewebswp.com/my-account/';
		private $admin_page_id       = 'thewebs_plugin_activation';
		private $admin_page_name     = 'KT Plugin Activation';
		private $admin_page_title    = 'Thewebs License Activation';
			...
			if ( ! empty( $inactive_plugins ) ) {
				if ( self::$multisite && is_multisite() ) {
					if ( current_user_can( 'manage_network_options' ) ) {
						...
						echo '<p>' . __( 'The following plugins have not been activated: ', 'thewebs-plugin-api-manager' ) . implode( ', ', $inactive_plugins ) . '. <a href="' . esc_url( network_admin_url( 'settings.php?page=' . esc_attr( $this->admin_page_id ) ) ) . '">' . __( 'Click here to activate.', 'thewebs-plugin-api-manager' ) . '</a></p>';
						echo '</div>';
					}

				} else {
					echo '<div class="error">';
					echo '<p>' . __( 'The following plugins have not been activated: ', 'thewebs-plugin-api-manager' ) . implode( ', ', $inactive_plugins ) . '. <a href="' . esc_url( admin_url( 'options-general.php?page=' . esc_attr( $this->admin_page_id ) ) ) . '">' . __( 'Click here to activate.', 'thewebs-plugin-api-manager' ) . '</a></p>';
					echo '</div>';
				}
			}
		}
		...
		public function load_scripts() {
			wp_enqueue_style( $this->admin_page_id . '-css', plugin_dir_url(__FILE__) . '/thewebs-api-manage.css', array(), $this->version, 'all' );
		}
		...
		public function check_external_blocking() {
			...
			if ( defined( 'WP_HTTP_BLOCK_EXTERNAL' ) && WP_HTTP_BLOCK_EXTERNAL === true ) {
				...
				if ( ! defined( 'WP_ACCESSIBLE_HOSTS' ) || stristr( WP_ACCESSIBLE_HOSTS, $host ) === false ) {
					?>
					<div class="error">
						<p><?php printf( __( '<b>Warning!</b> You\'re blocking external requests which means you won\'t be able to get certain plugin updates. Please add %s to %s.', 'thewebs-plugin-api-manager' ), '<strong>' . $host . '</strong>', '<code>WP_ACCESSIBLE_HOSTS</code>' ); ?></p>
					</div>
					<?php
				}
			}
		...
		public function config_page() {
			...
			<div class="wrap kt_theme_license">
				...
				<div class="kt_title_area">
					<h1>
						<?php echo __( 'Thewebs Plugin Activation.', 'thewebs-plugin-api-manager' ); ?>
					</h1>
					<h5>
					<?php printf( __( 'Activating your license allows for plugin updates. If you need your api key you will find it by logging in to your %s Thewebs WP account%s.', 'thewebs-plugin-api-manager' ), '<a href="https://www.thewebswp.com/my-account/" target="_blank">', '</a>' );
					?>
					</h5>
				</div>
				<?php if ( isset( $_GET['kt_updated'] ) && 'true' === $_GET['kt_updated'] ) : ?>
					<div id="message" class="updated notice is-dismissible"><p><?php _e( 'Activated', 'thewebs-plugin-api-manager' ); ?></p></div>
				<?php elseif ( isset( $_GET['kt_updated'] ) && 'false' === $_GET['kt_updated'] ) : ?>
					<div id="message" class="updated error"><p><?php _e( 'Could Not Activate', 'thewebs-plugin-api-manager'); ?></p></div>
				<?php elseif ( isset( $_GET['kt_deactivate'] ) && 'true' === $_GET['kt_deactivate'] ) : ?>
					<div id="message" class="updated notice is-dismissible"><p><?php _e( 'Deactivated', 'thewebs-plugin-api-manager' ); ?></p></div>
				<?php elseif ( isset( $_GET['kt_deactivate'] ) && 'false' === $_GET['kt_deactivate'] ) : ?>
					<div id="message" class="updated error"><p><?php _e( 'Could Not Deactivate, Try again.', 'thewebs-plugin-api-manager'); ?></p></div>
				<?php endif; ?>
						...
						<div class="kt-main">
							...
							foreach ( self::$products as $p_key => $p_values ) {
								...
								if ( $membership_active ) {
									echo '<h2>' . __( 'Thewebs Membership activated through theme', 'thewebs-plugin-api-manager' ) . '</h2>';
									echo '<h3 class="kt-primary-color">' . __( ' Status: Active', 'thewebs-plugin-api-manager' ) . '</h3>';
								} else {
									if ( self::$multisite && is_multisite() ) {
										echo '<form action="edit.php?action=kt_activate_update_network_options" method="post">';
											...
											if ( empty( $data['api_key'] ) ) {
												submit_button( __( 'Activate', 'thewebs-plugin-api-manager' ) );
											}
										echo '</form>';
									} else {
									...
									if ( ! empty( $data['api_key'] ) ) {
										if ( self::$multisite && is_multisite() ) {
											echo '<form action="edit.php?action=kt_deactivate_update_network_options" method="post" class="kt-deactivation-form">';
												...
												submit_button( __( 'Deactivate', 'thewebs-plugin-api-manager' ), 'kt-deactivation-submit' );
											echo '</form>';
										} else {
		...
		public function load_settings() {
			foreach ( self::$products as $p_key => $p_values ) {
				...
				add_settings_section( $p_values['product_key'], __( 'API License Activation', 'thewebs-plugin-api-manager' ), array( $this, 'api_key_text' ), $p_values['product_id'] );

				add_settings_field( $p_values['product_key'], __( 'API License Key', 'thewebs-plugin-api-manager' ), array( $this, 'api_key_field' ), $p_values['product_id'], $p_values['product_key'], array( 'p_key' => $p_key ) );
				add_settings_field( 'activation_email', __( 'API License Email', 'thewebs-plugin-api-manager' ), array( $this, 'api_email_field' ), $p_values['product_id'], $p_values['product_key'], array( 'p_key' => $p_key ) );
				...
				add_settings_field( 'deactivate_button', __( 'Check box to deactivate API License key', 'thewebs-plugin-api-manager' ), array( $this, 'deactivate_inputs' ), $p_values['product_id'] . '_deactivation', 'deactivate_button', array( 'p_key' => $p_key, 'class' => 'kt-plugin-deactivation' ));
			}
		...
		public function validate_options( $input ) {
			...
			if ( $_REQUEST['option_page'] == self::$products[ $current_product_id ]['product_data_key'] ) {
				if ( empty( $current_key ) ) {
					add_settings_error( 'api_error_missing', 'api_key_error',  __( 'Missing API Key, PLease add. ', 'thewebs-plugin-api-manager' ), 'error' );
					return false;
				}
				if ( empty( $current_email ) ) {
					add_settings_error( 'api_error_missing', 'api_email_error',  __( 'Missing API Email, PLease add. ', 'thewebs-plugin-api-manager' ), 'error' );
					return false;
				}
				$args = array(
					'email'       => $current_email,
				...
				if ( isset( $activate_results['activated'] ) && $activate_results['activated'] === true ) {
					...
					add_settings_error( 'activate_text', 'activate_msg', __( 'Plugin activated.', 'thewebs-plugin-api-manager' ), 'updated' );
				...
				if ( $activate_results == false ) {
					add_settings_error( 'api_key_check_text', 'api_key_check_error', __( 'Connection failed to the License Key API server. Make sure your host servers php version has the curl module installed and enabled.', 'thewebs-plugin-api-manager' ), 'error' );
		...
		public function key_deactivation( $input ) {
			...
			if ( $_REQUEST['option_page'] == self::$products[$current_product_id]['product_data_key'] . '_deactivation' ) {
				...
				if ( ! isset( $current_checkbox ) || empty( $current_checkbox ) ) {
					add_settings_error( 'thewebs_deactivate_needs_check', 'deactivate_msg', __( 'Please check box to deactivate. ', 'thewebs-plugin-api-manager' ), 'error' );
					return false;
				}
				...
				if ( isset( $activate_results['deactivated'] ) && $activate_results['deactivated'] == true ) {
					...
					add_settings_error( 'thewebs_deactivate_text', 'deactivate_msg', __( 'License deactivated.', 'thewebs-plugin-api-manager' ), 'updated' );
		...
		public function activate( $args ) {
			...
			if ( 'thewebs_gutenberg_pro' === $args['product_id'] ) {
				if ( 'vps' == $license ) {
					$args['product_id'] = 'vps';
				} else if ( 'pps' == $license ) {
					$args['product_id'] = 'pps';
...
$kt_plugin_api = Thewebs_Plugin_API_Manager::get_instance();
$kt_plugin_api->add_product( 'thewebs_pro_activation', 'kt_api_manager_thewebs_pro_data', 'thewebs_pro', 'Thewebs Pro' );
