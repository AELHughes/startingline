...
if ( !class_exists('Thewebs_Puc_StateStore', false) ):

	class Thewebs_Puc_StateStore {
		/**
		...
		/**
		 * @var Thewebs_Puc_Update|null Cached update.
		 */
		...
		/**
		 * @return null|Thewebs_Puc_Update
		 */
		 ...
		 * @return $this
		 */
		public function setUpdate(Thewebs_Puc_Update $update = null) {
			$this->lazyLoad();
			$this->update = $update;
			return $this;
		}
		...
		public function save() {
			...
			if ( isset($this->update)) {
				...
				if ( Thewebs_Puc_Utils::startsWith($updateClass, $prefix) ) {
		...
		protected function load() {
			...
			$this->lastCheck = intval(Thewebs_Puc_Utils::get($state, 'lastCheck', 0));
			$this->checkedVersion = Thewebs_Puc_Utils::get($state, 'checkedVersion', '');
			$this->update = null;
