<?php
if ( !class_exists('Thewebs_Puc_Plugin_Package', false) ):

	class Thewebs_Puc_Plugin_Package extends Thewebs_Puc_InstalledPackage {
		/**
		 * @var Thewebs_Puc_Plugin_UpdateChecker
		 */
		...
		public function isMuPlugin() {
			...
			if ( $cachedResult === null ) {
				...
				if (($muPluginDir === false) || ($pluginPath === false)) {
					$muPluginDir = Thewebs_Update_Checker::normalizePath(WPMU_PLUGIN_DIR);
					$pluginPath  = Thewebs_Update_Checker::normalizePath($this->pluginAbsolutePath);
				}
