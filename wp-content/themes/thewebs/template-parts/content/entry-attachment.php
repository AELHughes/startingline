...
<article <?php post_class( 'entry content-bg loop-entry' ); ?>>
	<?php
		...
		$feature_element = thewebs()->option( $slug . '_archive_element_feature', $defaults );
		if ( isset( $feature_element ) && is_array( $feature_element ) && true === $feature_element['enabled'] ) {
			...
			if ( isset( $feature_element['imageLink'] ) && ! $feature_element['imageLink'] ) {
				?>
				<div class="post-thumbnail thewebs-thumbnail-ratio-<?php echo esc_attr( $ratio ); ?>">
				...
				?>
				<a class="post-thumbnail thewebs-thumbnail-ratio-<?php echo esc_attr( $ratio ); ?>" href="<?php the_permalink(); ?>">
					<div class="post-thumbnail-inner">
						<?php
						echo wp_get_attachment_image(
							get_the_ID(),
	...
	<div class="entry-content-wrap">
		 ...
		 *
		 * @hooked Thewebs\loop_entry_header - 10
		 * @hooked Thewebs\loop_entry_summary - 20
		 * @hooked Thewebs\loop_entry_footer - 30
		 */
		do_action( 'thewebs_loop_entry_content' );
		?>
	</div>
</article>
...
if ( is_singular( get_post_type() ) ) {
	...
	if ( ! empty( $post->post_parent ) ) {
		...
		if ( ! empty( $prev_link ) ) {
			...
			$attachment_navigation .= '<div class="post-navigation-sub"><span>' . esc_html__( 'Previous:', 'thewebs' ) . '</span></div>';
		...
		if ( ! empty( $next_link ) ) {
			...
			$attachment_navigation .= '<div class="post-navigation-sub"><span>' . esc_html__( 'Next:', 'thewebs' ) . '</span></div>';
			$attachment_navigation .= $next_link;
			$attachment_navigation .= '</div>';
		}

		if ( ! empty( $attachment_navigation ) ) {
			echo _navigation_markup( $attachment_navigation, $class = 'post-navigation', __( 'Post navigation', 'thewebs' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
	}
