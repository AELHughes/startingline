	...
	window.thewebs = {
		...
		initNavToggleSubmenus: function() {
			...
			for ( let i = 0; i < navTOGGLE.length; i++ ) {
				window.thewebs.initEachNavToggleSubmenu( navTOGGLE[ i ] );
				window.thewebs.initEachNavToggleSubmenuInside( navTOGGLE[ i ] );
			}
		},
		initEachNavToggleSubmenu: function( nav ) {
			...
			for ( let i = 0; i < SUBMENUS.length; i++ ) {
				...
				if ( dropdown ) {
					...
					dropdownBtn.setAttribute( 'aria-label', ( dropdown_label ? thewebsConfig.screenReader.expandOf + ' ' + dropdown_label : thewebsConfig.screenReader.expand ) );
					...
					dropdownBtn.addEventListener( 'click', function( e ) {
						e.preventDefault();
						window.thewebs.toggleSubMenu( e.target.closest('li') );
					} );
		
					// Clean up the toggle if a mouse takes over from keyboard.
					parentMenuItem.addEventListener( 'mouseleave', function( e ) {
						window.thewebs.toggleSubMenu( e.target, false );
					} );
					...
					parentMenuItem.querySelector( 'a' ).addEventListener( 'focus', function( e ) {
						...
						for ( let j = 0; j < parentMenuItemsToggled.length; j++ ) {
							window.thewebs.toggleSubMenu( parentMenuItemsToggled[ j ], false );
						}
					} );
					...
					SUBMENUS[ i ].addEventListener( 'keydown', function( e ) {
						...
						if ( 9 === e.keyCode ) {
							if ( e.shiftKey ) {
								...
								if ( window.thewebs.isfirstFocusableElement (SUBMENUS[ i ], document.activeElement, focusSelector ) ) {
									window.thewebs.toggleSubMenu( SUBMENUS[ i ].parentNode, false );
								}
								// Means we're tabbing out of the end of the submenu.
							} else if ( window.thewebs.islastFocusableElement( SUBMENUS[ i ], document.activeElement, focusSelector ) ) {
								window.thewebs.toggleSubMenu( SUBMENUS[ i ].parentNode, false );
							}
						}
			...
			for ( let i = 0; i < SUBMENUPARENTS.length; i++ ) {		
				...
				SUBMENUPARENTS[ i ].addEventListener( 'mouseenter', function( e ) {
					if ( SUBMENUPARENTS[ i ].querySelector( 'ul.sub-menu' ) ) {
						...
						var off = window.thewebs.getOffset( elm );
						var l = off.left;
		...
		toggleSubMenu: function( parentMenuItem, forceToggle ) {
			...
			*/
			if ( parentMenuItemToggled ) {
				...
				toggleButton.setAttribute( 'aria-label', ( dropdown_label ? thewebsConfig.screenReader.expandOf + ' ' + dropdown_label : thewebsConfig.screenReader.expand ) );
				...
				for ( let i = 0; i < subMenuItemsToggled.length; i++ ) {
					window.thewebs.toggleSubMenu( subMenuItemsToggled[ i ], false );
				}
			} else {
				...
				for ( let i = 0; i < parentMenuItemsToggled.length; i++ ) {
					window.thewebs.toggleSubMenu( parentMenuItemsToggled[ i ], false );
				}

				// Toggle "on" the submenu.
				parentMenuItem.classList.add( 'menu-item--toggled-on' );
				subMenu.classList.add( 'toggle-show' );
				toggleButton.setAttribute( 'aria-label', ( dropdown_label ? thewebsConfig.screenReader.collapseOf + ' ' + dropdown_label : thewebsConfig.screenReader.collapse ) );
			}
		},
		/**
		...
		toggleDrawer: function( element, changeFocus ) {
			...
			window.thewebs.toggleAttribute( toggle, 'aria-expanded', 'true', 'false' );
			if ( target.classList.contains('show-drawer') ) {
				...
				document.body.classList.remove( 'thewebs-scrollbar-fixer' );
				setTimeout(function () {
					...
					if ( toggle.dataset.setFocus && changeFocus ) {
						...
						if ( focusElement ) {
							...
							if ( focusElement.hasAttribute( 'aria-expanded') ) {
								window.thewebs.toggleAttribute( focusElement, 'aria-expanded', 'true', 'false' );
							}
						}
					}
				}, duration);
				...
				if ( toggle.dataset.toggleBodyClass ) {
					...
					if ( toggle.dataset.toggleBodyClass.includes( 'showing-popup-drawer-' ) ) {
						...
						document.body.classList.add( 'thewebs-scrollbar-fixer' );
					}
				}
				setTimeout(function () {
		...
		initToggleDrawer: function() {
			...
			for ( let i = 0; i < drawerTOGGLE.length; i++ ) {
				drawerTOGGLE[ i ].addEventListener('click', function( event ) {
					...
					window.thewebs.toggleDrawer( drawerTOGGLE[ i ] );
				} );
			}
				...
				if ( event.keyCode === 27 ) {
					if ( document.querySelectorAll( '.popup-drawer.show-drawer.active' ) ) {
						...
						document.querySelectorAll( '.popup-drawer.show-drawer.active' ).forEach(function ( element ) {
							window.thewebs.toggleDrawer( document.querySelector('*[data-toggle-target="' + element.dataset.drawerTargetString + '"]' ) );
						} );
					}
				}
			  });
			...
			document.addEventListener( 'click', function (event) {
				...
				if ( target === modal ) {
					window.thewebs.toggleDrawer(document.querySelector('*[data-toggle-target="' + modal.dataset.drawerTargetString + '"]'));
				}
		...
		initMobileToggleSub: function() {
			...
			modalMenus.forEach( function( modalMenu ) {
				...
				if ( activeMenuItem ) {
					window.thewebs.findParents( activeMenuItem, 'li' ).forEach( function( element ) {
						var subMenuToggle = element.querySelector( '.drawer-sub-toggle' );
						if ( subMenuToggle ) {
							window.thewebs.toggleDrawer( subMenuToggle, true );
						}
					} );
				}
			} );
			...
			for ( let i = 0; i < drawerSubTOGGLE.length; i++ ) {
				drawerSubTOGGLE[ i ].addEventListener('click', function (event) {
					event.preventDefault();
					window.thewebs.toggleDrawer( drawerSubTOGGLE[ i ] );
				} );
			}
		},
		/**
		...
		initMobileToggleAnchor: function() {
			...
			for ( let i = 0; i < menuLink.length; i++ ) {
				menuLink[ i ].addEventListener('click', function (event) {
					window.thewebs.toggleDrawer( mobileModal.querySelector( '.menu-toggle-close' ), false );
				} );
			}
		},
		/**
		...
		initTransHeaderPadding: function() {
			...
			var updateHeroPadding = function( e ) {
				header
				if ( thewebsConfig.breakPoints.desktop <= window.innerWidth ) {
		...
		initStickyHeader: function() {
			var desktopSticky = document.querySelector( '#main-header .thewebs-sticky-header' ),
				mobileSticky  = document.querySelector( '#mobile-header .thewebs-sticky-header' ),
				wrapper = document.getElementById( 'wrapper' ),
				proSticky = document.querySelectorAll( '.thewebs-pro-fixed-above' ),
				proElements = document.querySelectorAll( '.thewebs-before-wrapper-item' ),
				activeSize = 'mobile',
				lastScrollTop = 0,
				activeOffsetTop = 0;
				if ( parseInt( thewebsConfig.breakPoints.desktop ) < window.innerWidth ) {
					activeSize = 'desktop';
					if ( desktopSticky ) {
						desktopSticky.style.position = 'static';
						activeOffsetTop = window.thewebs.getOffset( desktopSticky ).top;
						desktopSticky.style.position = null;
					}
				} else {
					if ( mobileSticky ) {
						mobileSticky.style.position = 'static';
						activeOffsetTop = window.thewebs.getOffset( mobileSticky ).top;
						mobileSticky.style.position = null;
					}
				}
				var updateSticky = function( e ) {
					var activeHeader;
					var offsetTop = window.thewebs.getOffset( wrapper ).top;
					if ( document.body.classList.toString().includes( 'boom_bar-static-top' ) ) {
						var boomBar = document.querySelector( '.boom_bar' );
						offsetTop = window.thewebs.getOffset( wrapper ).top - boomBar.offsetHeight;
					}
					if ( proElements.length ) {
						...
						offsetTop = window.thewebs.getOffset( wrapper ).top - proElementOffset;
					}
					if ( proSticky.length ) {
						var proOffset = 0;
						...
						offsetTop = window.thewebs.getOffset( wrapper ).top + proOffset;
					}
					if ( thewebsConfig.breakPoints.desktop <= window.innerWidth ) {
					...
					if ( thewebsConfig.breakPoints.desktop <= window.innerWidth ) {
						if ( activeSize === 'mobile' ) {
							activeOffsetTop = window.thewebs.getOffset( activeHeader ).top;
							activeSize = 'desktop';
					...
					if ( 'true' === shrink ) {
						...
						if ( shrinkHeight ) {
							...
							var customShrinkLogo = activeHeader.querySelector( '.thewebs-sticky-logo' );
				...
				if ( desktopSticky || mobileSticky ) {
					...
					if ( document.body.classList.contains( 'woocommerce-demo-store' ) && document.body.classList.contains( 'thewebs-store-notice-placement-above' ) ) {
		...
		getTopOffset: function( event = 'scroll' ) {
			if ( event === 'load' ) {
				...
				var desktopSticky = document.querySelector( '#main-header .thewebs-sticky-header:not([data-reveal-scroll-up="true"])' ),
					mobileSticky  = document.querySelector( '#mobile-header .thewebs-sticky-header:not([data-reveal-scroll-up="true"])' );
			}
			...
			return Math.floor( activeScrollOffsetTop + activeScrollAdminOffsetTop + Math.floor( thewebsConfig.scrollOffset ) );
		},
		scrollToElement: function( element, history, event = 'scroll' ) {
			history = (typeof history !== 'undefined') ? history : true;
			var offsetSticky = window.thewebs.getTopOffset( event );
		...
		anchorScrollToCheck: function( e, respond ) {
			...
			e.preventDefault();
			window.thewebs.scrollToElement( targetAnchor );
		},
		/**
		...
		initStickySidebarWidget: function() {
			...
			var offsetSticky = window.thewebs.getTopOffset(),
			...
			if ( window.location.hash != '' ) {
				...
				if ( element ) {
					window.setTimeout( function() {
						window.thewebs.scrollToElement( element, false, 'load' );
					}, 100 );
				}
			}
				...
				try {
					...
					if ( targetURL.pathname === window.location.pathname ) {
						element.addEventListener( 'click', function( e ) {
							window.thewebs.anchorScrollToCheck( e );
						} );
					}
				} catch (error) {
		...
		init: function() {
			window.thewebs.initNavToggleSubmenus();
			window.thewebs.initToggleDrawer();
			window.thewebs.initMobileToggleAnchor();
			window.thewebs.initMobileToggleSub();
			window.thewebs.initOutlineToggle();
			window.thewebs.initStickyHeader();
			window.thewebs.initStickySidebar();
			window.thewebs.initStickySidebarWidget();
			window.thewebs.initTransHeaderPadding();
			window.thewebs.initAnchorScrollTo();
			window.thewebs.initScrollToTop();
		}
	}
	if ( 'loading' === document.readyState ) {
		// The DOM has not yet been loaded.
		document.addEventListener( 'DOMContentLoaded', window.thewebs.init );
	} else {
