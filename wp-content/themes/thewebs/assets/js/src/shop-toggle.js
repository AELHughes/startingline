...
(function() {
	...
	window.thewebsShop = {
		createCookie: function( name, value, length, unit ) {
			...
			document.cookie = thewebsShopConfig.siteSlug + '-' + name+"="+value+expires+"; path=/";
		},
		/**
		...
		getCookie: function ( name ) {
			...
			var parts = value.split("; " + thewebsShopConfig.siteSlug + '-' + name + "=");
		...
		initShopToggle: function() {
			...
			var shopToggle = document.querySelectorAll( '.thewebs-toggle-shop-layout' );
			...
			var shopCookie = window.thewebsShop.getCookie( 'shopLayout' );
			if ( shopCookie && ( 'grid' === shopCookie || 'list' === shopCookie ) ) {
				var startToggle = document.querySelector( '.thewebs-toggle-' + shopCookie );
				window.thewebsShop.toggleShop( startToggle );
			}

			for ( let i = 0; i < shopToggle.length; i++ ) {
				shopToggle[ i ].addEventListener( 'click', function (event) {
					event.preventDefault();
					window.thewebsShop.toggleShop( shopToggle[ i ] );
				} );
			}
		},
		/**
		...
		toggleShop: function( element ) {
			if ( ! element.classList.contains('toggle-active') ) {
				...
				window.thewebsShop.createCookie( 'shopLayout', element.dataset.archiveToggle, 30, 'days' );
			}
			if ( 'grid' === element.dataset.archiveToggle ) {
				var otherToggle = document.querySelector( '.thewebs-toggle-list' );
			} else {
				var otherToggle = document.querySelector( '.thewebs-toggle-grid' );
			}
			...
			for ( let i = 0; i < catToggle.length; i++ ) {
				...
				if ( childUl.length ) {
					for ( let n = 0; n < childUl.length; n++ ) {
						...
						element.setAttribute( 'aria-label', thewebsConfig.screenReader.expand );
						element.classList.add( 'thewebs-cat-toggle-sub' );
						if ( childUl[n].classList.contains('current-cat') || childUl[n].classList.contains('current-cat-parent') ) {
							element.classList.add( 'toggle-active' );
							element.setAttribute( 'aria-label', thewebsConfig.screenReader.collapse );
							childUl[n].classList.add( 'sub-toggle-active' );
						}
						childUl[n].appendChild( element );
						element.addEventListener( 'click', function ( event ) {
							event.preventDefault();
							window.thewebsShop.toggleCatShop( event );
						} );
					}
				}
			}
		...
		init: function() {
			window.thewebsShop.initShopToggle();
			window.thewebsShop.initShopCatToggle();
		}
	}
	if ( 'loading' === document.readyState ) {
		// The DOM has not yet been loaded.
		document.addEventListener( 'DOMContentLoaded', window.thewebsShop.init );
	} else {
		// The DOM has already been loaded.
		window.thewebsShop.init();
	}
})();
