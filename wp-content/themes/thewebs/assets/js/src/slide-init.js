...
(function() {
	...
	window.thewebsSlide = {
		...
		start: function( element ) {
			...
			if ( ! nextLabel ) {
				nextLabel = thewebsSlideConfig.next;
			}
			if ( ! prevLabel ) {
				prevLabel = thewebsSlideConfig.next;
			}
			if ( ! slideLabel ) {
				slideLabel = thewebsSlideConfig.slide;
			}
			...
			var slider = tns( {
				...
				ofLabel: thewebsSlideConfig.of,
				toLabel: thewebsSlideConfig.to,
		...
		initAll: function( element ) {
			document.querySelectorAll( '.thewebs-slide-init' ).forEach(function ( element ) {
				window.thewebsSlide.start( element );
			} );
		},
		// Initiate the menus when the DOM loads.
		init: function() {
			if ( typeof tns == 'function' ) {
				window.thewebsSlide.initAll();
			} else {
				var initLoadDelay = setInterval( function(){ if ( typeof tns == 'function' ) { window.thewebsSlide.initAll(); clearInterval(initLoadDelay); } }, 200 );
			}
		}
	}
	if ( 'loading' === document.readyState ) {
		// The DOM has not yet been loaded.
		document.addEventListener( 'DOMContentLoaded', window.thewebsSlide.init );
	} else {
		// The DOM has already been loaded.
		window.thewebsSlide.init();
	}
})();
