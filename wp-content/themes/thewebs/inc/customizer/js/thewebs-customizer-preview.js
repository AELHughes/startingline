( function( $ ) {
	...
	document.addEventListener('DOMContentLoaded', function () {
		...
		if ( hasSelectiveRefresh ) {
			wp.customize.selectiveRefresh.bind('partial-content-rendered', function (placement) {
				...
				if ( typeof window.thewebs.initStickyHeader !== "undefined" ) { 
	...
	var thewebsCustomizer = {
		live_css_typography : function( key, rules, newValue ) {
			var styleID = 'thewebs-customize-preview-css-' + key,
			    $style = $( '#' + styleID ),
				css = '',
	...
	if ( thewebsCustomizerPreviewData && thewebsCustomizerPreviewData.liveControl ) {
		_.each( thewebsCustomizerPreviewData.liveControl, function( liveControl, key ) {
			var eachLiveControl = {};
			...
			wp.customize( key, function( value ) {
				value.bind( function( newValue ) {
					_.each( eachLiveControl, function( rule, type ) {
						...
						thewebsCustomizer[ functionName ]( key, rule, newValue );
					});
				});
			});
		});
