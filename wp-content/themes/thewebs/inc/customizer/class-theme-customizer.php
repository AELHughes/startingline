...
use Thewebs_Control_Builder;
use Thewebs_Control_Builder_Tabs;
use Customizer_Sanitize;
use WP_Customize_Control;
use LearnDash_Settings_Section;
use Thewebs\WebFont_Loader;
...
class Theme_Customizer {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_thewebs_flush_fonts_folder', array( $this, 'ajax_delete_fonts_folder' ) );
	...
	public function convert_beaver_custom_fonts() {
		$beaver_fonts = apply_filters( 'thewebs_theme_add_custom_fonts', array() );
		if ( ! empty( $beaver_fonts ) && is_array( $beaver_fonts ) ) {
			add_filter(
				'thewebs_theme_custom_fonts',
	...
	public function ajax_delete_fonts_folder() {
		// Check request.
		if ( ! check_ajax_referer( 'thewebs-local-fonts-flush', 'nonce', false ) ) {
			wp_send_json_error( 'invalid_nonce' );
		}
		if ( ! current_user_can( 'edit_theme_options' ) ) {
			wp_send_json_error( 'invalid_permissions' );
		}
		if ( class_exists( '\Thewebs\WebFont_Loader' ) ) {
			$font_loader = new \Thewebs\WebFont_Loader( '' );
	...
	public function add_woocommerce_choices() {
		if ( class_exists( 'woocommerce' ) ) {
			...
			$options['cart'] = array(
				'name'    => esc_html__( 'Cart', 'thewebs' ),
				'section' => 'thewebs_customizer_cart',
			);
			...
			$options['mobile-cart'] = array(
				'name'    => esc_html__( 'Cart', 'thewebs' ),
				'section' => 'thewebs_customizer_mobile_cart',
			);
	...
	public function customizer_custom_widget_areas( $section_args, $section_id, $sidebar_id ) {
		if ( 'footer1' === $sidebar_id || 'footer2' === $sidebar_id || 'footer3' === $sidebar_id || 'footer4' === $sidebar_id || 'footer5' === $sidebar_id || 'footer6' === $sidebar_id ) {
			$section_args['panel'] = 'thewebs_customizer_footer';
		}
		return $section_args;
	}
	/**
	...
	public function override_defaults( $wp_customize ) {
		 ...
		 * Move Site Identity
		 */
		$wp_customize->get_control( 'site_icon' )->section = 'thewebs_customizer_site_identity';
		...
		if ( isset( $wp_customize->selective_refresh ) ) {
			...
			$wp_customize->selective_refresh->add_partial(
				...
				array(
					...
					'render_callback'     => 'Thewebs\site_branding',
				)
			);
		}

		if ( ! empty( $opt_name_key = apply_filters( 'thewebs_settings_key_custom_mapping', '', 'custom_logo' ) ) ) {
			...
			$wp_customize->add_setting(
				...
				array(
					...
					'default'           => isset( $setting['default'] ) && $control_class !== 'Thewebs_Control_Background' ? $setting['default'] : '',
					'sanitize_callback' => array( 'Thewebs\Customizer_Sanitize', 'thewebs_sanitize_option' ),
				)
			);
		...
		self::$live_control['thewebs_global_palette'] = $palette_live;
	...
	public function register_controls( $wp_customize ) {
		...
		require_once $path . 'class-thewebs-control-blank.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-thewebs-control-color.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-thewebs-control-range.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-thewebs-control-switch.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-thewebs-control-radio-icon.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-thewebs-control-multi-radio-icon.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-thewebs-control-builder.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-thewebs-control-available.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-thewebs-control-color-palette.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-thewebs-control-background.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-thewebs-control-border.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-thewebs-control-borders.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-thewebs-control-typography.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-thewebs-control-title.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-thewebs-control-focus-button.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-thewebs-control-color-link.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-thewebs-control-text.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-thewebs-control-measure.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-thewebs-control-editor.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-thewebs-control-sorter.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-thewebs-control-social.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-thewebs-control-contact.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-thewebs-control-check-icon.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-thewebs-control-select.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-thewebs-control-row.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-thewebs-control-tab.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		require_once $react_path . 'class-thewebs-control-shadow.php'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
	}
	...
	public static function get_panels() {
		...
		if ( is_null( self::$panels ) ) {
			$panels = array(
				'design' => array(
					'title'    => __( 'Colors & Fonts', 'thewebs' ),
					'priority' => 18,
				),
				'header' => array(
					'title'    => __( 'Header', 'thewebs' ),
					'priority' => 20,
				),
				'footer' => array(
					'title'    => __( 'Footer', 'thewebs' ),
					'priority' => 22,
				),
				'general' => array(
					'title'    => __( 'General', 'thewebs' ),
					'priority' => 23,
				),
				'all_posts' => array(
					'title'    => __( 'Posts/Pages Layout', 'thewebs' ),
					'priority' => 24,
				),
			);
			if ( class_exists( 'SFWD_LMS' ) ) {
				$extra_learn['learndash'] = array(
					'title'    => __( 'LearnDash', 'thewebs' ),
					'priority' => 25,
				);
				$panels = array_merge(
					$panels,
	...
	public static function get_sections() {
		...
		if ( is_null( self::$sections ) ) {
			$sections = array(
				'site_identity' => array(
					'title'    => __( 'Site Identity', 'thewebs' ),
					'priority' => 25,
				),
				'general_layout' => array(
					'title'    => __( 'Layout', 'thewebs' ),
					'panel'    => 'general',
					'priority' => 7,
				),
				'sidebar' => array(
					'title'    => __( 'Sidebar', 'thewebs' ),
					'panel'    => 'general',
					'priority' => 8,
				),
				'sidebar_design' => array(
					'title'    => __( 'Sidebar Design', 'thewebs' ),
					'panel'    => 'general',
					'priority' => 8,
				),
				'general_colors' => array(
					'title'    => __( 'Colors', 'thewebs' ),
					'panel'    => 'design',
					'priority' => 9,
				),
				'general_buttons' => array(
					'title'    => __( 'Buttons', 'thewebs' ),
					'panel'    => 'design',
					'priority' => 10,
				),
				...
				'general_image' => array(
					'title'    => __( 'Images', 'thewebs' ),
					'panel'    => 'general',
					'priority' => 11,
				),
				'scroll_up' => array(
					'title'    => __( 'Scroll To Top', 'thewebs' ),
					'panel'    => 'general',
					'priority' => 12,
				),
				...
				'general_social' => array(
					'title'    => __( 'Social Links', 'thewebs' ),
					'panel'    => 'general',
					'priority' => 20,
				),
				'general_comments' => array(
					'title'    => __( 'Comments', 'thewebs' ),
					'panel'    => 'general',
					'priority' => 20,
				),
				'general_performance' => array(
					'title'    => __( 'Performance', 'thewebs' ),
					'panel'    => 'general',
					'priority' => 21,
				),
				'breadcrumbs' => array(
					'title'    => __( 'Breadcrumbs', 'thewebs' ),
					'panel'    => 'general',
					'priority' => 20,
				),
				'general_404' => array(
					'title'    => __( '404 Page Layout', 'thewebs' ),
					'panel'    => 'general',
					'priority' => 20,
				),
				...
				'header_builder' => array(
					'title'    => __( 'Header Builder', 'thewebs' ),
					'panel'    => 'header',
					'priority' => 9,
				),
				'header_top' => array(
					'title'    => __( 'Top Row', 'thewebs' ),
					'panel'    => 'header',
					'priority' => 10,
				),
				'header_main' => array(
					'title'    => __( 'Main Row', 'thewebs' ),
					'panel'    => 'header',
					'priority' => 10,
				),
				'header_bottom' => array(
					'title'    => __( 'Bottom Row', 'thewebs' ),
					'panel'    => 'header',
					'priority' => 10,
				),
				'header_popup' => array(
					'title'    => __( 'Header Off Canvas', 'thewebs' ),
					'panel'    => 'header',
					'priority' => 10,
				),
				...
				'primary_navigation' => array(
					'title'    => __( 'Primary Navigation', 'thewebs' ),
					'panel'    => 'header',
					'priority' => 20,
				),
				'secondary_navigation' => array(
					'title'    => __( 'Secondary Navigation', 'thewebs' ),
					'panel'    => 'header',
					'priority' => 20,
				),
				'dropdown_navigation' => array(
					'title'    => __( 'Dropdown Navigation', 'thewebs' ),
					'panel'    => 'header',
					'priority' => 20,
				),
				...
				'mobile_trigger' => array(
					'title'    => __( 'Mobile Trigger', 'thewebs' ),
					'panel'    => 'header',
					'priority' => 20,
				),
				...
				'mobile_navigation' => array(
					'title'    => __( 'Mobile Navigation', 'thewebs' ),
					'panel'    => 'header',
					'priority' => 20,
				),
				'mobile_button' => array(
					'title'    => __( 'Header Mobile Button', 'thewebs' ),
					'panel'    => 'header',
					'priority' => 20,
				),
				...
				'mobile_html' => array(
					'title'    => __( 'Header Mobile HTML', 'thewebs' ),
					'panel'    => 'header',
					'priority' => 20,
				),
				'header_html' => array(
					'title'    => __( 'HTML Editor', 'thewebs' ),
					'panel'    => 'header',
					'priority' => 20,
				),
				...
				'cart' => array(
					'title'    => __( 'Header Cart', 'thewebs' ),
					'panel'    => 'header',
					'priority' => 20,
				),
				'cart_design' => array(
					'title'    => __( 'Header Cart Design', 'thewebs' ),
					'panel'    => 'header',
					'priority' => 20,
				),
				'mobile_cart' => array(
					'title'    => __( 'Mobile Header Cart', 'thewebs' ),
					'panel'    => 'header',
					'priority' => 20,
				),
				...
				'header_button' => array(
					'title'    => __( 'Header Button', 'thewebs' ),
					'panel'    => 'header',
					'priority' => 20,
				),
				...
				'header_social' => array(
					'title'    => __( 'Header Social', 'thewebs' ),
					'panel'    => 'header',
					'priority' => 20,
				),
				...
				'header_search' => array(
					'title'    => __( 'Header Search', 'thewebs' ),
					'panel'    => 'header',
					'priority' => 20,
				),
				'transparent_header' => array(
					'title'    => __( 'Transparent Header', 'thewebs' ),
					'panel'    => 'header',
					'priority' => 20,
				),
				...
				'header_sticky' => array(
					'title'    => __( 'Sticky Header', 'thewebs' ),
					'panel'    => 'header',
					'priority' => 20,
				),
				...
				'footer_builder' => array(
					'title'    => __( 'Footer Builder', 'thewebs' ),
					'panel'    => 'footer',
					'priority' => 9,
				),
				'footer_top' => array(
					'title'    => __( 'Footer Top', 'thewebs' ),
					'panel'    => 'footer',
					'priority' => 10,
				),
				...
				'footer_middle' => array(
					'title'    => __( 'Footer Middle', 'thewebs' ),
					'panel'    => 'footer',
					'priority' => 10,
				),
				...
				'footer_bottom' => array(
					'title'    => __( 'Footer Bottom', 'thewebs' ),
					'panel'    => 'footer',
					'priority' => 10,
				),
				...
				'footer_html' => array(
					'title'    => __( 'Footer Copyright HTML', 'thewebs' ),
					'panel'    => 'footer',
					'priority' => 10,
				),
				'footer_social' => array(
					'title'    => __( 'Footer Social', 'thewebs' ),
					'panel'    => 'footer',
					'priority' => 10,
				),
				...
				'footer_navigation' => array(
					'title'    => __( 'Footer Navigation', 'thewebs' ),
					'panel'    => 'footer',
					'priority' => 10,
				),
				'page_layout' => array(
				...
				'search' => array(
					'title'    => __( 'Search Results', 'thewebs' ),
					'priority' => 24,
				),
				'search_design' => array(
					'title'    => __( 'Search Results', 'thewebs' ),
				...
				'post_layout' => array(
					'title'    => __( 'Single Post Layout', 'thewebs' ),
					'panel'    => 'all_posts',
					'priority' => 7,
				),
				...
				'post_archive' => array(
					'title'    => __( 'Archive Layout', 'thewebs' ),
					'panel'    => 'all_posts',
					'priority' => 8,
				),
				...
				// 'custom_post' => array(
				// 	'title'    => __( 'Custom Post Types', 'thewebs' ),
				// 	'panel'    => 'all_posts',
				// 	'priority' => 24,
				// ),
			);
			if ( ( defined( 'GUTENBERG_VERSION' ) && version_compare( GUTENBERG_VERSION, '10.6.2', '>' ) ) || version_compare( substr( get_bloginfo( 'version' ), 0, 3 ), '5.8', '>=' ) ) {
				$blocks_extra['sidebar-widgets-footer1'] = array(
					'title'    => __( 'Widget Footer 1 Settings', 'thewebs' ),
					'panel'    => 'footer',
					'priority' => 10,
				);
				$blocks_extra['sidebar-widgets-footer2'] = array(
					'title'    => __( 'Widget Footer 2 Settings', 'thewebs' ),
					'panel'    => 'footer',
					'priority' => 10,
				);
				$blocks_extra['sidebar-widgets-footer3'] = array(
					'title'    => __( 'Widget Footer 3 Settings', 'thewebs' ),
					'panel'    => 'footer',
					'priority' => 10,
				);
				$blocks_extra['sidebar-widgets-footer4'] = array(
					'title'    => __( 'Widget Footer 4 Settings', 'thewebs' ),
					'panel'    => 'footer',
					'priority' => 10,
				);
				$blocks_extra['sidebar-widgets-footer5'] = array(
					'title'    => __( 'Widget Footer 5 Settings', 'thewebs' ),
					'panel'    => 'footer',
					'priority' => 10,
				);
				$blocks_extra['sidebar-widgets-footer6'] = array(
					'title'    => __( 'Widget Footer 6 Settings', 'thewebs' ),
					'panel'    => 'footer',
					'priority' => 10,
				);
				$blocks_extra['sidebar-widgets-header1'] = array(
					'title'    => __( 'Widget Header Settings', 'thewebs' ),
					'panel'    => 'header',
					'priority' => 10,
				);
				$blocks_extra['sidebar-widgets-header2'] = array(
					'title'    => __( 'Widget Area Settings', 'thewebs' ),
					'panel'    => 'header',
					'priority' => 10,
				);
				$blocks_extra['sidebar-widgets-product-filter'] = array(
					'title'    => __( 'Catalog Off Canvas Sidebar Settings', 'thewebs' ),
					'panel'    => 'woocommerce',
					'priority' => 18,
				);
			...
			if ( class_exists( 'woocommerce' ) ) {
				$extra_woo['woocommerce_product_catalog'] = array(
					'title'    => __( 'Product Catalog', 'thewebs' ),
					'panel'    => 'woocommerce',
					'priority' => 10,
				);
				...
				$extra_woo['product_layout'] = array(
					'title'    => __( 'Single Product Layout', 'thewebs' ),
					'panel'    => 'woocommerce',
					'priority' => 10,
				);
				...
				$extra_woo['account_layout'] = array(
					'title'    => __( 'My Account Layout', 'thewebs' ),
					'panel'    => 'woocommerce',
					'priority' => 24,
				);
				$extra_woo['woocommerce_store_notice_design'] = array(
					'title'    => __( 'Store Notice', 'thewebs' ),
					'panel'    => 'woocommerce',
					'priority' => 10,
				);
				$sections = array_merge(
			...
			if ( class_exists( 'SFWD_LMS' ) ) {
				...
				$extra_learn['sfwd_courses_layout'] = array(
					'title'    => __( 'Course Layout', 'thewebs' ),
					'panel'    => 'learndash',
					'priority' => 10,
				);
				...
				$extra_learn['sfwd_courses_archive_layout'] = array(
					'title'    => __( 'Course Archive Layout', 'thewebs' ),
					'panel'    => 'learndash',
					'priority' => 11,
				);
				...
				$extra_learn['sfwd_lesson_layout'] = array(
					'title'    => __( 'Lesson Layout', 'thewebs' ),
					'panel'    => 'learndash',
					'priority' => 12,
				);
				...
				$extra_learn['sfwd_groups_layout'] = array(
					'title'    => __( 'Groups Layout', 'thewebs' ),
					'panel'    => 'learndash',
					'priority' => 11,
				);
				...
				$extra_learn['sfwd_design'] = array(
					'title'    => __( 'Learn Dash Design', 'thewebs' ),
					'panel'    => 'learndash',
					'priority' => 12,
				);
				$extra_learn['sfwd_grid_layout'] = array(
					'title'    => __( 'Course Grid Design', 'thewebs' ),
					'panel'    => 'learndash',
					'priority' => 12,
				);
			...
			if ( class_exists( 'BBPress' ) ) {
				...
				$extra_bbpress['forum_archive'] = array(
					'title'    => __( 'Forum Archive', 'thewebs' ),
					'panel'    => 'bbpress',
					'priority' => 10,
				);
			...
			if ( class_exists( 'LifterLMS' ) ) {
				...
				$extra_lifter['course_archive'] = array(
					'title'    => __( 'Course Archive', 'thewebs' ),
					'panel'    => 'lifterlms',
					'priority' => 11,
				);
				...
				$extra_lifter['llms_membership_layout'] = array(
					'title'    => __( 'Membership Layout', 'thewebs' ),
					'panel'    => 'lifterlms',
					'priority' => 14,
				);
				$extra_lifter['llms_membership_archive'] = array(
					'title'    => __( 'Membership Archive', 'thewebs' ),
					'panel'    => 'lifterlms',
					'priority' => 15,
				);
				...
				$extra_lifter['llms_dashboard_layout'] = array(
					'title'    => __( 'User Dashboard', 'thewebs' ),
					'panel'    => 'lifterlms',
					'priority' => 16,
				);
				$sections = array_merge(
			...
			if ( defined( 'TRIBE_EVENTS_FILE' ) ) {
				$extra_events['tribe_events_layout'] = array(
					'title'    => __( 'Single Event Layout', 'thewebs' ),
				...
				$extra_events['tribe_events_archive'] = array(
					'title'    => __( 'Events Layout', 'thewebs' ),
					'panel'    => 'tribe_customizer',
					'priority' => 85,
				);
				$sections = array_merge(
			...
			$post_types        = thewebs()->get_post_types_objects();
			...
			foreach ( $post_types as $post_type_item ) {
				...
				if ( ! in_array( $post_type_name, $ignore_type, true ) ) {
					...
					$extras_post_types[ $post_type_name . '_layout' ] = array(
						'title'    => $post_type_label . ' ' . __( 'Layout', 'thewebs' ),
						'panel'    => $panel_name,
					...
					if ( $post_type_name !== 'ld-exam' ) {
						$extras_post_types[ $post_type_name . '_archive' ] = array(
							'title'    => $post_type_label . ' ' . __( 'Archive', 'thewebs' ),
							'panel'    => $panel_name,
							'priority' => $panel_priority,
						);
			...
			self::$sections = apply_filters( 'thewebs_theme_customizer_sections', $final_sections );
		}
		// Return sections.
	...
	public static function get_capability() {
		...
		if ( is_null( self::$capability ) ) {
			self::$capability = apply_filters( 'thewebs_theme_customizer_capability', 'manage_options' );
		}
		// Return capability.
	...
	public function register_settings( $wp_customize ) {
		...
		foreach ( self::$settings as $setting_key => $setting ) {
			...
			if ( isset( $setting['control_type'] ) && ! empty( $setting['control_type'] ) ) {
				if ( substr( $setting['control_type'], 0, 8 ) === 'thewebs_' ) {
					$control_class = 'Thewebs_Control_' . ucfirst( str_replace( 'thewebs_', '', str_replace( '_control', '', $setting['control_type'] ) ) );
				} else {
			...
			if ( isset( $setting['sanitize'] ) && ! empty( $setting['sanitize'] ) ) {
				if ( substr( $setting['sanitize'], 0, 8 ) === 'thewebs_' ) {
					$sanitize_function = array( 'Thewebs\Customizer_Sanitize', $setting['sanitize'] );
				} else {
					$sanitize_function = $setting['sanitize'];
				}
			} else {
				$sanitize_function = array( 'Thewebs\Customizer_Sanitize', 'thewebs_sanitize_option' );
				$sanitize_function = '';
			}
			if ( 'option' === thewebs()->get_option_type() ) {
				$setting_key = thewebs()->get_option_name() . '[' . $setting_key . ']';
			}
			if ( ! empty( $opt_name_key = apply_filters( 'thewebs_settings_key_custom_mapping', '', $setting_key ) ) ) {
			...
			if ( ! in_array( $section_key, self::$sections_added, true ) ) {
				...
				if ( ( defined( 'GUTENBERG_VERSION' ) && version_compare( GUTENBERG_VERSION, '10.6.2', '>' ) ) || version_compare( substr( get_bloginfo( 'version' ), 0, 3 ), '5.8', '>=' ) ) {
					if ( 'sidebar-widgets-footer1' === $section_key || 'sidebar-widgets-footer2' === $section_key || 'sidebar-widgets-footer3' === $section_key || 'sidebar-widgets-footer4' === $section_key || 'sidebar-widgets-footer5' === $section_key || 'sidebar-widgets-footer6' === $section_key || 'sidebar-widgets-header1' === $section_key || 'sidebar-widgets-header2' === $section_key || 'sidebar-widgets-product-filter' === $section_key ) {
						$section_id = 'thewebs_customizer_' . $section_key;
					}
				}
				// Register section.
	...
	public static function maybe_add_panel( $wp_customize, $child ) {
		...
		if ( ! empty( $child['panel'] ) && isset( $panels[ $child['panel'] ] ) ) {
			...
			if ( ! in_array( $panel_key, self::$panels_added, true ) ) {
				...
				// Register panel.
				$wp_customize->add_panel( 'thewebs_customizer_' . $panel_key, $panel_config );
		...
		foreach ( self::$settings as $setting_key => $setting ) {
			...
			if ( ! empty( $setting['section'] ) ) {
				...
				if ( ( defined( 'GUTENBERG_VERSION' ) && version_compare( GUTENBERG_VERSION, '10.6.2', '>' ) ) || version_compare( substr( get_bloginfo( 'version' ), 0, 3 ), '5.8', '>=' ) ) {
					if ( 'sidebar-widgets-footer1' === $setting['section'] || 'sidebar-widgets-footer2' === $setting['section'] || 'sidebar-widgets-footer3' === $setting['section'] || 'sidebar-widgets-footer4' === $setting['section'] || 'sidebar-widgets-footer5' === $setting['section'] || 'sidebar-widgets-footer6' === $setting['section'] || 'sidebar-widgets-header1' === $setting['section'] || 'sidebar-widgets-header2' === $setting['section'] || 'sidebar-widgets-product-filter' === $setting['section'] ) {
						$control_config['section'] = 'thewebs_customizer_' . $setting['section'];
					}
				}
			}
			...
			if ( ! empty( $setting['panel'] ) ) {
				if ( 'woocommerce' === $control_config['panel'] ) {
					...
					$control_config['panel'] = 'thewebs_customizer_' . $setting['panel'];
				}
			}
	...
	public static function active_callback( $object ) {
		if ( ! isset( $object->setting->id ) && 'thewebs_builder_control' !== $object->type ) {
			return true;
		}
		if ( 'thewebs_builder_control' === $object->type ) {
			$id = str_replace( thewebs()->get_option_name() . '_', '', $object->id );
		} else {
		...
		if ( ! isset( $settings[ $id ]['active_callback'][0] ) && 3 === count( $settings[ $id ]['active_callback'] ) ) {
			...
			$field_value = thewebs()->option( $field_id );
	...
	public static function get_builder_choices() {
		return apply_filters( 'thewebs_theme_customizer_control_choices', self::$choices );
	}
	/**
	...
	public function enqueue_customizer_scripts() {
		...
		if ( isset( $_GET['wpum_email_customizer'] ) || apply_filters( 'prevent_thewebs_customizer_scripts', false ) ) {
			return;
		}
		...
		wp_enqueue_style( 'thewebs-customizer-styles', $path . 'css/thewebs-customizer.css', false, THEWEBS_VERSION );
		if ( is_rtl() ) {
			wp_enqueue_style( 'thewebs-customizer-rtl', $path . 'css/rtl-thewebs-customizer.css', false, THEWEBS_VERSION );
		}
		...
		wp_enqueue_script( 'customizer-text-widgets', get_template_directory_uri() . '/assets/js/text-widgets.min.js', array( 'jquery', 'backbone', 'editor', 'wp-util', 'wp-a11y', 'text-widgets' ), THEWEBS_VERSION, false );
		wp_enqueue_script( 'thewebs-customizer-controls', get_template_directory_uri() . '/assets/js/admin/customizer.js', $editor_dependencies, THEWEBS_VERSION, true );
		wp_enqueue_style( 'thewebs-customizer-controls', $path . 'react/build/controls.css', array( 'wp-components' ), THEWEBS_VERSION );
		wp_localize_script(
			'thewebs-customizer-controls',
			'thewebsCustomizerControlsData',
			array(
				'contexts'           => self::get_control_contexts(),
				'choices'            => self::get_builder_choices(),
				'palette'            => thewebs()->get_palette_for_customizer(),
				'gfontvars'          => $google_font_variants,
				'cfontvars'          => apply_filters( 'thewebs_theme_custom_fonts', array() ),
				'palettePresets'     => apply_filters( 'thewebs_theme_palette_presets', $palette_presets ),
				'flushFonts'         => wp_create_nonce( 'thewebs-local-fonts-flush' ),
				'fontPairs'          => apply_filters( 'thewebs_font_pairs_array', $fonts ),
				'source'             => apply_filters( 'thewebs_settings_extra_source', false ),
		...
		$css = ':root {
			--global-palette1: ' . thewebs()->palette_option( 'palette1' ) . ';
			--global-palette2: ' . thewebs()->palette_option( 'palette2' ) . ';
			--global-palette3: ' . thewebs()->palette_option( 'palette3' ) . ';
			--global-palette4: ' . thewebs()->palette_option( 'palette4' ) . ';
			--global-palette5: ' . thewebs()->palette_option( 'palette5' ) . ';
			--global-palette6: ' . thewebs()->palette_option( 'palette6' ) . ';
			--global-palette7: ' . thewebs()->palette_option( 'palette7' ) . ';
			--global-palette8: ' . thewebs()->palette_option( 'palette8' ) . ';
			--global-palette9: ' . thewebs()->palette_option( 'palette9' ) . ';
			--global-palette-highlight: ' . $this->render_color( thewebs()->sub_option( 'link_color', 'highlight' ) ) . ';
			--global-palette-highlight-alt: ' . $this->render_color( thewebs()->sub_option( 'link_color', 'highlight-alt' ) ) . ';
			--global-palette-highlight-alt2: ' . $this->render_color( thewebs()->sub_option( 'link_color', 'highlight-alt2' ) ) . ';
			--global-palette-btn: ' . $this->render_color( thewebs()->sub_option( 'buttons_color', 'color' ) ) . ';
			--global-palette-btn-hover: ' . $this->render_color( thewebs()->sub_option( 'buttons_color', 'hover' ) ) . ';
			--global-palette-btn-bg: ' . $this->render_color( thewebs()->sub_option( 'buttons_background', 'color' ) ) . ';
			--global-palette-btn-bg-hover: ' . $this->render_color( thewebs()->sub_option( 'buttons_background', 'hover' ) ) . ';
			--global-base-font: ' . thewebs()->sub_option( 'base_font', 'family' ) . ';
			--global-heading-font: ' . ( 'inherit' !== thewebs()->sub_option( 'heading_font', 'family' ) ? thewebs()->sub_option( 'heading_font', 'family' ) : 'var(--global-base-font)' ) . ';
		}';
		wp_add_inline_style( 'thewebs-customizer-controls', wp_strip_all_tags( $css ) );
		if ( function_exists( 'wp_set_script_translations' ) ) {
			wp_set_script_translations( 'thewebs-customizer-controls', 'thewebs' );
		}
	}
	/**
	...
	public function action_enqueue_customize_preview_scripts() {
		$path = get_template_directory_uri() . '/inc/customizer/';
		wp_enqueue_style( 'thewebs-customizer-preview', $path . 'css/thewebs-customizer-preview.css', false, THEWEBS_VERSION );
		wp_enqueue_script( 'thewebs-webfont-js', 'https://ajax.googleapis.com/ajax/libs/webfont/1.6.26/webfont.js', array( 'jquery' ), '1.6.26', true );
		wp_enqueue_script( 'thewebs-customizer-preview', $path . 'js/thewebs-customizer-preview.min.js', array( 'customize-preview' ), THEWEBS_VERSION, true );
		wp_localize_script(
			'thewebs-customizer-preview',
			'thewebsCustomizerPreviewData',
			array(
	...
	public static function get_control_contexts() {
		// Return contexts.
		return apply_filters( 'thewebs_theme_customizer_control_contexts', self::$contexts );
	}
	...
	public static function get_live_control_data() {
		// Return contexts.
		return apply_filters( 'thewebs_theme_customizer_live_control_data', self::$live_control );
	}
