...
class ColorComponent extends Component {
	constructor(props) {
		...
		let baseDefault =  ( thewebsCustomizerControlsData.palette ? JSON.parse( thewebsCustomizerControlsData.palette ) : { palette: [] } );
		...
		let defaultParams = {
			...
			palettes: ( thewebsCustomizerControlsData.palettePresets ? thewebsCustomizerControlsData.palettePresets : [] ),
			colors: {
				palette1: {
					tooltip: __( '1 - Accent', 'thewebs' ),
					palette: false,
				},
				palette2: {
					tooltip: __( '2 - Accent - alt', 'thewebs' ),
					palette: false,
				},
				palette3: {
					tooltip: __( '3 - Strongest text', 'thewebs' ),
					palette: false,
				},
				palette4: {
					tooltip: __( '4 - Strong Text', 'thewebs' ),
					palette: false,
				},
				palette5: {
					tooltip: __( '5 - Medium text', 'thewebs' ),
					palette: false,
				},
				palette6: {
					tooltip: __( '6 - Subtle Text', 'thewebs' ),
					palette: false,
				},
				palette7: {
					tooltip: __( '7 - Subtle Background', 'thewebs' ),
					palette: false,
				},
				palette8: {
					tooltip: __( '8 - Lighter Background', 'thewebs' ),
					palette: false,
				},
				palette9: {
					tooltip: __( '9 - White or offwhite', 'thewebs' ),
					palette: false,
				},
			},
			paletteMap: {
				'palette': {
					'tooltip': __( 'Palette 1', 'thewebs' ),
				},
				'second-palette': {
					'tooltip': __( 'Palette 2', 'thewebs' ),
				},
				'third-palette': {
					'tooltip': __( 'Palette 3', 'thewebs' ),
				}
			}
		};
	...
	render() {
		...
		return (
				<div className="thewebs-control-field thewebs-palette-control thewebs-color-control">
					<div className="thewebs-palette-header">
					...
					<div ref={ this.anchorNodeRef } className="thewebs-palette-colors">
					...
					<div className={'thewebs-palette-import-wrap'}>
						<Button className={ 'thewebs-palette-import' } onClick={ () => { this.state.isVisible ? toggleClose() : toggleVisible() } }>
							<Dashicon icon="portfolio" />
						</Button>
						{ this.state.isVisible && (
							<Popover position="bottom right" className="thewebs-palette-popover-copy-paste thewebs-customizer-popover" onClose={ toggleClose }>
								<TabPanel className="thewebs-palette-popover-tabs"
									activeClass="active-tab"
									initialTabName={ 'import'}
									tabs={ [
										{
											name: 'import',
											title: __( 'Select Palette', 'thewebs' ),
											className: 'thewebs-color-presets',
										},
										{
											name: 'custom',
											title: __( 'Export/Import', 'thewebs' ),
											className: 'thewebs-export-import',
										}
									] }>
									{
										( tab ) => {
													...
													tabout = (
														<Fragment>
															{ Object.keys( presetPalettes ).map( ( item, index ) => {
																return (
																	<Button
																		className={ 'thewebs-palette-item' }
																		style={ {
																			height: '100%',
																			width: '100%',
																		} }
																	...
																	>
																		{ Object.keys( presetPalettes[item] ).map( ( color, subIndex ) => {
																			return (
																				...
																				} } className="thewebs-swatche-item-wrap">
																					<span
																						className={ 'thewebs-swatch-item' }
																						style={ {
																							height: '100%',
																							display: 'block',
																							width: '100%',
													...
													tabout = (
														<Fragment>
															...
															{ this.state.importError && (
																<p style={{color:'red'}}>{ __( 'Error with Import data', 'thewebs') }</p>
															) }
															<Button
																className={ 'thewebs-import-button' }
																isPrimary
					...
					{ this.props.control.params.description && (
						<span className="customize-control-description"><a href="https://thewebs-theme.com/docs/how-to-use-the-thewebs-color-palette/" >{ this.props.control.params.description }</a></span>
					) }
				</div>
		);
	}
	...
	updateValues( value ) {
		...
		thewebsCustomizerControlsData.palette = JSON.stringify( value );
	}
}
