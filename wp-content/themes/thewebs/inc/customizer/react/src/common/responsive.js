...
class ResponsiveControl extends Component {
	...
	render() {
		...
		deviceMap = {
			'desktop': {
				'tooltip': __( 'Desktop', 'thewebs' ),
				'icon': <Icon icon={ Icons.desktop }/>
			},
			'tablet': {
				'tooltip': __( 'Tablet', 'thewebs' ),
				'icon': <Icon icon={ Icons.tablet }/>
			},
			'mobile': {
				'tooltip': __( 'Mobile', 'thewebs' ),
														...
														<Button
																...
																onClick={() => {
																	let event = new CustomEvent(
																			'thewebsChangedRepsonsivePreview', {
																				'detail': device
																			} );
	...
	linkResponsiveButtons() {
		let self = this;
		document.addEventListener( 'thewebsChangedRepsonsivePreview', function(e) {
