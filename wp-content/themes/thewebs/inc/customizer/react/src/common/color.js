...
import ThewebsColorPicker from './color-picker';
...
class ColorControl extends Component {
	...
	render() {
		...
		return (
			<div className="thewebs-color-picker-wrap">
				...
				{ showingGradient && (
					<Fragment>
						{ this.state.isVisible && (
							<Popover position={ position } anchor={( undefined !== this.props?.controlRef?.current ? this.props.controlRef.current : undefined )} className="thewebs-popover-color thewebs-popover-color-gradient thewebs-customizer-popover" onClose={ toggleClose }>
				...
				{ ! showingGradient && (
					<Fragment>
						{ this.state.isVisible && this.state.refresh && (
							<Popover position="top right" anchor={( undefined !== this.props?.controlRef?.current ? this.props.controlRef.current : undefined )} className="thewebs-popover-color thewebs-customizer-popover" onClose={ toggleClose }>
								<ThewebsColorPicker
									color={ ( this.state.isPalette && this.state.palette.palette && this.state.palette.palette[parseInt(this.state.color.slice(-1), 10 ) - 1] ? this.state.palette.palette[parseInt(this.state.color.slice(-1), 10 ) - 1 ].color : this.state.color ) }
									onChange={ ( color ) => this.onChangeState( color, '' ) }
									onChangeComplete={ ( color ) => this.onChangeComplete( color, '' ) }
						...
						{ this.state.isVisible && ! this.state.refresh &&  (
							<Popover position={ position } anchor={ ( undefined !== this.props?.controlRef?.current ? this.props.controlRef.current : undefined ) } className="thewebs-popover-color thewebs-customizer-popover" onClose={ toggleClose }>
								<ThewebsColorPicker
									//presetColors={ [] }
									color={ ( this.state.isPalette && this.state.palette[ this.state.activePalette ] && this.state.palette[ this.state.activePalette ][parseInt(this.state.color.slice(-1), 10 ) - 1] ? this.state.palette[ this.state.activePalette ][parseInt(this.state.color.slice(-1), 10 ) - 1 ].color : this.state.color ) }
									onChange={ ( color ) => this.onChangeState( color, '' ) }
				...
				<Tooltip text={ this.props.tooltip ? this.props.tooltip : __( 'Select Color', 'thewebs' ) }>
					<div className="color-button-wrap">
						<Button className={ 'thewebs-color-icon-indicate' } onClick={ () => { this.state.isVisible ? toggleClose() : toggleVisible() } }>
							<ColorIndicator className="thewebs-advanced-color-indicate" colorValue={ ( this.state.isPalette ? 'var(--global-' + this.props.color + ')' : this.props.color ) } />
