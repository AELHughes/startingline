if ( thewebsgooglefonts === undefined ) {
	var thewebsgooglefonts = [];
}
const {
	Component,
} = wp.element;
...
class ThewebsWebfontLoader extends Component {
	constructor(props) {
		super( props );
	...
	addFont( font ) {
		if ( ! thewebsgooglefonts.includes( font ) ) {
			thewebsgooglefonts.push( font );
		}
	};
	...
	loadFonts() {
		...
		if ( ! thewebsgooglefonts.includes( this.props.config.google.families[ 0 ] ) ) {
			WebFont.load( {
				...this.props.config,
...
ThewebsWebfontLoader.propTypes = {
	config: PropTypes.object.isRequired,
	children: PropTypes.element,
	onStatus: PropTypes.func.isRequired,
};

ThewebsWebfontLoader.defaultProps = {
	onStatus: noop,
};

export default ThewebsWebfontLoader;
