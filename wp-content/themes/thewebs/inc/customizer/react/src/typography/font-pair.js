...
class FontPairModal extends Component {
	constructor() {
		...
		this.state = {
			isVisible: false,
			confirm: '',
			fonts: ( thewebsCustomizerControlsData.fontPairs ? thewebsCustomizerControlsData.fontPairs : [] ),
		};
	}
	render() {
		const toggleVisible = () => {
		...
		return (
			<div className={'thewebs-font-pair-wrap'}>
				<Button className={ 'thewebs-font-pair-btn' } label={ __( 'Choose a font', 'thewebs' ) } onClick={ () => { this.state.isVisible ? toggleClose() : toggleVisible() } }>
					<Dashicon icon="portfolio" />
				</Button>
				{ this.state.isVisible && (
					<Popover position="bottom left" className="thewebs-font-pair-popover thewebs-customizer-popover" onClose={ toggleClose }>
						<h2 style={{ textAlign:'center' }}>{ __( 'Select a Font Pairing', 'thewebs' ) }</h2>
						<ButtonGroup className="kt-font-pair-group" aria-label={ __( 'Select a Font Pair', 'thewebs' ) }>
							{ map( this.state.fonts, ( { hfont, bfont, hv, img, name } ) => {
								return (
									this.state.confirm === name ? (
										...
										>
											{ __( 'Confirm Change Settings?', 'thewebs' ) }
										</Button>
									) : (
										<Button
	...
	updateValues( bodyFont, headingFont ) {
		...
		var event = new CustomEvent('thewebsRemoteUpdateFonts', {
			'detail': 'typography'
		});
