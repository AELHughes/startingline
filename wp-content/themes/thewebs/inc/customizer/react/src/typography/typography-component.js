...
import ThewebsWebfontLoader from '../common/font-loader.js';
...
class TypographyComponent extends Component {
	constructor() {
		...
		this.state = {
			...
			activeFont: [],
			value: value,
			fontVars: ( thewebsCustomizerControlsData.gfontvars ? thewebsCustomizerControlsData.gfontvars : [] ),
			customFontVars: ( thewebsCustomizerControlsData.cfontvars ? thewebsCustomizerControlsData.cfontvars : [] ),
		};
	...
	render() {
		...
		const fontFamilyTab = (
			<Fragment>
				<div className="thewebs-font-family-search">
					<TextControl
						type="text"
					...
					{ undefined !== this.state.search && '' !== this.state.search && (
						<Button
							className="thewebs-clear-search"
							onClick={ () => {
				...
				<div className="thewebs-font-family-list-wrapper">
					<ButtonGroup id={ this.controlParams.id } className="thewebs-font-family-list" aria-label={ __( 'Font Family List' ) }>
		...
		const fontStyleTab = (
			<Fragment>
				<div className="thewebs-font-variant-list-wrapper">
					<ButtonGroup id={ this.controlParams.id + '-style' } className="thewebs-font-variant-list" aria-label={ __( 'Font Style List' ) }>
		...
		const fontSizeTab = (
			<Fragment>
				<div class="thewebs-range-control">
					<ResponsiveControl
						onChange={ ( currentDevice) => this.setState( { currentDevice } ) }
						controlLabel={ __( 'Font Size', 'thewebs' ) }
						tooltip={ false }
					>
						<RangeControl
							allowReset={ true }
						...
						{ this.controlParams.sizeUnits && (
							<div className="thewebs-select-units">
				...
				<div class="thewebs-range-control">
					<ResponsiveControl
						onChange={ ( currentDevice) => this.setState( { currentDevice } ) }
						controlLabel={ __( 'Line Height', 'thewebs' ) }
						tooltip={ false }
					>
						<RangeControl
							allowReset={ true }
				...
				<div class="thewebs-range-control">
					<ResponsiveControl
						onChange={ ( currentDevice) => this.setState( { currentDevice } ) }
						controlLabel={ __( 'Letter Spacing', 'thewebs' ) }
						tooltip={ false }
					>
						<RangeControl
							allowReset={ true }
				...
				<div class="thewebs-range-control thewebs-transform-controls">
					<span className="customize-control-title">{ __( 'Transform' ) }</span>
					<ButtonGroup className="thewebs-radio-container-control thewebs-radio-icon-container-control">
			...
			<Fragment>
				{
					...
					<span className="customize-control-title">
						<Tooltip text={ __( 'Reset Values', 'thewebs' ) }>
							<Button
								className="reset thewebs-reset"
								onClick={ () => {
		...
		return (
			<div ref={ this.anchorNodeRef } className="thewebs-control-field thewebs-typography-control-wrap">
				<div className="thewebs-typography-control">
					{ controlLabel }
					<div className="thewebs-typography-controls">
						{ this.state.isVisible && 'family' === this.controlParams.options && (
							<Popover position="top right" className="thewebs-popover-color thewebs-popover-typography thewebs-customizer-popover" onClose={ toggleClose }>
								<div className="thewebs-popover-typography-single-item">
									{ fontFamilyTab }
								</div>
							</Popover>
						) }
						{ this.state.isVisible && ( 'all' === this.controlParams.options || 'no-color' === this.controlParams.options ) && (
							<Popover position="top left" className="thewebs-popover-color thewebs-popover-typography thewebs-customizer-popover" onClose={ toggleClose }>
								<TabPanel className="thewebs-popover-tabs thewebs-typography-tabs thewebs-background-tabs"
									...
									tabs={ [
										{
											name: 'font',
											title: __( 'Font', 'thewebs' ),
											className: 'thewebs-font-typography',
										},
										{
											name: 'style',
											title: __( 'Style', 'thewebs' ),
											className: 'thewebs-style-typography',
										},
										{
											name: 'size',
											title: __( 'Size', 'thewebs' ),
											className: 'thewebs-size-typography',
										},
									] }>
									{
										( tab ) => {
						...
						{ ( 'all' === this.controlParams.options || 'family' === this.controlParams.options || 'no-color' === this.controlParams.options ) && (
							<Tooltip text={ this.controlParams.tooltip ? this.controlParams.tooltip : __( 'Select Font', 'thewebs' ) }>
								<div className="typography-button-wrap">
									<Button className={ 'thewebs-typography-family-indicate' } onClick={ () => { this.state.isVisible ? toggleClose() : toggleVisible('font') } }>
						...
						{ ( 'all' === this.controlParams.options || 'no-color' === this.controlParams.options ) && (
							<Tooltip text={ this.controlParams.tooltip ? this.controlParams.tooltip : __( 'Select Style', 'thewebs' ) }>
								<div className="typography-button-wrap">
									<Button className={ 'thewebs-typography-style-indicate' } onClick={ () => { this.state.isVisible ? toggleClose() : toggleVisible('style') } }>
										{ this.state.value.variant ? this.state.value.variant : __( 'inherit', 'thewebs' ) }
									</Button>
								</div>
							</Tooltip>
						) }
						{ ( 'all' === this.controlParams.options || 'size' === this.controlParams.options || 'no-color' === this.controlParams.options ) && (
							<Tooltip text={ this.controlParams.tooltip ? this.controlParams.tooltip : __( 'Select Size', 'thewebs' ) }>
								<div className="typography-button-wrap">
									<Button className={ 'thewebs-typography-size-indicate' } onClick={ () => { this.state.isVisible ? toggleClose() : toggleVisible('size') } }>
										{ currentSize ? currentSize + this.state.value.sizeType : __( 'inherit', 'thewebs' ) }
									</Button>
								</div>
							</Tooltip>
						) }
						{ ( 'all' === this.controlParams.options || 'family' === this.controlParams.options || 'no-color' === this.controlParams.options ) && (
							<Tooltip text={ __( 'Show Preview Text', 'thewebs' ) }>
								<div className="typography-button-wrap">
									<Button className={ 'thewebs-typography-preview-indicate' } onClick={ () => { this.state.isPreviewVisible ? toggleClosePreview() : toggleVisiblePreview() } }>
				...
				{ this.state.value.google && (
					<ThewebsWebfontLoader config={ configVariants } >
					</ThewebsWebfontLoader>
				) }
				{ this.state.isPreviewVisible && (
					<div className="thewebs-preview-font" style={ { 
						...
						{ __( 'Design is not just what it looks like and feels like. Design is how it works.', 'thewebs' ) }
					</div>
				) }
			</div>
		);
	...
	linkRemoteUpdate() {
		if ( 'base_font' === this.controlParams.id || 'heading_font' === this.controlParams.id ) {
			let self = this;
			document.addEventListener( 'thewebsRemoteUpdateFonts', function( e ) {
