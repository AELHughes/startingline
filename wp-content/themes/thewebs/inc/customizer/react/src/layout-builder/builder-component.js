...
class BuilderComponent extends Component {
	...
	focusPanel( item ) {
		if ( undefined !== this.props.customizer.section( 'thewebs_customizer_' + item ) ) {
			this.props.customizer.section( 'thewebs_customizer_' + item ).focus();
		}
	}
	focusItem( item ) {
	...
	render() {
		return (
			<div className={ `thewebs-control-field thewebs-builder-items${ ( this.controlParams.rows.includes( 'popup' ) ? ' thewebs-builder-items-with-popup' : '' ) }` }>
				...
				<div className="thewebs-builder-row-items">
	...
	// linkFocusButtons() {
	// 	this.props.control.container.on( 'click', '.thewebs-builder-areas .thewebs-builder-item', function( e ) {
	// 		e.preventDefault();
