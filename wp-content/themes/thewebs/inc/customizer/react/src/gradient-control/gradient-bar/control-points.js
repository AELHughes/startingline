...
function CustomColorPickerDropdown( {
	...
	return (
		<CustomDropdown
			contentClassName="components-color-palette__custom-color-dropdown-content thewebs-pop-color-popover"
			...
			ignoreMarkerPosition !== initialPosition && (
				<GradientColorPickerDropdown
					...
					renderContent={ ( { onClose } ) => (
						<div className="thewebs-pop-gradient-color-picker">
							<HStack
							...
							{ colors && (
								...
								} } className="thewebs-swatches-wrap">
								{ map( colors, ( { color, slug, name } ) => {
									...
									return (
										<div key={ key } style={ {
											...
											<Button
												className={ `thewebs-swatch-item ${ isActive ? 'swatch-active' : 'swatch-inactive' }` }
												style={ {
													height: '100%',
													width: '100%',
							...
							{ ! disableRemove && controlPoints.length > 2 && (
								...
								>
									<Button
										...
										variant="link"
									>
										{ __( 'Remove Control Point', 'thewebs' ) }
									</Button>
								</HStack>
							) }
						</div>
