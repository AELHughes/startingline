...
import ThewebsGradientPicker from '../gradient-control/index.js';
import ThewebsColorPicker from '../common/color-picker';
...
class BackgroundComponent extends Component {
	constructor(props) {
		...
		let defaultParams = {
			...
			repeat: {
				'no-repeat': {
					name: __( 'No Repeat', 'thewebs' ),
				},
				'repeat': {
					name: __( 'Repeat', 'thewebs' ),
				},
				'repeat-x': {
					name: __( 'Repeat-X', 'thewebs' ),
				},
				'repeat-y': {
					name: __( 'Repeat-y', 'thewebs' ),
				},
			},
			size: {
				auto: {
					name: __( 'Auto', 'thewebs' ),
				},
				cover: {
					name: __( 'Cover', 'thewebs' ),
				},
				contain: {
					name: __( 'Contain', 'thewebs' ),
				},
			},
			attachment: {
				scroll: {
					name: __( 'Scroll', 'thewebs' ),
				},
				fixed: {
					name: __( 'Fixed', 'thewebs' ),
				},
			},
			attachments: {
				desktop: {},
		...
		const palette = JSON.parse( this.props.customizer.control( 'thewebs_color_palette' ).setting.get() );
		//console.log( palette );
		this.state = {
			value: value,
	...
	render() {
		...
		const toggleVisible = () => {
			const updateColors = JSON.parse( this.props.customizer.control( 'thewebs_color_palette' ).setting.get() );
		...
		const controlLabel = (
			<Fragment>
				{ this.state.currentDevice !== 'desktop' && (
					<Tooltip text={ __( 'Reset Device Values', 'thewebs' ) }>
						<Button
		...
		if( this.state.supportGradient ) {
			tab_options = [
				{
					name: 'color',
					title: __( 'Color', 'thewebs' ),
					className: 'thewebs-color-background',
				},
				{
					name: 'gradient',
					title: __( 'Gradient', 'thewebs' ),
					className: 'thewebs-gradient-background',
				},
				{
					name: 'image',
					title: __( 'Image', 'thewebs' ),
					className: 'thewebs-image-background',
				},
			];
		} else {
			tab_options = [
		...
		return (
			<div className="thewebs-control-field thewebs-background-control">
				<ResponsiveControl
					onChange={ ( currentDevice) => this.setState( { currentDevice } ) }
					controlLabel={ controlLabel }
				>
					<div className="thewebs-background-picker-wrap">
						{ this.state.isVisible && (
							<Popover position="top left" className="thewebs-popover-color thewebs-customizer-popover" onClose={ maybeToggleClose }>
								<TabPanel className="thewebs-popover-tabs thewebs-background-tabs"
												...
												if ( 'image' === tab.name ) {
													tabout = (
														<Fragment>
															...
															<span className="customize-control-title">{ __( 'Background Repeat', 'thewebs' ) }</span>
															...
															<span className="customize-control-title">{ __( 'Background Size', 'thewebs' ) }</span>
															<ButtonGroup className="thewebs-radio-container-control">
															...
															<span className="customize-control-title">{ __( 'Background Attachment', 'thewebs' ) }</span>
															<ButtonGroup className="thewebs-radio-container-control">
													...
													tabout = (
														<Fragment>
															{ this.state.isVisible && this.state.refresh && (
																<div className="thewebs-background-color-wrap">
																	<ThewebsColorPicker
																		//presetColors={ [] }
						...
						<Tooltip text={ this.controlParams.tooltip ? this.controlParams.tooltip : __( 'Select Background', 'thewebs' ) }>
							<div className="background-button-wrap">
								<Button className={ 'thewebs-background-icon-indicate' } onClick={ () => { this.state.isVisible ? toggleClose() : toggleVisible() } }>
									{ ( undefined === this.state.value[ this.state.currentDevice ] || undefined === this.state.value[ this.state.currentDevice ].type || this.state.value[ this.state.currentDevice ].type === 'color' ) && (
										<Fragment>
											<ColorIndicator className="thewebs-advanced-color-indicate" colorValue={ getColorPreviewValue( this.state.currentDevice ) } />
											...
											{ showGradientPreview( this.state.currentDevice ) &&
												<ColorIndicator className="thewebs-advanced-color-indicate" colorValue={ getGradientPreviewValue( this.state.currentDevice ) } />
											}
										</Fragment>
									) }
	...
	preventClose() {
		let self = this;
		document.addEventListener( 'thewebsOpenMediaModal', function(e) {
