 ...
 *
 * @author  Carlos Rios - Edited by Ben Ritner for use in Thewebs Theme
 * @package  Thewebs
 * @version  1.1
 */
...
class Thewebs_CSS {
	...
	public function render_font_family( $font, $area = 'headers' ) {
		...
		return apply_filters( 'thewebs_theme_font_family_string', $font_string );
	}
	/**
	...
	public function render_font_no_color( $font, $css, $inherit = null ) {
		...
		if ( ! empty( $family ) ) {
			...
			$css->add_property( 'font-family', apply_filters( 'thewebs_theme_font_family_string', $family ) );
			if ( isset( $font['google'] ) && true === $font['google'] ) {
				if ( ! empty( $inherit ) && 'body' === $inherit ) {
	...
	public function maybe_add_google_variant( $font, $area = null ) {
		...
		if ( ! empty( $area ) && 'headers' === $area ) {
			$parent_font = thewebs()->option( 'heading_font' );
			if ( isset( $parent_font['family'] ) && 'inherit' === $parent_font['family'] ) {
				$parent_font = thewebs()->sub_option( 'base_font' );
	...
	public function maybe_add_google_font( $font, $full = null ) {
		...
		} elseif ( ! empty( $full ) && 'body' === $full && 'inherit' !== thewebs()->sub_option( 'heading_font', 'family' ) ) {
			$new_variant = array( $font['variant'], '700' );
		}
		if ( ! empty( $full ) && 'body' === $full ) {
			if ( thewebs()->option( 'load_base_italic' ) ) {
