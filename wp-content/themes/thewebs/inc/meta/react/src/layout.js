...
class ThewebsThemeLayout extends Component {
	constructor() {
		super( ...arguments );
		this.state = {
		};
	...
	render() {
		const selectOptions = Object.keys( thewebsMetaParams.sidebars ).map( ( item ) => {
			return ( { label: thewebsMetaParams.sidebars[ item ].label, value: thewebsMetaParams.sidebars[ item ].value } );
		} );
		...
		const titleOptions = {
			'default': {
				tooltip: __( 'Inherited Default', 'thewebs' ),
				icon: 'inherit',
			},
			'hide': {
				tooltip: __( 'Hide Title', 'thewebs' ),
				icon: 'hidetitle',
			},
			'normal': {
				tooltip: __( 'Show In Content', 'thewebs' ),
				icon: 'incontent',
			},
			'above': {
				tooltip : __( 'Show Above Content', 'thewebs' ),
				icon: 'abovecontent',
			},
		}
		...
		return (
			<Fragment>
				<PluginSidebarMoreMenuItem
					...
					icon={ icon }
				>
					{ thewebsMetaParams.post_type_name + ' ' + __( 'Settings', 'thewebs' ) }
				</PluginSidebarMoreMenuItem>
				<PluginSidebar
					isPinnable={ true }
					icon={ icon }
					name="theme-meta-panel"
					title={ thewebsMetaParams.post_type_name + ' ' + __( 'Settings', 'thewebs' ) }
				>
					<div className="thewebs-sidebar-container components-panel__body is-opened">
						<RadioIconComponent
						...
						/>
						<RadioIconComponent
							label={ thewebsMetaParams.post_type_name + ' ' + __( 'Title', 'thewebs' ) }
							customClass="three-col-short"
							...
							onChange={ value => {
								let title = value;
								if( 'default' === value ) {
									title = thewebsMetaParams.title;
								} else if ( 'show' === value ) {
									title = thewebsMetaParams.title_position;
								}
						...
						/>
						<RadioIconComponent
							label={ thewebsMetaParams.post_type_name + ' ' + __( 'Layout', 'thewebs' ) }
							...
							onChange={ value => {
								...
								if ( 'left' === value || 'right' === value ) {
									...
									layout = thewebsMetaParams.layout;
									sidebar = thewebsMetaParams.sidebar;
								}
						...
						/>
						{ ( ( undefined !== this.props.meta && undefined !== this.props.meta._kad_post_layout && '' !== this.props.meta._kad_post_layout && ( 'left' === this.props.meta._kad_post_layout || 'right' === this.props.meta._kad_post_layout ) ) || ( undefined !== this.props.meta && undefined !== this.props.meta._kad_post_layout && 'default' === this.props.meta._kad_post_layout && ( 'left' === thewebsMetaParams.sidebar || 'right' === thewebsMetaParams.sidebar ) ) || ( undefined !== this.props.meta && undefined !== this.props.meta._kad_post_layout && '' === this.props.meta._kad_post_layout && ( 'left' === thewebsMetaParams.sidebar || 'right' === thewebsMetaParams.sidebar ) ) ) && (
							<div className="thewebs-control-field thewebs-select-control">
								<div className="thewebs-title-control-bar">
									<span className="customize-control-title">{ capitalizeFirstLetter( thewebsMetaParams.post_type ) + ' ' + __( 'sidebar' )  }</span>
								</div>
								<SelectControl
						...
						<RadioIconComponent
							label={ __( 'Content Style', 'thewebs' ) }
						...
						/>
						<RadioIconComponent
							label={ __( 'Content Vertical Padding', 'thewebs' ) }
							...
							onChange={ value => {
								let padding = value;
								if( 'default' === value ) {
									padding = thewebsMetaParams.vpadding;
								}
						...
						/>
						{ thewebsMetaParams.supports_feature && (
							<Fragment>
								<RadioIconComponent
									label={ __( 'Show Featured Image', 'thewebs' ) }
								...
								/>
								{ ( ( undefined !== this.props.meta && undefined !== this.props.meta._kad_post_feature && 'show' === this.props.meta._kad_post_feature ) || ( undefined !== this.props.meta && undefined !== this.props.meta._kad_post_feature && 'default' === this.props.meta._kad_post_feature && 'show' === thewebsMetaParams.feature ) || ( undefined !== this.props.meta && undefined !== this.props.meta._kad_post_feature && '' === this.props.meta._kad_post_feature && 'show' === thewebsMetaParams.feature ) ) && (
									<RadioIconComponent
										label={ __( 'Featured Image Position', 'thewebs' ) }
						...
						<ToggleControl
							label={ __( 'Disable Header', 'thewebs' ) }
						...
						/>
						<ToggleControl
							label={ __( 'Disable Footer', 'thewebs' ) }
