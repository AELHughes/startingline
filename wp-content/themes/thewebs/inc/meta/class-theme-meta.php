...
class Theme_Meta {
	...
	public function add_metabox() {
		...
		add_meta_box(
			...
			__( 'Post Settings', 'thewebs' ),
			array( $this, 'render_metabox' ),
			apply_filters( 'thewebs_classic_meta_box_post_types', $extras_post_types ),
			'side',
			'low',
			array(
	...
	public function save_metabox( $post_id, $post ) {
		...
		$nonce_action = 'thewebs_theme_classic_meta_nonce_action';

		// Check if nonce is set.
		if ( ! isset( $_POST['thewebs_theme_classic_meta_nonce'] ) ) {
			return;
		}

		// Check if nonce is valid.
		if ( ! wp_verify_nonce( $_POST['thewebs_theme_classic_meta_nonce'], $nonce_action ) ) {
			return;
		}
	...
	public function render_product_metabox( $post ) {
		// Add nonce for security and authentication.
		wp_nonce_field( 'thewebs_theme_classic_meta_nonce_action', 'thewebs_theme_classic_meta_nonce' );
		$output = '<div class="thewebs_classic_meta_boxes">';
		$output .= '<div class="thewebs_classic_meta_boxes" style="padding: 10px 0 0;">';
		$output .= '<div style="padding-bottom:10px;">';
		$output .= '<label for="_kad_post_transparent" style="font-weight: 600;">' . esc_html__( 'Transparent Header', 'thewebs' ) . '</label>';
		$output .= '</div>';
		$output .= '<div>';
		$trans_option_values = array(
			'default' => __( 'Default', 'thewebs' ),
			'enable'  => __( 'Enable', 'thewebs' ),
			'disable' => __( 'Disable', 'thewebs' ),
		);
		...
		$output .= '<div class="thewebs_classic_meta_boxes" style="padding: 10px 0 0;">';
		$output .= '<div style="padding-bottom:10px;">';
		$output .= '<label for="_kad_post_layout" style="font-weight: 600;">' . esc_html__( 'Layout', 'thewebs' ) . '</label>';
		$output .= '</div>';
		$output .= '<div>';
		$layout_option_values = array(
			'default'   => __( 'Default', 'thewebs' ),
			'normal'    => __( 'Normal', 'thewebs' ),
			'narrow'    => __( 'Narrow', 'thewebs' ),
			'fullwidth' => __( 'Fullwidth', 'thewebs' ),
			'left'      => __( 'Sidebar Left', 'thewebs' ),
			'right'     => __( 'Sidebar Right', 'thewebs' ),
		);
		...
		$output .= '<div class="thewebs_classic_meta_boxes" style="padding: 10px 0 0;">';
		$output .= '<div style="padding-bottom:10px;">';
		$output .= '<label for="_kad_post_content_style" style="font-weight: 600;">' . esc_html__( 'Content Style', 'thewebs' ) . '</label>';
		$output .= '</div>';
		$output .= '<div>';
		$content_option_values = array(
			'default' => __( 'Default', 'thewebs' ),
			'boxed'   => __( 'Boxed', 'thewebs' ),
			'unboxed' => __( 'Unboxed', 'thewebs' ),
		);
		...
		$output .= '<div class="thewebs_classic_meta_boxes" style="padding: 10px 0 0;">';
		$output .= '<div style="padding-bottom:10px;">';
		$output .= '<label for="_kad_post_vertical_padding" style="font-weight: 600;">' . esc_html__( 'Content Vertical Padding', 'thewebs' ) . '</label>';
		$output .= '</div>';
		$output .= '<div>';
		$padding_option_values = array(
			'default' => __( 'Default', 'thewebs' ),
			'show'    => __( 'Enable', 'thewebs' ),
			'hide'    => __( 'Disable', 'thewebs' ),
			'top'     => __( 'Top Only', 'thewebs' ),
			'bottom'  => __( 'Bottom Only', 'thewebs' ),
		);
	...
	public function render_metabox( $post ) {
		...
		$option_title_position = thewebs()->option( get_post_type() . '_title_layout' );
		...
		$output .= '<div class="thewebs_classic_meta_boxes" style="padding: 10px 0 0;">';
		$output .= '<div style="padding-bottom:10px;">';
		$output .= '<label for="_kad_post_title" style="font-weight: 600;">' . esc_html__( 'Display Title', 'thewebs' ) . '</label>';
		$output .= '</div>';
		$output .= '<div>';
		$option_values = array(
			'default'       => __( 'Default', 'thewebs' ),
			$title_position => __( 'Enable', 'thewebs' ),
			'hide'          => __( 'Disable', 'thewebs' ),
		);
	...
	public function script_enqueue() {
		...
		$ignore_type       = thewebs()->get_public_post_types_to_ignore();
		...
		$option_title_position = thewebs()->option( $post_type . '_title_layout' );
		if ( 'above' === $option_title_position || 'normal' === $option_title_position ) {
			$title_position = $option_title_position;
		}
		$option_nav = thewebs()->option( $post_type . '_navigation' );
		if ( $option_nav ) {
			$navigation = 'show';
		}
		...
		$option_feature_position = thewebs()->option( $post_type . '_feature_postition' );
		...
		wp_enqueue_style( 'thewebs-meta', $path . 'build/meta-controls.css', false, THEWEBS_VERSION );
		wp_enqueue_script( 'thewebs-meta' );
		wp_localize_script(
			'thewebs-meta',
			'thewebsMetaParams',
			array(
	...
	public function get_sidebar_options() {
		$sidebars = array(
			array( 'value' => 'default', 'label' => __( 'Default', 'thewebs' ) )
		);
		$nonsidebars = array(
			'header1',
			'header2',
	...
	public function plugin_register() {
		...
		if ( function_exists( 'wp_set_script_translations' ) ) {
			wp_set_script_translations( 'thewebs-meta', 'thewebs' );
		}
	}
}
