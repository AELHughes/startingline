<?php
/**
 * Thewebs\Elementor\Component class
 *
 * @package thewebs
 */

namespace Thewebs\Elementor;
...
class Component implements Component_Interface {
	...
	public function add_body_class( $classes ) {
		$classes[] = 'thewebs-elementor-colors';
	...
	public function elementor_after_save( $object, $data ) {
		if ( apply_filters( 'thewebs_add_global_colors_to_elementor', true ) ) {
			// Prevent Errors.
			if ( ! current_user_can( 'edit_theme_options' ) ) {
				return;
			}
			if ( $data && isset( $data['settings'] ) && is_array( $data['settings'] ) && isset( $data['settings']['thewebs_colors'] ) && is_array( $data['settings']['thewebs_colors'] ) ) {
				$update_palette = false;
				$palette = json_decode( thewebs()->get_palette(), true );
				if ( isset( $palette['active'] ) && ! empty( $palette['active'] ) ) {
					$active = $palette['active'];
				} else {
					$palette = json_decode( thewebs()->get_default_palette(), true );
					$active  = $palette['active'];
				}
				foreach ( $data['settings']['thewebs_colors'] as $key => $value ) {
				...
				if ( $update_palette ) {
					update_option( 'thewebs_global_palette', json_encode( $palette ) );
				}
			}
		}
	}
	...
	public function add_styles() {
		wp_enqueue_style( 'thewebs-elementor', get_theme_file_uri( '/assets/css/elementor.min.css' ), array(), THEWEBS_VERSION );
	}
	/**
	...
	public function elementor_add_scripts() {
		if ( apply_filters( 'thewebs_add_global_colors_to_elementor', true ) ) {
			wp_enqueue_style( 'thewebs-elementor-admin', get_theme_file_uri( '/assets/css/elementor-admin.min.css' ), array(), THEWEBS_VERSION );
		}
	}
	/**
		...
		if ( apply_filters( 'thewebs_add_global_colors_to_elementor', true ) ) {
			...
			if ( $current && isset( $current['custom_colors'] ) ) {
				...
				foreach ( $custom_colors as $key => $value ) {
					if ( is_array( $value ) && isset( $value['_id'] ) && in_array( $value['_id'], $thewebs ) ) {
						...
						if ( $value['_id'] == 'thewebs1' ) {
							...
							$thewebs_add_array['thewebs1'] = false;
						...
						if ( $value['_id'] == 'thewebs2' ) {
							...
							$thewebs_add_array['thewebs2'] = false;
						...
						if ( $value['_id'] == 'thewebs3' ) {
							...
							$thewebs_add_array['thewebs3'] = false;
						...
						if ( $value['_id'] == 'thewebs4' ) {
							...
							$thewebs_add_array['thewebs4'] = false;
						...
						if ( $value['_id'] == 'thewebs5' ) {
							...
							$thewebs_add_array['thewebs5'] = false;
						...
						if ( $value['_id'] == 'thewebs6' ) {
							...
							$thewebs_add_array['thewebs6'] = false;
						...
						if ( $value['_id'] == 'thewebs7' ) {
							...
							$thewebs_add_array['thewebs7'] = false;
						...
						if ( $value['_id'] == 'thewebs8' ) {
							...
							$thewebs_add_array['thewebs8'] = false;
						...
						if ( $value['_id'] == 'thewebs9' ) {
							...
							$thewebs_add_array['thewebs9'] = false;
				...
				if ( $thewebs_add ) {
					...
					$i       = 0;
					$new_add = array();
					foreach ( $thewebs_add_array as $key => $value ) {
						if ( $value ) {
				...
				\Elementor\Plugin::$instance->kits_manager->update_kit_settings_based_on_option( 'custom_colors', $custom_colors );
				\Elementor\Plugin::$instance->kits_manager->update_kit_settings_based_on_option( 'thewebs_colors', $theme_placeholder_colors );
				// Refresh cache.
				if ( $clear_cache ) {
	...
	public function elementor_add_theme_color_controls( $tab, $args ) {
		if ( apply_filters( 'thewebs_add_global_colors_to_elementor', true ) ) {
			$tab->start_controls_section(
				...
				array(
					'label' => __( 'Theme Global Colors', 'thewebs' ),
					'tab' => 'global-colors',
				)
			);
	...
	public function elementor_page_meta_setting() {
		if ( ! apply_filters( 'thewebs_theme_elementor_default', true ) || 'post' === get_post_type() ) {
			return;
		}
