<?php
/**
 * Thewebs\LifterLMS\Component class
 *
 * @package thewebs
 */

namespace Thewebs\LifterLMS;
...
class Component implements Component_Interface {
	...
	public function dashboard_wrapper_open() {
		echo '<div class="thewebs-llms-dash-wrap thewebs-llms-dash-nav-' . esc_attr( thewebs()->option( 'llms_dashboard_navigation_layout' ) ) . '">';
	}
	/**
	...
	public function lifter_syllabus_thumbnails( $show ) {
		if ( thewebs()->option( 'course_syllabus_thumbs' ) ) {
			$show = true;
		} else {
			$show = false;
		}
	...
	public function set_lifter_columns( $columns ) {
		...
		if ( get_the_ID() === $dash_id ) {
			$columns = absint( thewebs()->option( 'llms_dashboard_archive_columns' ) );
		} elseif ( is_archive() ) {
			if ( is_post_type_archive( 'course' ) || is_tax( 'course_cat' ) || is_tax( 'course_tag' ) || is_tax( 'course_track' ) ) {
				$columns = absint( thewebs()->option( 'course_archive_columns' ) );
			} elseif ( is_post_type_archive( 'llms_membership' ) || is_tax( 'membership_cat' ) || is_tax( 'membership_tag' ) ) {
				$columns = absint( thewebs()->option( 'llms_membership_archive_columns' ) );
			}
		}
		return $columns;
	}
	...
	public function lifterlms_styles() {
		wp_enqueue_style( 'thewebs-lifterlms', get_theme_file_uri( '/assets/css/lifterlms.min.css' ), array(), THEWEBS_VERSION );
	}
