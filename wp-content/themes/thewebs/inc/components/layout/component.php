<?php
/**
 * Thewebs\Layout\Component class
 *
 * @package thewebs
 */

namespace Thewebs\Layout;
 ...
 * * `thewebs()->get_layout()`
 * * `thewebs()->get_feature()`
 * * `thewebs()->show_feature()`
 * * `thewebs()->show_feature_above()`
 * * `thewebs()->show_feature_below()`
 * * `thewebs()->get_feature_position()`
 * * `thewebs()->show_comments()`
 * * `thewebs()->show_hero_title()`
 * * `thewebs()->show_in_content_title()`
 * * `thewebs()->get_boxed()`
 * * `thewebs()->has_sidebar()`
 * * `thewebs()->sidebar_id()`
 * * `thewebs()->sidebar_id_class()`
 * * `thewebs()->sidebar_side()`
...
class Component implements Component_Interface, Templating_Component_Interface {
	...
	public function action_register_sidebars() {
		$widgets = array(
			'sidebar-primary'   => __( 'Sidebar 1', 'thewebs' ),
			'sidebar-secondary' => __( 'Sidebar 2', 'thewebs' ),
			'footer1'           => __( 'Footer 1', 'thewebs' ),
			'footer2'           => __( 'Footer 2', 'thewebs' ),
			'footer3'           => __( 'Footer 3', 'thewebs' ),
			'footer4'           => __( 'Footer 4', 'thewebs' ),
			'footer5'           => __( 'Footer 5', 'thewebs' ),
			'footer6'           => __( 'Footer 6', 'thewebs' ),
		);

		foreach ( $widgets as $id => $name ) {
			register_sidebar(
				apply_filters(
					'thewebs_widget_area_args',
					array(
						'name'          => $name,
						'id'            => $id,
						'description'   => esc_html__( 'Add widgets here.', 'thewebs' ),
	...
	public function display_sidebar() {
		...
		echo apply_filters( 'thewebs_dynamic_sidebar_content', ob_get_clean() ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
	/**
	...
	public static function show_hero_title() {
		if ( is_null( self::$layout ) ) {
			self::$layout = apply_filters( 'thewebs_post_layout', self::check_conditionals() );
		}
	...
	public static function desk_transparent_header() {
		...
		return ( 'enable' === self::$layout['transparent'] && thewebs()->sub_option( 'transparent_header_device', 'desktop' ) ? true : false );
	}
	/**
	 * Checks if page should have desktop transparent header.
	 *
	...
	public static function get_desk_transparent_class() {
		...
		return ( 'enable' === self::$layout['transparent'] && thewebs()->sub_option( 'transparent_header_device', 'desktop' ) ? 'transparent-header' : 'non-transparent-header' );
	}
	/**
	...
	public static function get_mobile_transparent_class() {
		...
		return ( 'enable' === self::$layout['transparent'] && thewebs()->sub_option( 'transparent_header_device', 'mobile' ) ? 'mobile-transparent-header' : 'mobile-non-transparent-header' );
	}
	/**
	 * Checks if page should have mobile transparent header.
	 *
	...
	public static function mobile_transparent_header() {
		...
		return ( 'enable' === self::$layout['transparent'] && thewebs()->sub_option( 'transparent_header_device', 'mobile' ) ? true : false );
	}
	/**
	...
	public static function check_conditionals() {
		...
		$footer      = 'enable';
		$content     = 'enable';
		$transparent = ( thewebs()->option( 'transparent_header_enable' ) ? 'enable' : 'disable' );
		if ( ( is_singular() || is_front_page() ) && ! is_home() ) {
			...
			if ( isset( $postsidebar ) && ! empty( $postsidebar ) && 'defualt' !== $postsidebar && 'default' !== $postsidebar ) {
				...
				$sidebar_id = thewebs()->option( $post_type . '_sidebar_id', $sidebar_id );
			}
			// Transparent.
			if ( isset( $posttrans ) && ( 'enable' === $posttrans || 'disable' === $posttrans ) ) {
				$transparent = $posttrans;
			} else {
				$option_trans = thewebs()->option( 'transparent_header_' . $trans_type, null );
			...
			if ( isset( $posttitle ) && ( 'above' === $posttitle || 'normal' === $posttitle || 'hide' === $posttitle ) ) {
				...
				$option_title_layout = thewebs()->option( $post_type . '_title_layout' );
				if ( 'above' === $option_title_layout || 'normal' === $option_title_layout ) {
					$title = $option_title_layout;
				}
			} else {
				$option_title = thewebs()->option( $post_type . '_title' );
			...
			if ( isset( $postvpadding ) && ( 'show' === $postvpadding || 'hide' === $postvpadding || 'top' === $postvpadding || 'bottom' === $postvpadding ) ) {
				...
				$option_vpadding = thewebs()->option( $post_type . '_vertical_padding' );
			...
			// Post Navigation.
			if ( 'post' === $post_type ) {
				$option_navigation = thewebs()->option( $post_type . '_navigation' );
				if ( $option_navigation ) {
					$navigation = 'show';
				}
			}
			// Post Comments.
			$option_comments = thewebs()->option( $post_type . '_comments' );
			if ( $option_comments ) {
				$comments = 'show';
			}
			...
			if ( isset( $postboxed ) && ( 'unboxed' === $postboxed || 'boxed' === $postboxed ) ) {
				...
				$option_boxed = thewebs()->option( $post_type . '_content_style' );
			...
			if ( isset( $postfeature ) && ( 'show' === $postfeature || 'hide' === $postfeature ) ) {
				...
				$option_feature = thewebs()->option( $post_type . '_feature' );
				if ( $option_feature ) {
					$feature = 'show';
				}
			}
			...
			if ( isset( $postf_position ) && ( 'above' === $postf_position || 'behind' === $postf_position || 'below' === $postf_position ) ) {
				...
				$option_f_position = thewebs()->option( $post_type . '_feature_position' );
			...
			if ( isset( $postlayout ) && ( 'narrow' === $postlayout || 'fullwidth' === $postlayout ) ) {
				...
				$option_layout = thewebs()->option( $post_type . '_layout' );
			...
			if ( isset( $archivesidebar ) && ! empty( $archivesidebar ) && 'default' !== $archivesidebar && 'defualt' !== $archivesidebar ) {
				...
				$sidebar_id = thewebs()->option( $archive_type . '_sidebar_id', $sidebar_id );
			}
			// Archive Transparent.
			if ( isset( $archivetrans ) && ( 'enable' === $archivetrans || 'disable' === $archivetrans ) ) {
				$transparent = $archivetrans;
			} else {
				$option_trans = thewebs()->option( 'transparent_header_' . $trans_type );
				if ( true === $option_trans ) {
					$transparent = 'disable';
				}
			}
			...
			if ( isset( $archivetitle ) && ( 'above' === $archivetitle || 'normal' === $archivetitle || 'hide' === $archivetitle ) ) {
				...
				$option_title = thewebs()->option( $archive_type . '_title' );
				if ( false === $option_title ) {
					$title = 'hide';
				} else {
					$option_title_layout = thewebs()->option( $archive_type . '_title_layout' );
					if ( empty( $option_title_layout ) ) {
						$option_title_layout = thewebs()->option( 'post_archive_title_layout' );
					}
					if ( 'above' === $option_title_layout || 'normal' === $option_title_layout ) {
						$title = $option_title_layout;
					}
			...
			if ( is_home() && is_front_page() ) {
				if ( ! thewebs()->option( 'post_archive_home_title' ) ) {
					$title = 'hide';
				}
			}
			if ( is_404() ) {
			...
			if ( isset( $archiveboxed ) && ( 'unboxed' === $archiveboxed || 'boxed' === $archiveboxed ) ) {
				...
				$option_boxed = thewebs()->option( $archive_type . '_content_style' );
				if ( empty( $option_boxed ) ) {
					$option_boxed = thewebs()->option( 'post_archive_content_style' );
				}
				if ( 'unboxed' === $option_boxed || 'boxed' === $option_boxed ) {
					$boxed = $option_boxed;
				}
			...
			if ( isset( $archivevpadding ) && ( 'show' === $archivevpadding || 'hide' === $archivevpadding || 'top' === $archivevpadding || 'bottom' === $archivevpadding ) ) {
				...
				$option_vpadding = thewebs()->option( $archive_type . '_vertical_padding' );
			...
			if ( isset( $archivelayout ) && ( 'narrow' === $archivelayout || 'fullwidth' === $archivelayout ) ) {
				...
				$option_layout = thewebs()->option( $archive_type . '_layout' );
				if ( empty( $option_layout ) ) {
					$option_layout = thewebs()->option( 'post_archive_layout' );
				}
				if ( 'narrow' === $option_layout || 'fullwidth' === $option_layout ) {
					$layout = $option_layout;
				}
	...
	public function filter_body_classes( array $classes ) : array {
		...
		if ( self::has_sidebar() ) {
			...
			if ( thewebs()->option( 'sidebar_sticky' ) ) {
				if ( thewebs()->option( 'sidebar_sticky_last_widget' ) ) {
