<?php
/**
 * Thewebs\Breadcrumbs\Component class
 *
 * @package thewebs
 */

namespace Thewebs\Breadcrumbs;
...
class Component implements Component_Interface, Templating_Component_Interface {
	...
	public function print_breadcrumb( $args = array() ) {
		if ( 'rankmath' === thewebs()->option( 'breadcrumb_engine' ) ) {
			if ( function_exists( 'rank_math_the_breadcrumbs' ) ) {
				echo '<div class="thewebs-breadcrumbs rankmath-bc-wrap">';
				rank_math_the_breadcrumbs();
				echo '</div>';
			}
		} elseif ( 'yoast' === thewebs()->option( 'breadcrumb_engine' ) ) {
			if ( function_exists( 'yoast_breadcrumb' ) ) {
				yoast_breadcrumb( '<div class="thewebs-breadcrumbs yoast-bc-wrap">','</div>' );
			}
		} elseif ( 'seopress' === thewebs()->option( 'breadcrumb_engine' ) ) {
			if ( function_exists( 'seopress_display_breadcrumbs' ) ) {
				echo '<div class="thewebs-breadcrumbs seopress-bc-wrap">';
				seopress_display_breadcrumbs();
				echo '</div>';
			}
		} else {
			if ( function_exists( 'is_bbpress' ) && is_bbpress() && function_exists( 'bbp_breadcrumb' ) ) {
				echo '<div class="thewebs-breadcrumbs bbpress-topic-meta">';
				bbp_breadcrumb();
				echo '</div>';
			} else {
				echo thewebs()->get_breadcrumb( $args );
			}
		}
	}
	/**
	...
	public function get_breadcrumb( $args = array() ) {
		$this->args = apply_filters(
			'thewebs_local_breadcrumb_args',
			wp_parse_args(
				$args,
				array(
					'home_title'     => __( 'Home', 'thewebs' ),
					'404_title'      => __( 'Error 404', 'thewebs' ),
					'search_title'   => __( 'Search results for', 'thewebs' ),
					'page'           => __( 'Page', 'thewebs' ),
					'show_shop'      => true,
					'show_title'     => true,
					'color_style'    => '',
					'blog_id'        => '',
		...
		$this->settings = wp_parse_args(
			apply_filters( 'thewebs_breadcrumb_args', array() ),
			array(
				'home'             => true,
				'home_icon'        => thewebs()->option( 'breadcrumb_home_icon' ),
				'before'           => '<span class="thewebs-bread-current">',
				'after'            => '</span>',
				'home_link'        => home_url( '/' ),
				'wrap_before'      => '<nav id="thewebs-breadcrumbs" aria-label="' . esc_attr__( 'Breadcrumbs', 'thewebs' ) . '"  class="thewebs-breadcrumbs"><div class="thewebs-breadcrumb-container"' . ( $this->args['color_style'] ? ' style="' . esc_attr( $this->args['color_style'] ) . '"' : '' ) . '>',
				'wrap_after'       => '</div></nav>',
				'delimiter'        => apply_filters( 'thewebs_breadcrumb_delimiter', '/' ),
		...
		$this->post_types = wp_parse_args(
			apply_filters( 'thewebs_breadcrumb_post_types', array() ),
			array(
				'product'      => array(
					'post_type'     => 'product',
				...
				'ht_kb'      => array(
					...
					'archive_label' => esc_html__( 'Knowledge Base', 'thewebs' ),
				),
			)
		);
		$html = '';
		...
		if ( ! is_front_page() ) {
			...
			$html = apply_filters( 'thewebs_breadcrumbs_after_home', $html );
			if ( is_home() ) {
		 ...
		 */
		return apply_filters( 'thewebs_breadcrumb_html', $html );
	}
	/**
	...
	private function get_crumbs_frontpage() {
		if ( $this->settings['home_icon'] ) {
			$output = $this->settings['link_before'] . '<a href="' . esc_url( $this->settings['home_link'] ) . '" title="'. esc_attr( $this->args['home_title'] ) . '" itemprop="url" class="thewebs-bc-home thewebs-bc-home-icon" ' . ( $this->args['color_style'] ? 'style="' . esc_attr( $this->args['color_style'] ) . '"' : '' ) . '>' . $this->settings['link_in_before'] . thewebs()->get_icon( 'home' ) . $this->settings['link_in_after'] . '</a>' . $this->settings['link_after'];
		} else {
			$output = $this->settings['link_before'] . '<a href="' . esc_url( $this->settings['home_link'] ) . '" itemprop="url" class="thewebs-bc-home" ' . ( $this->args['color_style'] ? 'style="' . esc_attr( $this->args['color_style'] ) . '"' : '' ) . '>' . $this->settings['link_in_before'] . esc_html( $this->args['home_title'] ) . $this->settings['link_in_after'] . '</a>' . $this->settings['link_after'];
		}
		return $output;
	}
