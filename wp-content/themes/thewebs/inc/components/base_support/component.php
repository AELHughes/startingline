<?php
/**
 * Thewebs\Base_Support\Component class
 *
 * @package thewebs
 */

namespace Thewebs\Base_Support;
 ...
 * * `thewebs()->get_version()`
 * * `thewebs()->get_post_types()`
 * * `thewebs()->get_asset_version( string $filepath )`
 */
class Component implements Component_Interface, Templating_Component_Interface {
	...
	public function initialize() {
		...
		add_action( 'admin_notices', array( $this, 'thewebs_starter_templates_notice' ) );
		add_action( 'admin_notices', array( $this, 'thewebs_turn_off_gutenberg_plugin_notice' ) );
		add_action( 'wp_ajax_thewebs_dismiss_notice', array( $this, 'ajax_dismiss_starter_notice' ) );
		add_action( 'wp_ajax_thewebs_dismiss_gutenberg_notice', array( $this, 'ajax_dismiss_gutenberg_notice' ) );
		add_action( 'wp_ajax_thewebs_install_starter', array( $this, 'install_plugin_ajax_callback' ) );
	...
	/**
	 * Add Notice for Thewebs Starter templates
	 */
	public function thewebs_starter_templates_notice() {
		if ( defined( 'THEWEBS_STARTER_TEMPLATES_VERSION' ) || get_option( 'thewebs_starter_plugin_notice' ) || ! current_user_can( 'install_plugins' ) ) {
			return;
		}
		...
		?>
		<?php
		wp_enqueue_script( 'thewebs-starter-install' );
		wp_localize_script(
			'thewebs-starter-install',
			'thewebsStarterInstall',
			array(
	 ...
	 */
	public function thewebs_turn_off_gutenberg_plugin_notice() {
		if ( ! defined( 'GUTENBERG_VERSION' ) || get_option( 'thewebs_disable_gutenberg_notice' ) || ! current_user_can( 'install_plugins' ) ) {
			return;
		}
		?>
		<div id="thewebs-notice-gutenberg-plugin" class="notice is-dismissible notice-warning">
			<h2 class="notice-title"><?php echo esc_html__( 'Gutenberg Plugin Detected', 'thewebs' ); ?></h2>
			<p class="thewebs-notice-description"><?php /* translators: %s: <a> */ printf( esc_html__( 'The %1$sGutenberg plugin%2$s is not recommended for use in a production site. Many things may be broken by using this plugin. Please deactivate.', 'thewebs' ), '<a href="https://wordpress.org/plugins/gutenberg/" >', '</a>' ); ?></p>
		</div>
		<?php
		wp_enqueue_script( 'thewebs-gutenberg-deactivate' );
		wp_localize_script(
			'thewebs-gutenberg-deactivate',
			'thewebsGutenbergDeactivate',
			array(
				'ajax_url'   => admin_url( 'admin-ajax.php' ),
				'ajax_nonce' => wp_create_nonce( 'thewebs-ajax-verification' ),
			)
	...
	public function ajax_dismiss_gutenberg_notice() {
		...
		if ( ! isset( $_POST['action'] ) || 'thewebs_dismiss_gutenberg_notice' !== $_POST['action'] ) {
			return;
		}
		// Security check: Make sure nonce is OK.
		check_ajax_referer( 'thewebs-ajax-verification', 'security', true );

		// If we got this far, we need to dismiss the notice.
		update_option( 'thewebs_disable_gutenberg_notice', true, false );
	}
	/**
	 * Run check to see if we need to dismiss the notice.
	 * If all tests are successful then call the dismiss_notice() method.
	...
	public function ajax_dismiss_starter_notice() {
		...
		// Sanity check: Early exit if we're not on a wptrt_dismiss_notice action.
		if ( ! isset( $_POST['action'] ) || 'thewebs_dismiss_notice' !== $_POST['action'] ) {
			return;
		}
		// Security check: Make sure nonce is OK.
		check_ajax_referer( 'thewebs-ajax-verification', 'security', true );
		...
		// If we got this far, we need to dismiss the notice.
		update_option( 'thewebs_starter_plugin_notice', true, false );
	}
	...
	public function install_starter_script() {
		wp_register_script( 'thewebs-starter-install', get_template_directory_uri() . '/assets/js/admin-activate.min.js', array( 'jquery' ), THEWEBS_VERSION, false );
		wp_register_script( 'thewebs-gutenberg-deactivate', get_template_directory_uri() . '/assets/js/gutenberg-notice.min.js', array( 'jquery' ), THEWEBS_VERSION, false );
	}
	/**
	 * Active Plugin Check
	 *
	...
	public function install_plugin_ajax_callback() {
		if ( ! check_ajax_referer( 'thewebs-ajax-verification', 'security', false ) ) {
			wp_send_json_error( __( 'Security Error, Please reload the page.', 'thewebs' ) );
		}
		if ( ! current_user_can( 'install_plugins' ) ) {
			wp_send_json_error( __( 'Security Error, Need higher Permissions to install plugin.', 'thewebs' ) );
		}
		...
		if ( 'install' === $status ) {
			$api = plugins_api(
				'plugin_information',
				array(
					'slug' => 'thewebs-starter-templates',
					'fields' => array(
			...
			if ( ! is_wp_error( $api ) ) {
				...
				if ( $installed ) {
					$activate = activate_plugin( 'thewebs-starter-templates/thewebs-starter-templates.php', '', false, true );
		...
		if ( false === $install ) {
			wp_send_json_error( __( 'Error, plugin could not be installed, please install manually.', 'thewebs' ) );
		} else {
	...
	public function custom_excerpt_length( $length ) {
		...
		if ( is_search() ) {
			if ( thewebs()->sub_option( 'search_archive_element_excerpt', 'words' ) ) {
				$length = intval( thewebs()->sub_option( 'search_archive_element_excerpt', 'words' ) );
			}
		} else if ( 'post' === get_post_type() ) {
			if ( thewebs()->sub_option( 'post_archive_element_excerpt', 'words' ) ) {
				$length = intval( thewebs()->sub_option( 'post_archive_element_excerpt', 'words' ) );
			}
		} else {
			if ( thewebs()->sub_option( get_post_type() . '_archive_element_excerpt', 'words' ) ) {
				$length = intval( thewebs()->sub_option( get_post_type() . '_archive_element_excerpt', 'words' ) );
			}
		}
		return $length;
	}
	...
	public function classic_embed_wrap( $cache, $url, $attr = array(), $post_ID = '' ) {
		if ( doing_filter( 'the_content' ) && ! has_blocks() && ! empty( $cache ) && ! empty( $url ) ) {
			...
			$patterns = apply_filters( 'thewebs_maybe_wrap_embed_patterns', $patterns );
	...
	public static function customizer_quick_link() {
		if ( is_customize_preview() ) {
			...
			<div class="customize-partial-edit-shortcut thewebs-custom-partial-edit-shortcut">
				<button aria-label="<?php esc_attr_e( 'Click to edit this element.', 'thewebs' ); ?>" title="<?php esc_attr_e( 'Click to edit this element.', 'thewebs' ); ?>" class="customize-partial-edit-shortcut-button item-customizer-focus"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M13.89 3.39l2.71 2.72c.46.46.42 1.24.03 1.64l-8.01 8.02-5.56 1.16 1.16-5.58s7.6-7.63 7.99-8.03c.39-.39 1.22-.39 1.68.07zm-2.73 2.79l-5.59 5.61 1.11 1.11 5.54-5.65zm-2.97 8.23l5.58-5.6-1.07-1.08-5.59 5.6z"></path></svg></button>
			</div>
			<?php
		}
	}
	...
	public static function get_post_types_to_ignore() {
		if ( is_null( self::$ignore_post_types ) ) {
			...
			self::$ignore_post_types = apply_filters( 'thewebs_customizer_post_type_ignore_array', $ignore_post_types );
		}
	...
	public static function get_transparent_post_types_to_ignore() {
		if ( is_null( self::$transparent_ignore_post_types ) ) {
			...
			self::$transparent_ignore_post_types = apply_filters( 'thewebs_transparent_post_type_ignore_array', $transparent_ignore_post_types );
		}
	...
	public static function get_public_post_types_to_ignore() {
		if ( is_null( self::$public_ignore_post_types ) ) {
			...
			self::$public_ignore_post_types = apply_filters( 'thewebs_public_post_type_ignore_array', $public_ignore_post_types );
		}
	...
	public static function get_post_types() {
		if ( is_null( self::$post_types ) ) {
			...
			self::$post_types = apply_filters( 'thewebs_public_post_type_array', array_merge( $builtin, $post_types ) );
		}
	...
	public static function get_post_types_objects() {
		if ( is_null( self::$post_types_objects ) ) {
			...
			$operator         = 'and';
			$post_types       = get_post_types( $args, $output, $operator );
			self::$post_types_objects = apply_filters( 'thewebs_public_post_type_objects', $post_types );
		}
	...
	public function action_essential_theme_support() {
		if ( 'em' === thewebs()->sub_option( 'content_width', 'unit' ) || 'rem' === thewebs()->sub_option( 'content_width', 'unit' ) ) {
			$thewebs_content_width = intval( thewebs()->sub_option( 'content_width', 'size' ) * 17 );
		} else {
			$thewebs_content_width = thewebs()->sub_option( 'content_width', 'size' );
		}
		$GLOBALS['content_width'] = intval( $thewebs_content_width ); // phpcs:ignore WPThemeReview.CoreFunctionality.PrefixAllGlobals.NonPrefixedVariableFound
	...
	public function add_search_icon( $markup ) {
		$markup = str_replace( '</form>', '<div class="thewebs-search-icon-wrap">' . thewebs()->get_icon( 'search', '', false ) . '</div></form>', $markup );
		return $markup;
	}
	/**
	...
	public function filter_body_classes_add_link_style( array $classes ) : array {
		...
		$classes[] = 'link-style-' . thewebs()->sub_option( 'link_color', 'style' );
