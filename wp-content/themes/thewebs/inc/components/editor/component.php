<?php
/**
 * Thewebs\Editor\Component class
 *
 * @package thewebs
 */

namespace Thewebs\Editor;
...
class Component implements Component_Interface {
	...
	public function initialize() {
		...
		remove_action( 'admin_head-post.php', 'thewebs_blocks_admin_editor_width', 100 );
		remove_action( 'admin_head-post-new.php', 'thewebs_blocks_admin_editor_width', 100 );
		add_filter( 'thewebs_blocks_editor_width', array( $this, 'thewebs_blocks_admin_editor_width_filter' ), 100 );
	...
	public function google_font_add_editor_styles() {
		if ( thewebs()->sub_option( 'base_font', 'google' ) ) {
			$font        = thewebs()->option( 'base_font' );
			if ( $font['family'] ) {
				$font_string = $font['family'] . ':' . ( $font['variant'] ? $font['variant'] : '400' ) . ',700';
				if ( thewebs()->sub_option( 'heading_font', 'google' ) ) {
					$heading_font = thewebs()->option( 'heading_font' );
		...
		} elseif ( thewebs()->sub_option( 'heading_font', 'google' ) ) {
			$font        = thewebs()->option( 'heading_font' );
			if ( $font['family'] ) {
	...
	/**
	 * Adds filter for Thewebs Editor Width.
	 *
	 * @param bool $enable the editor width enable.
	 */
	public function thewebs_blocks_admin_editor_width_filter( $enable ) {
		return false;
	}
	/**
	...
	public function filter_admin_body_class( $classes ) {
		...
		if ( 'post' == $screen->base ) {
			...
			$post_layout = apply_filters( 'thewebs_post_layout', $post_layout );
			...
			if ( isset( $vpadding ) && ( 'show' === $vpadding || 'hide' === $vpadding ) ) {
				...
				$option_padding = thewebs()->option( $post_type . '_vertical_padding' );
				if ( $option_padding ) {
	...
	public function action_add_editor_support() {
		...
		if ( apply_filters( 'thewebs-theme-block-templates-support', true ) ) {
		...
		add_theme_support(
			...
			array(
				array(
					...
					'shortName' => __( 'S', 'thewebs' ),
					'size'      => 14,
					'slug'      => 'small',
				),
				array(
					'name'      => __( 'Medium', 'thewebs' ),
					'shortName' => __( 'M', 'thewebs' ),
					'size'      => 24,
					'slug'      => 'medium',
				),
				array(
					'name'      => __( 'Large', 'thewebs' ),
					'shortName' => __( 'L', 'thewebs' ),
					'size'      => 32,
					'slug'      => 'large',
				),
				array(
					'name'      => __( 'Larger', 'thewebs' ),
					'shortName' => __( 'XL', 'thewebs' ),
					'size'      => 40,
					'slug'      => 'larger',
				),
			)
