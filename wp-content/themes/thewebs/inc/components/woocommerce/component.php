<?php
/**
 * Thewebs\Woocommerce\Component class
 *
 * @package thewebs
 */

namespace Thewebs\Woocommerce;
...
class Component implements Component_Interface {
	...
	public function initialize() {
		...
		add_filter( 'thewebs_blocks_carousel_woocommerce_product_loop_start', array( $this, 'product_loop_start' ), 5 );
		// Add Post grid class.
		...
		add_action( 'thewebs_woocommerce_template_before_block_loop', array( $this, 'add_filter_for_add_to_cart_link' ) );
		add_action( 'thewebs_woocommerce_template_after_block_loop', array( $this, 'add_filter_for_add_to_cart_link' ) );
		// My Account.
		...
		add_action( 'thewebs_before_main_content', array( $this, 'wc_print_notices_none_woo' ) );
		...
		add_filter( 'thewebs_shop_kit_product_loop_classes', array( $this, 'add_product_archive_loop_classes_shopkit' ), 10 );
	...
	public function setup_shop_content_filter() {
		global $wp_embed;
		add_filter( 'thewebs_theme_shop_content', array( $wp_embed, 'run_shortcode' ), 8 );
		add_filter( 'thewebs_theme_shop_content', array( $wp_embed, 'autoembed'     ), 8 );
		add_filter( 'thewebs_theme_shop_content', 'do_blocks' );
		add_filter( 'thewebs_theme_shop_content', 'wptexturize' );
		add_filter( 'thewebs_theme_shop_content', 'convert_chars' );
		add_filter( 'thewebs_theme_shop_content', 'shortcode_unautop' );
		add_filter( 'thewebs_theme_shop_content', 'do_shortcode', 11 );
		add_filter( 'thewebs_theme_shop_content', 'convert_smilies', 20 );
	}
	/**
		...
		if ( is_post_type_archive( 'product' ) && in_array( absint( get_query_var( 'paged' ) ), array( 0, 1 ), true ) ) {
			...
			if ( $shop_page ) {
				if ( has_blocks( $shop_page->post_content ) ) {
					$description = apply_filters( 'thewebs_theme_shop_content', $shop_page->post_content );
				} else {
	...
	public function remove_product_tab_heading( $heading ) {
		if ( ! thewebs()->option( 'product_tab_title' ) ) {
			return false;
		}
		return $heading;
	}
	...
	public function category_widget_toggle_script( $args ) {
		wp_enqueue_script( 'thewebs-shop-toggle' );
		return $args;
	}
	/**
	...
	public function related_products_columns( $args ) {
		$columns = absint( thewebs()->option( 'product_related_columns' ) );
	...
	public function add_product_archive_loop_classes_shopkit( $classes ) {
		$product_image_hover_style = thewebs()->option( 'product_archive_image_hover_switch' );
		$product_btn_style = thewebs()->option( 'product_archive_button_style' );
	...
	public function custom_block_html( $html, $data, $product ) {
		...
		if ( 'button' === $product_btn_style ) {
			$cart_text = sprintf(
				...
				esc_html( $product->add_to_cart_text() ) . '' . thewebs()->get_icon( 'spinner' ) . '' . thewebs()->get_icon( 'check' )
			);
		} else {
			$cart_text = sprintf(
				'<a href="%s" %s>%s</a>',
				esc_url( $product->add_to_cart_url() ),
				wc_implode_html_attributes( $attributes ),
				esc_html( $product->add_to_cart_text() ) . '' . thewebs()->get_icon( 'arrow-right-alt' ) . '' . thewebs()->get_icon( 'spinner' ) . '' . thewebs()->get_icon( 'check' )
			);
		}
		...
		if ( thewebs()->option( 'product_archive_button_align' ) ) {
			$align_archive_button = 'block-align-buttons-bottom';
		}
		$product_style     = thewebs()->option( 'product_archive_style' );
		...
		$boxed = thewebs()->option( 'product_archive_content_style' );
	...
	public function single_product_layout() {
		// Product Single Cat.
		$category_element = thewebs()->option( 'product_content_element_category' );
		...
		// Product Rating.
		$rating_element = thewebs()->option( 'product_content_element_rating' );
		if ( isset( $rating_element ) && is_array( $rating_element ) && false === $rating_element['enabled'] ) {
			remove_action( 'woocommerce_single_product_summary', 'woocommerce_template_single_rating' );
		}
		// Product Price.
		$price_element = thewebs()->option( 'product_content_element_price' );
		...
		// Product Payments.
		$payments_element = thewebs()->option( 'product_content_element_payments' );
		...
		// Product Meta.
		$meta_element = thewebs()->option( 'product_content_element_product_meta' );
		if ( isset( $meta_element ) && is_array( $meta_element ) && false === $meta_element['enabled'] ) {
			remove_action( 'woocommerce_single_product_summary', 'woocommerce_template_single_meta', 40 );
		}
		// Product Sharing.
		$sharing_element = thewebs()->option( 'product_content_element_sharing' );
		...
		if ( false === thewebs()->option( 'product_additional_weight_dimensions' ) ) {
			add_filter( 'wc_product_enable_dimensions_display', '__return_false' );
		}
		// Related Products.
		if ( false === thewebs()->option( 'product_related' ) ) {
	...
	public function woocommerce_product_single_extras() {
		...
		if ( isset( $extras_element ) && is_array( $extras_element ) && isset( $extras_element['feature_1'] ) && ! empty( $extras_element['feature_1'] ) ) {
			echo '<li>' . thewebs()->get_icon( isset( $extras_element['feature_1_icon'] ) && ! empty( $extras_element['feature_1_icon'] ) ? $extras_element['feature_1_icon'] : 'shield_check' ) . ' ' . wp_kses_post( $extras_element['feature_1'] ) . '</li>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
		if ( isset( $extras_element ) && is_array( $extras_element ) && isset( $extras_element['feature_2'] ) && ! empty( $extras_element['feature_2'] ) ) {
			echo '<li>' . thewebs()->get_icon( isset( $extras_element['feature_2_icon'] ) && ! empty( $extras_element['feature_2_icon'] ) ? $extras_element['feature_2_icon'] : 'shield_check' ) . ' ' . wp_kses_post( $extras_element['feature_2'] ) . '</li>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
		if ( isset( $extras_element ) && is_array( $extras_element ) && isset( $extras_element['feature_3'] ) && ! empty( $extras_element['feature_3'] ) ) {
			echo '<li>' . thewebs()->get_icon( isset( $extras_element['feature_3_icon'] ) && ! empty( $extras_element['feature_3_icon'] ) ? $extras_element['feature_3_icon'] : 'shield_check' ) . ' ' . wp_kses_post( $extras_element['feature_3'] ) . '</li>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
		if ( isset( $extras_element ) && is_array( $extras_element ) && isset( $extras_element['feature_4'] ) && ! empty( $extras_element['feature_4'] ) ) {
			echo '<li>' . thewebs()->get_icon( isset( $extras_element['feature_4_icon'] ) && ! empty( $extras_element['feature_4_icon'] ) ? $extras_element['feature_4_icon'] : 'shield_check' ) . ' ' . wp_kses_post( $extras_element['feature_4'] ) . '</li>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
		if ( isset( $extras_element ) && is_array( $extras_element ) && isset( $extras_element['feature_5'] ) && ! empty( $extras_element['feature_5'] ) ) {
			echo '<li>' . thewebs()->get_icon( isset( $extras_element['feature_5_icon'] ) && ! empty( $extras_element['feature_5_icon'] ) ? $extras_element['feature_5_icon'] : 'shield_check' ) . ' ' . wp_kses_post( $extras_element['feature_5'] ) . '</li>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
		echo '</ul>';
		echo '</div>';
	}
	...
	public function woocommerce_product_single_payments() {
		...
		if ( isset( $payments_element ) && is_array( $payments_element ) && isset( $payments_element['stripe'] ) && true === $payments_element['stripe'] ) {
			echo '<li class="single-product-payments-stripe">' . thewebs()->get_icon( 'stripe' ) . '</li>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
		if ( isset( $payments_element ) && is_array( $payments_element ) && isset( $payments_element['visa'] ) && true === $payments_element['visa'] ) {
			echo '<li class="single-product-payments-visa">' . thewebs()->get_icon( ( 'inherit' !== $colors ? 'visa_gray' : 'visa' ) ) . '</li>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
		if ( isset( $payments_element ) && is_array( $payments_element ) && isset( $payments_element['mastercard'] ) && true === $payments_element['mastercard'] ) {
			echo '<li class="single-product-payments-mastercard">' . thewebs()->get_icon( ( 'inherit' !== $colors ? 'mastercard_gray' : 'mastercard' ) ) . '</li>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
		if ( isset( $payments_element ) && is_array( $payments_element ) && isset( $payments_element['amex'] ) && true === $payments_element['amex'] ) {
			echo '<li class="single-product-payments-amex">' . thewebs()->get_icon( ( 'inherit' !== $colors ? 'amex_gray' : 'amex' ) ) . '</li>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
		if ( isset( $payments_element ) && is_array( $payments_element ) && isset( $payments_element['discover'] ) && true === $payments_element['discover'] ) {
			echo '<li class="single-product-payments-discover">' . thewebs()->get_icon( ( 'inherit' !== $colors ? 'discover_gray' : 'discover' ) ) . '</li>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
		if ( isset( $payments_element ) && is_array( $payments_element ) && isset( $payments_element['paypal'] ) && true === $payments_element['paypal'] ) {
			echo '<li class="single-product-payments-paypal">' . thewebs()->get_icon( ( 'inherit' !== $colors ? 'paypal_gray' : 'paypal' ) ) . '</li>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
		if ( isset( $payments_element ) && is_array( $payments_element ) && isset( $payments_element['applepay'] ) && true === $payments_element['applepay'] ) {
			echo '<li class="single-product-payments-applepay">' . thewebs()->get_icon( ( 'inherit' !== $colors ? 'applepay_gray' : 'applepay' ) ) . '</li>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
	...
	public function woo_extra_body_classes( $classes ) {
		...
		if ( is_store_notice_showing() ) {
			$placement = thewebs()->option( 'woo_store_notice_placement' );
			$classes[] = esc_attr( 'thewebs-store-notice-placement-' . $placement );
			if ( 'above' === $placement ) {
				if ( thewebs()->option( 'woo_store_notice_hide_dismiss' ) ) {
					add_filter( 'woocommerce_demo_store', array( $this, 'woocommerce_demo_store_remove_dismiss' ), 15, 2 );
				}
				remove_action( 'wp_footer', 'woocommerce_demo_store' );
				add_action( 'thewebs_before_header', 'woocommerce_demo_store' );
			}
		}
	...
	public function my_account_body_classes( array $classes ) : array {
		if ( is_account_page() ) {
			$classes[] = 'thewebs-account-nav-' . esc_attr( thewebs()->option( 'woo_account_navigation_layout' ) );
		}
		return $classes;
	}
	/**
	...
	public function single_product_body_classes( array $classes ) : array {
		if ( is_product() ) {
			$cart_element = thewebs()->option( 'product_content_element_add_to_cart' );
			...
			} else if ( thewebs()->option( 'product_large_cart_button' ) ) {
				$size = 'large';
			} else {
				$size = 'normal';
			}
			$classes[] = 'product-tab-style-' . esc_attr( thewebs()->option( 'product_tab_style' ) );
			$classes[] = 'product-variation-style-' . esc_attr( thewebs()->option( 'variation_direction' ) );
			$classes[] = 'thewebs-cart-button-' . esc_attr( $size );
		}
		return $classes;
	}
	/**
	...
	public function get_refreshed_fragments_mini( $fragments ) {
		...
		echo '<div class="thewebs-mini-cart-refresh">';
		woocommerce_mini_cart();
		echo '</div>';
		$fragments['div.thewebs-mini-cart-refresh'] = ob_get_clean();
	...
	public function check_for_fragment_support() {
		...
		if ( thewebs()->option( 'header_cart_show_total' ) ) {
			self::$show_cart_total = true;
			add_filter( 'woocommerce_add_to_cart_fragments', array( $this, 'get_refreshed_fragments_number' ) );
		}
		if ( 'slide' === thewebs()->option( 'header_cart_style' ) || 'slide' === thewebs()->option( 'header_mobile_cart_style' ) || 'dropdown' === thewebs()->option( 'header_cart_style' ) ) {
	...
	public function check_conditional_for_fragment_support() {
		if ( thewebs()->option( 'header_cart_show_total' ) && ! self::$show_cart_total ) {
			self::$show_cart_total = true;
			add_filter( 'woocommerce_add_to_cart_fragments', array( $this, 'get_refreshed_fragments_number' ) );
		}
		if ( ( 'slide' === thewebs()->option( 'header_cart_style' ) || 'slide' === thewebs()->option( 'header_mobile_cart_style' ) || 'dropdown' === thewebs()->option( 'header_cart_style' ) ) && ! self::$show_mini_cart ) {
			self::$show_mini_cart = true;
			add_filter( 'woocommerce_add_to_cart_fragments', array( $this, 'get_refreshed_fragments_mini' ) );
		}
	}
	...
	public function cart_in_header() {
		$in_header = false;
		$elements  = thewebs()->option( 'header_desktop_items' );
	...
	public function action_enqueue_product_scripts() {
		if ( class_exists( 'woocommerce' ) && is_product() ) {
			...
			// wp_script_add_data( 'thewebs-product-cls', 'async', true );
			// wp_script_add_data( 'thewebs-product-cls', 'precache', true );
		}
	}
	/**
	...
	public function action_enqueue_scripts() {
		...
		if ( thewebs()->option( 'custom_quantity' ) ) {
			...
			wp_enqueue_script(
				'thewebs-shop-spinner',
				get_theme_file_uri( '/assets/js/shop-spinner.min.js' ),
				array( 'jquery' ),
				THEWEBS_VERSION,
				true
			);
			wp_script_add_data( 'thewebs-shop-spinner', 'async', true );
			wp_script_add_data( 'thewebs-shop-spinner', 'precache', true );
		}
		if ( class_exists( 'woocommerce' ) && is_cart() ) {
			...
			wp_script_add_data( 'thewebs-cart-update', 'async', true );
			wp_script_add_data( 'thewebs-cart-update', 'precache', true );
		}
		...
		wp_script_add_data( 'thewebs-shop-toggle', 'async', true );
		wp_script_add_data( 'thewebs-shop-toggle', 'precache', true );
		wp_localize_script(
			'thewebs-shop-toggle',
			'thewebsShopConfig',
			array(
	...
	public function wc_print_notices_none_woo() {
		if ( ! is_shop() && ! is_woocommerce() && ! is_cart() && ! is_checkout() && ! is_account_page() ) {
			if ( function_exists( 'wc_print_notices' ) ) {
				echo '<div class="woocommerce thewebs-woo-messages-none-woo-pages woocommerce-notices-wrapper">';
	...
	public function cart_summary_title() {
		echo '<div class="cart-summary"><h2>' . esc_html__( 'Cart Summary', 'thewebs' ) . '</h2></div>';
	}
	/**
	 * Insert the Cart Form wrap.
	 */
	public function cart_form_wrap_before() {
		echo '<div class="thewebs-woo-cart-form-wrap">';
	}
	/**
	...
	public function myaccount_nav_avatar() {
		...
		if ( thewebs()->option( 'woo_account_navigation_avatar' ) && 0 !== $current_user->ID ) {
			?>
			<div class="thewebs-account-avatar">
				<div class="thewebs-customer-image">
				<a class="kt-link-to-gravatar" href="https://gravatar.com/"  rel="no" title="<?php echo esc_attr__( 'Update Profile Photo', 'thewebs' ); ?>">
					<?php echo get_avatar( $current_user->ID, 60 ); ?>
				</a>
				</div>
				<div class="thewebs-customer-name">
	...
	public function product_loop_start( $output ) {
		...
		$product_btn_style = thewebs()->option( 'product_archive_button_style' );
		$product_image_hover_style = thewebs()->option( 'product_archive_image_hover_switch' );
		if ( is_main_query() && is_archive() && wc_get_loop_prop( 'is_paginated' ) && apply_filters( 'thewebs_enabled_product_archive_attributes', true, $GLOBALS['woocommerce_loop'] ) ) {
	...
	public function get_archive_infinite_attributes() {
		$attributes = '';
		return apply_filters( 'thewebs_product_archive_infinite_attributes', $attributes );
	}
	/**
	...
	public function archive_loop_image_link_open() {
		...
		$has_hover_image = '';
		if ( 'none' !== thewebs()->option( 'product_archive_image_hover_switch' ) ) {
			if ( is_a( $product, 'WC_Product' ) ) {
				$attachment_ids = $product->get_gallery_image_ids();
				if ( $attachment_ids ) {
	...
	public function archive_content_wrap_start() {
		echo apply_filters( 'thewebs_archive_content_wrap_start', '<div class="product-details content-bg entry-content-wrap">' );
	}
	/**
	 * Close the content wrap.
	 */
	public function archive_content_wrap_end() {
		echo apply_filters( 'thewebs_archive_content_wrap_end', '</div>' );
	}
	/**
	...
	public function archive_excerpt() {
		if ( is_main_query() && is_archive() ) {
			...
			if ( 1 === $columns || thewebs()->option( 'product_archive_toggle' ) ) {
				global $post;
	...
	public function archive_loop_top() {
		...
		if ( thewebs()->option( 'product_archive_show_results_count' ) || thewebs()->option( 'product_archive_show_order' ) || thewebs()->option( 'product_archive_toggle' ) || apply_filters( 'thewebs_product_archive_show_top_row', false ) ) {
			echo '<div class="thewebs-shop-top-row">';
			do_action( 'thewebs_woocommerce_before_shop_loop_top_row' );
			if ( thewebs()->option( 'product_archive_show_results_count' ) ) {
				echo '<div class="thewebs-shop-top-item thewebs-woo-results-count">';
					woocommerce_result_count();
				echo '</div>';
			}
			if ( thewebs()->option( 'product_archive_show_order' ) ) {
				echo '<div class="thewebs-shop-top-item thewebs-woo-ordering">';
					woocommerce_catalog_ordering();
				echo '</div>';
			}
			if ( thewebs()->option( 'product_archive_toggle' ) ) {
				echo '<div class="thewebs-shop-top-item thewebs-woo-toggle">';
					$this->toggle_list();
				echo '</div>';
			}
			do_action( 'thewebs_woocommerce_after_shop_loop_top_row' );
			echo '</div>';
		}
	}
	/**
	...
	public function toggle_list() {
		...
		if ( 1 === wc_get_loop_prop( 'columns' ) ) {
			echo '<div class="thewebs-product-toggle-container thewebs-product-toggle-outer thewebs-single-to-grid">';
				echo '<button title="' . esc_attr__( 'List View', 'thewebs' ) . '" class="thewebs-toggle-shop-layout thewebs-toggle-list toggle-active" data-archive-toggle="list">';
					thewebs()->print_icon( 'list', '', false );
				echo '</button>';
				echo '<button title="' . esc_attr__( 'Grid View', 'thewebs' ) . '" class="thewebs-toggle-shop-layout thewebs-toggle-grid" data-archive-toggle="grid">';
					thewebs()->print_icon( 'grid', '', false );
				echo '</button>';
			echo '</div>';
		} else {
			echo '<div class="thewebs-product-toggle-container thewebs-product-toggle-outer">';
				echo '<button title="' . esc_attr__( 'Grid View', 'thewebs' ) . '" class="thewebs-toggle-shop-layout thewebs-toggle-grid toggle-active" data-archive-toggle="grid">';
					thewebs()->print_icon( 'grid', '', false );
				echo '</button>';
				echo '<button title="' . esc_attr__( 'List View', 'thewebs' ) . '" class="thewebs-toggle-shop-layout thewebs-toggle-list" data-archive-toggle="list">';
					thewebs()->print_icon( 'list', '', false );
				echo '</button>';
			echo '</div>';
		}
	...
	public function output_product_above() {
		if ( is_product() && 'breadcrumbs' === thewebs()->option( 'product_above_layout' ) ) {
	...
	public function output_product_above_title() {
		if ( is_product() && 'title' === thewebs()->option( 'product_above_layout' ) ) {
	...
	public function output_content_wrapper() {
		...
		if ( ! is_product() && thewebs()->show_in_content_title() ) {
